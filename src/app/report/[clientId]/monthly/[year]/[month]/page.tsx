...
function KpiCard({ label, value, varVal, icon: Icon, accent = false }: {
    ...
    return (
        <div className={`rounded-2xl p-5 border ${accent ? 'bg-[#1E3A5F] border-[#1E3A5F] text-white' : 'bg-white border-gray-100 text-gray-900'} shadow-sm print:shadow-none`}>
...
export default function MonthlyReportPage() {
    ...
    const periodoLabel = useMemo(() => {
        try {
            return format(parseISO(`${year}-${String(monthNum).padStart(2, '0')}-01`), 'MMMM yyyy', { locale: es })
        } catch { return `${month}/${year}` }
    }, [year, month, monthNum])

    useEffect(() => {
        fetch(`/api/reports/monthly?clientId=${clientId}&year=${year}&month=${month}`)
            .then(r => r.json())
    ...
    if (error || !data) return (
        <div className="max-w-5xl mx-auto px-4 py-20 text-center">
            ...
            <p className="text-gray-600">{error || 'Reporte no disponible'}</p>
        </div>
    )

    const { client, summary, daily, campaigns, spend_distribution, audience, creatives, previous_month, notes } = data
            ...
            <div className="max-w-5xl mx-auto px-4 py-10 space-y-10 print:py-4 print:space-y-6">
                ...
                <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-8 flex flex-col md:flex-row items-start md:items-center justify-between gap-6 print:shadow-none">
                    <div className="flex items-center gap-5">
                        {client.logo_url
                            ? <img src={client.logo_url} alt={client.name} className="h-16 w-16 object-contain rounded-xl border border-gray-100" />
                            : (
                        ...
                        <div>
                            ...
                            <h1 className="text-2xl md:text-3xl font-bold text-[#1E3A5F] capitalize">{client.name}</h1>
                ...
                <div>
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                        <KpiCard
                            label="Inversión Total" accent
                            value={fmt(summary.spend, 2, `${client.currency} `)}
                            varVal={variation(summary.spend, previous_month.spend, false)}
                            icon={DollarSign}
                        />
                        <KpiCard
                            label="Resultados"
                            value={summary.results.toLocaleString('es-MX')}
                            varVal={variation(summary.results, previous_month.results)}
                            icon={Target}
                        />
                        <KpiCard
                            label="CPA"
                            value={fmt(summary.cpa, 2, `${client.currency} `)}
                            varVal={variation(summary.cpa, previous_month.cpa, true)}
                            icon={BarChart3}
                        />
                        <KpiCard
                            label="CTR"
                            value={pct(summary.ctr)}
                            varVal={variation(summary.ctr, previous_month.ctr)}
                            icon={MousePointerClick}
                        />
                        <KpiCard
                            label="Alcance"
                            value={summary.reach.toLocaleString('es-MX')}
                            varVal={variation(summary.reach, previous_month.reach)}
                            icon={Users}
                        />
                        <KpiCard
                            label="Impresiones"
                            value={summary.impressions.toLocaleString('es-MX')}
                            icon={Eye}
                        />
                        <KpiCard
                            label="CPM"
                            value={fmt(summary.cpm, 2, `${client.currency} `)}
                            icon={DollarSign}
                        />
                        {summary.roas !== null
                            ? <KpiCard label="ROAS" value={fmt(summary.roas, 2, '', 'x')} icon={TrendingUp} />
                            : <KpiCard label="Clics" value={summary.clicks.toLocaleString('es-MX')} icon={MousePointerClick} />
                        }
                    </div>
                </div>
                ...
                {daily.length > 0 && (
                    <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 print:shadow-none">
                        ...
                        <p className="text-xs text-gray-400 mb-5">Gasto e resultados por día del mes</p>
                        <ResponsiveContainer width="100%" height={260}>
                            <AreaChart data={daily} margin={{ top: 5, right: 20, left: 0, bottom: 0 }}>
                                <defs>
                                ...
                                <Tooltip
                                    ...
                                    labelFormatter={d => { try { return format(parseISO(d as string), "d 'de' MMMM", { locale: es }) } catch { return d as string } }}
                                    formatter={(value: any, name: string) => [
                                        name === 'spend' ? `$${Number(value).toFixed(2)}` : value,
                                        name === 'spend' ? 'Gasto' : 'Resultados'
                                    ]}
                                />
                                <Area yAxisId="left" type="monotone" dataKey="spend" stroke={BRAND_BLUE} strokeWidth={2} fill="url(#gradSpend)" dot={false} />
                                <Area yAxisId="right" type="monotone" dataKey="results" stroke={BRAND_GOLD} strokeWidth={2} fill="url(#gradResults)" dot={false} />
                            ...
                            <table className="w-full text-sm">
                                ...
                                <tbody className="divide-y divide-gray-50">
                                    {sortedCampaigns.map((c, i) => (
                                        <tr key={i} className={`${rowBg(c.roasStatus)} hover:bg-gray-50/60 transition`}>
                                            <td className="px-4 py-3 text-gray-800 font-medium max-w-[240px]">
                                ...
                                <tfoot>
                                    <tr className="bg-[#1E3A5F]/5 border-t-2 border-[#1E3A5F]/10">
                                        ...
                                        <td className="px-3 py-3 text-right font-bold font-mono text-[#1E3A5F]">${summary.spend.toFixed(2)}</td>
                                        <td className="px-3 py-3 text-right font-mono text-gray-600">{summary.impressions.toLocaleString('es-MX')}</td>
                                        <td className="px-3 py-3 text-right font-mono text-gray-600">{summary.clicks.toLocaleString('es-MX')}</td>
                                        <td className="px-3 py-3 text-right font-mono text-gray-600">{summary.ctr !== null ? `${summary.ctr.toFixed(2)}%` : '—'}</td>
                                        <td className="px-3 py-3 text-right font-mono text-gray-600">{summary.cpa !== null ? `$${summary.cpa.toFixed(2)}` : '—'}</td>
                                        <td className="px-3 py-3 text-right font-bold font-mono text-[#1E3A5F]">{summary.results}</td>
                                    </tr>
                                </tfoot>
                            </table>
                        </div>
                ...
                {/* ── BLOQUE 6: Audiencia (if data available) ────────────────── */}
                {(audience.by_age.length > 0 || audience.by_gender.length > 0) && (
                    <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 print:shadow-none">
                        <h2 className="text-lg font-bold text-[#1E3A5F] mb-5">Análisis de Audiencia</h2>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                            {audience.by_age.length > 0 && (
                                <div>
                                    <p className="text-sm font-semibold text-gray-500 mb-3">Por Edad</p>
                                    <ResponsiveContainer width="100%" height={180}>
                                        <BarChart data={audience.by_age} layout="vertical" margin={{ left: 8, right: 8 }}>
                            ...
                            {audience.by_gender.length > 0 && (
                                <div>
                                    <p className="text-sm font-semibold text-gray-500 mb-3">Por Género</p>
                                    <ResponsiveContainer width="100%" height={180}>
                                        <BarChart data={audience.by_gender} layout="vertical" margin={{ left: 8, right: 8 }}>
                                            <XAxis type="number" tick={{ fontSize: 10, fill: '#9CA3AF' }} tickLine={false} axisLine={false} unit="%" />
                ...
                {(creatives.top.length > 0 || creatives.bottom.length > 0) && (
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        {creatives.top.length > 0 && (
                            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 print:shadow-none">
                ...
                <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden print:shadow-none">
                    ...
                    <div className="overflow-x-auto">
                        <table className="w-full text-sm">
                            ...
                            <tbody className="divide-y divide-gray-50">
                                {[
                                    { label: 'Inversión', prev: previous_month.spend, curr: summary.spend, fmt: (v: number) => `$${v.toFixed(2)}`, invert: false },
                                    { label: 'Resultados', prev: previous_month.results, curr: summary.results, fmt: (v: number) => v.toLocaleString('es-MX'), invert: false },
                                    { label: 'CPA', prev: previous_month.cpa, curr: summary.cpa, fmt: (v: number) => `$${v.toFixed(2)}`, invert: true },
                                    { label: 'CTR', prev: previous_month.ctr, curr: summary.ctr, fmt: (v: number) => `${v.toFixed(2)}%`, invert: false },
                                    { label: 'Alcance', prev: previous_month.reach, curr: summary.reach, fmt: (v: number) => v.toLocaleString('es-MX'), invert: false },
                                ].map(row => {
                                    ...
                                    return (
                                        <tr key={row.label} className="hover:bg-gray-50/50 transition">
                                            ...
                                            <td className="px-4 py-3 text-right font-semibold text-[#1E3A5F] font-mono">
