...
export function NotificacionesClient({ initialItems, initialTotal, initialPreferences, initialType, userRole }: Props) {
            ...
            <Tabs defaultValue="historial">
                ...
                <TabsContent value="historial" className="space-y-4">
                    <div className="flex items-center gap-3 flex-wrap">
                        <Select
                            value={typeFilter}
                            onValueChange={(v) => { setTypeFilter(v); load(0, v, unreadOnly) }}
                        >
                        ...
                        <Button
                            ...
                            size="sm"
                            onClick={() => { const next = !unreadOnly; setUnreadOnly(next); load(0, typeFilter, next) }}
                        >
                            Solo no leídas
                        </Button>
                    ...
                    <div className={`rounded-xl border border-border divide-y divide-border overflow-hidden ${isPending ? 'opacity-60' : ''}`}>
                        {items.length === 0 ? (
                            ...
                            items.map((notif) => (
                                ...
                                >
                                    ...
                                    <span className="flex-1 min-w-0">
                                        ...
                                        {notif.message && (
                                            <span className="block text-xs text-muted-foreground mt-0.5">{notif.message}</span>
                                        )}
