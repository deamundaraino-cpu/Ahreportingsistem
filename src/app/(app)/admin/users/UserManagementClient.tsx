...
const ROLE_COLORS: Record<Role, string> = {
    superadmin: 'bg-amber-500/10 text-amber-700 dark:text-amber-400 border-amber-500/20',
    admin:      'bg-purple-500/10 text-purple-700 dark:text-purple-400 border-purple-500/20',
    trafficker: 'bg-blue-500/10 text-blue-700 dark:text-blue-400 border-blue-500/20',
            ...
            <div className="bg-card border border-border rounded-2xl w-full max-w-lg shadow-2xl">
                ...
                <div className="flex items-center justify-between px-6 py-5 border-b border-border">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h3 className="text-foreground font-semibold text-base">Nuevo Usuario</h3>
                            <p className="text-muted-foreground/70 text-xs">Crea una cuenta y asigna accesos</p>
                        </div>
                    </div>
                    <button onClick={onClose} className="text-muted-foreground/70 hover:text-foreground transition-colors p-1">
                        <X className="h-5 w-5" />
                    </button>
                </div>
                    ...
                    <div>
                        ...
                        <div className="relative">
                            ...
                            <input
                                ...
                                className="w-full bg-background border border-input rounded-lg pl-9 pr-3 py-2.5 text-sm text-foreground placeholder:text-muted-foreground/70 focus:outline-none focus:border-blue-500/60 focus:ring-1 focus:ring-blue-500/30 transition"
                            />
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <div className="relative">
                            ...
                            <input
                                ...
                                className="w-full bg-background border border-input rounded-lg pl-9 pr-10 py-2.5 text-sm text-foreground placeholder:text-muted-foreground/70 focus:outline-none focus:border-blue-500/60 focus:ring-1 focus:ring-blue-500/30 transition"
                            />
                            <button
                                type="button"
                                ...
                                <button
                                    ...
                                    className={`px-3 py-1.5 rounded-lg text-xs font-medium border transition-all ${
                                        role === r
                                            ...
                                            : 'bg-muted border-input text-muted-foreground hover:border-ring hover:text-foreground/90'
                                    }`}
                                >
                                    {ROLE_LABELS[r]}
                                </button>
                                        ...
                                        <button
                                            ...
                                            className={`w-full flex items-center justify-between px-3 py-2 rounded-lg border text-sm transition-all ${
                                                selected
                                                    ...
                                                    : 'border-input bg-muted/50 text-muted-foreground hover:border-ring hover:text-foreground/90'
                                            }`}
                                        >
                                            <span>{client.nombre}</span>
                                            {selected && <Check className="h-3.5 w-3.5 text-blue-600 dark:text-blue-400 flex-shrink-0" />}
                                        </button>
                                    )
                                })}
                            </div>
...
export function UserManagementClient({ initialUsers, allClients, currentRole, currentUserId }: Props) {
    ...
    return (
        <div className="space-y-4">
                    ...
                    <Card className="bg-card border-border w-full max-w-md shadow-2xl">
                        <CardContent className="p-6 space-y-4">
                            <div className="flex items-center justify-between">
                                <div>
                                    <h3 className="text-foreground font-semibold text-lg">Asignar Clientes</h3>
                                    <p className="text-muted-foreground text-sm truncate">{assignPanelUser.email}</p>
                                </div>
                                <button onClick={() => setAssignPanelUser(null)} className="text-muted-foreground/70 hover:text-foreground transition-colors">
                                    <X className="h-5 w-5" />
                                </button>
                            </div>
                            ...
                            {assignLoading ? (
                                ...
                                <div className="space-y-2 max-h-72 overflow-y-auto pr-1">
                                    {allClients.length === 0 ? (
                                        <p className="text-muted-foreground/70 text-sm text-center py-4">No hay clientes registrados.</p>
                                    ) : allClients.map(client => {
                                        ...
                                        return (
                                            ...
                                            >
                                                <span>{client.nombre}</span>
                                                {assigned && <Check className="h-4 w-4 text-blue-600 dark:text-blue-400" />}
                                            </button>
                                        )
                                    })}
                                </div>
                            ...
                            <div className="flex gap-3 pt-2">
                                <Button variant="outline" className="flex-1 bg-secondary border-input text-secondary-foreground hover:bg-accent" onClick={() => setAssignPanelUser(null)}>
                                    Cancelar
                                </Button>
            ...
            <div className="flex items-center justify-between">
                <p className="text-sm text-muted-foreground/70">{users.length} usuario{users.length !== 1 ? 's' : ''} registrado{users.length !== 1 ? 's' : ''}</p>
                {canCreateUsers && (
                    <Button
            ...
            <div className="grid gap-3">
                {users.map((user) => {
                    ...
                    return (
                        <Card key={user.id} className="bg-card border-border hover:border-ring transition-all duration-200 shadow-lg">
                            <CardContent className="p-5">
                                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                                    <div className="flex items-center gap-4">
                                        <div className="h-10 w-10 rounded-full bg-muted flex items-center justify-center border border-border shadow-inner flex-shrink-0">
                                        ...
                                        <div>
                                            <div className="flex items-center gap-2 flex-wrap">
                                                ...
                                                <span className={`text-sm ${user.full_name ? 'text-muted-foreground' : 'text-foreground font-medium'}`}>{user.email}</span>
                                                <Badge className={ROLE_COLORS[user.role] || ROLE_COLORS.viewer}>
                                                    {ROLE_LABELS[user.role] || user.role}
                                                </Badge>
                                                {isMe && (
                                                    <Badge className="bg-muted/50 text-muted-foreground border-border/50 text-[10px]">Tú</Badge>
                                                )}
                                            </div>
                                    ...
                                    <div className="flex items-center gap-2 flex-wrap">
                                        {canEdit && user.role === 'trafficker' && (
                                            <Button
                                                ...
                                                className="bg-secondary border-input hover:bg-accent text-secondary-foreground gap-1.5"
                                            >
                                        ...
                                        {canEdit && availableRoles.length > 0 && (
                                            <div className="relative">
                                                ...
                                                {roleMenuOpen === user.id && (
                                                    <div className="absolute right-0 mt-1 w-40 bg-popover border border-border rounded-lg shadow-xl z-20 overflow-hidden">
                                                        {availableRoles.map(r => (
                                                            <button
                                                                key={r}
                                                            ...
                                                            >
                                                                {ROLE_LABELS[r]}
                                                                {user.role === r && <Check className="h-3.5 w-3.5 text-foreground/90" />}
                                                            </button>
                                                        ))}
                                                    </div>
                                                )}
                                        ...
                                        {canEdit && (
                                            <Button
                                                ...
                                                className="bg-red-500/5 border-red-500/20 hover:bg-red-500/10 text-red-600 dark:text-red-400 hover:text-red-700 dark:hover:text-red-300"
                                            >
                ...
                {users.length === 0 && (
                    <Card className="bg-card border-border p-12 text-center">
                        ...
                        <h3 className="text-foreground font-medium">No hay usuarios</h3>
                        <p className="text-muted-foreground/70 mt-1">Haz clic en "Agregar Usuario" para crear el primero.</p>
                    </Card>
                )}
            </div>
        </div>
