                    ...
                    {clientes.map((cliente: any) => (
                        <a key={cliente.id} href={`/admin/settings/${cliente.id}`} className="block group">
                            <Card className="bg-zinc-900 border-zinc-800 group-hover:border-zinc-600 transition h-full flex flex-col">
                                ...
                                <CardFooter className="pt-2 border-t border-zinc-800 mt-2 flex justify-between items-center gap-2">
                                    <span className="text-zinc-500 text-xs uppercase flex-1">
                                        {new Date(cliente.created_at).toLocaleDateString()}
                                    </span>
                                    <Link
                                        href={`/admin/campaign-groups/${cliente.id}`}
                                        className="text-zinc-400 hover:text-indigo-400 transition text-xs flex items-center gap-1 px-2 py-1 rounded hover:bg-zinc-800"
