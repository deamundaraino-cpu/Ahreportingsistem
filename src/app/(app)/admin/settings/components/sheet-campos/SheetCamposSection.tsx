...
import {
    Loader2, Plus, Pencil, Trash2, RefreshCw, AlertCircle, CheckCircle2, Tags,
} from 'lucide-react'
import {
    listSheetCampos, saveSheetCampo, deleteSheetCampo,
    saveSheetVista, deleteSheetVista, listCampoValores,
    recalcularSheetCampos, listSheetColumnas,
} from '../../_actions'
...
const CAMPO_NUEVO: Partial<SheetCampoDef> = {
    nombre: '', rol: 'dimension', formato: 'number', agregacion: 'count',
    origenes: [], valores_map: {}, valores_orden: [], sin_mapear: 'crudo',
    max_valores: 200, activo: true,
}
...
export function SheetCamposSection({ clienteId }: { clienteId: string }) {
    ...
    const cargarValores = useCallback(async (campoId: string) => {
        const res = await listCampoValores(clienteId, campoId)
        if ('error' in res) { setValores([]); setTotalDistintos(0); return }
    ...
    const borrar = async (campo: SheetCampoDef) => {
        ...
        if (!confirm(`¿Borrar el campo "${campo.nombre}"${detalle}? Los informes que lo usen dejarán de mostrarlo.`)) return
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <CardDescription className="mt-1">
                                Une columnas que miden lo mismo en distintas pestañas bajo un nombre propio,
                                agrupa sus valores y úsalas como métrica en informes y dashboards.
                            </CardDescription>
                        </div>
                    </div>
            ...
            <CardContent className="space-y-3">
                ...
                {cargando ? (
                    ...
                    <div className="rounded-lg border border-dashed border-border p-6 text-center space-y-1">
                        ...
                        <p className="text-xs text-muted-foreground/70">
                            Un campo une, por ejemplo, la columna “rango de ingresos” de un formulario con
                            “cuál es tu rango de ingresos” de otro, y agrupa “20 a 100” y “20-100” en un
                            solo valor por el que puedes filtrar.
                        </p>
                    </div>
                ) : (
                                ...
                                >
                                    <span className="min-w-0 flex-1">
                                        <span className="flex items-center gap-2">
                                            ...
                                            {!campo.activo && (
                                                <span className="text-[10px] text-muted-foreground/60 border border-border rounded px-1">inactivo</span>
                                            )}
                                        ...
                                        <span className="block text-[11px] text-muted-foreground/70 truncate">
                                            ...
                                            {suyas.length > 0 && ` · ${suyas.length} ${suyas.length === 1 ? 'vista' : 'vistas'}`}
                                            {' · '}
                                            <span className="font-mono">sf_{campo.clave}</span>
                                            {!campo.recalculado_at && <span className="text-amber-500"> · sin calcular</span>}
                                        </span>
                                    </span>
                ...
                {!cargando && fuentes.length === 0 && (
                    <p className="text-xs text-muted-foreground/60">
                        Aún no hay pestañas sincronizadas. Configura y sincroniza un Google Sheet arriba
