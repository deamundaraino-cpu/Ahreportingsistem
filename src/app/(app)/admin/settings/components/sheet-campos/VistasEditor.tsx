...
import type { SheetCampoDef, SheetCampoVistaDef, CampoValorCrudo, CampoAgg } from '@/lib/sheets/campos'
 ...
 */
export function VistasEditor({ campo, vistas, valores, guardando, onGuardar, onBorrar }: {
    campo: SheetCampoDef
    ...
    return (
        <div className="space-y-2">
            {propias.map(v => (
                <div key={v.id} className="flex items-center gap-2 rounded-md border border-border bg-muted/30 px-2.5 py-1.5">
            ...
            {nueva ? (
                <div className="rounded-md border border-indigo-500/30 bg-indigo-500/5 p-2.5 space-y-2">
                    ...
                    <div className="flex items-center justify-end gap-2">
                        <Button type="button" variant="ghost" size="sm" onClick={() => setNueva(null)} className="h-7 text-xs">
                            Cancelar
                        </Button>
                        <Button
                            type="button" size="sm"
                            disabled={guardando || !nueva.nombre?.trim() || (nueva.valores ?? []).length === 0}
                            onClick={() => { onGuardar({ ...nueva, campo_id: campo.id }); setNueva(null) }}
                ...
                <Button
                    ...
                    onClick={() => setNueva({ agregacion: 'count', operador: 'in', valores: [], formato: 'number' })}
                    className="h-7 text-xs gap-1"
                >
