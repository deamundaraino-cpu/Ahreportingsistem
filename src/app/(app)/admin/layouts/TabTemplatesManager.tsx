...
export function TabTemplatesManager({ templates, isAdmin }: { templates: TabTemplate[]; isAdmin: boolean }) {
    ...
    async function handleDelete(t: TabTemplate) {
        if (!confirm(`¿Eliminar la plantilla "${t.nombre}"? Las pestañas ya creadas con ella no se ven afectadas.`)) return
        setBusyId(t.id)
    ...
    return (
        <Card className="bg-card border-border">
            <CardHeader>
                ...
                <CardDescription>
                    Visualizaciones reutilizables (tarjetas, gráficos, ranking y métricas) que se aplican al crear una pestaña nueva en cualquier cliente. Para crear una, abre una pestaña y usa &quot;Guardar como plantilla&quot;.
                </CardDescription>
            </CardHeader>
                        ...
                        {editingId === t.id ? (
                            <div className="space-y-2">
                                ...
                                <div className="flex items-center gap-2">
                                    ...
                                    >
                                        {busyId === t.id ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <Save className="w-3.5 h-3.5" />}
                                        Guardar
                                    </Button>
                                    <Button
                                        size="sm"
