...
const STATUS_MAP: Record<string, { label: string; color: string; icon: any }> = {
    abierto:     { label: 'Planeado',      color: 'bg-blue-500/10 text-blue-400 border-blue-500/20',     icon: AlertCircle },
    en_progreso: { label: 'En desarrollo', color: 'bg-amber-500/10 text-amber-400 border-amber-500/20',   icon: Clock },
    completado:  { label: 'Lanzado',       color: 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20', icon: CheckCircle2 },
    cancelado:   { label: 'Descartado',    color: 'bg-zinc-700/30 text-zinc-500 border-zinc-700/30',      icon: MessageSquare },
}
...
const TYPE_MAP: Record<string, { label: string; color: string; icon: any }> = {
    ...
    tarea:   { label: 'Tarea',   color: 'bg-zinc-600 text-white border-zinc-400 shadow-sm shadow-zinc-900/40',       icon: ListTodo },
}
        ...
        <div className="space-y-6">
            ...
            {showForm && (
                <div className="rounded-xl border border-indigo-500/30 border-t-4 border-t-indigo-500 bg-zinc-900 p-6 animate-in slide-in-from-top-3 duration-300">
                    <h2 className="text-base font-semibold text-white mb-4">Registrar Nuevo Ítem del Roadmap</h2>
                    <form onSubmit={handleCreate} className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                ...
                                {Object.entries(TYPE_MAP).map(([k, tp]) => {
                                    ...
                                    return (
                                        <button key={k} type="button"
                                            ...
                                            className={`flex items-center gap-1.5 py-2 px-3 rounded-lg border text-xs font-medium transition ${form.tipo === k ? tp.color : 'bg-zinc-950 border-zinc-800 text-zinc-500 hover:border-zinc-700'}`}
                                        >
                        ...
                        <div className="space-y-1.5">
                            <label className="text-xs font-semibold text-zinc-400 uppercase tracking-wider">Cliente <span className="text-zinc-500 normal-case font-normal">(opcional)</span></label>
                            <select
            ...
            <div className="flex flex-col sm:flex-row gap-3">
                ...
                <Select value={tipoFilter} onValueChange={setTipoFilter}>
                    <SelectTrigger className="w-full sm:w-40 h-9 text-sm bg-zinc-900 border-zinc-800 text-zinc-300">
                        <SelectValue />
                    </SelectTrigger>
            ...
            <div className="overflow-hidden rounded-xl border border-white/[0.06] bg-zinc-900">
                {filtered.length === 0 ? (
                    <div className="flex flex-col items-center justify-center py-16 text-zinc-500">
                        ...
                        <p className="font-medium text-zinc-400">Sin ítems que mostrar</p>
                            ...
                            return (
                                <div key={t.id} className="grid grid-cols-1 gap-3 px-4 py-3.5 hover:bg-zinc-800/30 transition-colors sm:grid-cols-[1fr_2fr_1fr] sm:items-start">
                                    ...
                                    <div className="flex flex-col gap-0.5">
                                        ...
                                        <span className="text-sm font-semibold text-blue-400">{t.cliente?.nombre ? `🏢 ${t.cliente.nombre}` : '🌐 Interno'}</span>
