...
import { Plus, Search, AlertCircle, Clock, CheckCircle2, MessageSquare, Edit2, User, Calendar, Bug, Sparkles, Wrench, ListTodo } from 'lucide-react'
...
const STATUS_MAP: Record<string, { label: string; color: string; icon: any }> = {
    abierto:     { label: 'Planeado',      color: 'bg-blue-500/10 text-blue-700 dark:text-blue-400 border-blue-500/20',     icon: AlertCircle },
    en_progreso: { label: 'En desarrollo', color: 'bg-amber-500/10 text-amber-700 dark:text-amber-400 border-amber-500/20',   icon: Clock },
    completado:  { label: 'Lanzado',       color: 'bg-emerald-500/10 text-emerald-700 dark:text-emerald-400 border-emerald-500/20', icon: CheckCircle2 },
    cancelado:   { label: 'Descartado',    color: 'bg-muted/50 text-muted-foreground border-border',      icon: MessageSquare },
}

const PRIORITY_MAP: Record<number, { label: string; color: string; bg: string }> = {
    1: { label: 'Alta',  color: 'text-red-600 dark:text-red-400',     bg: 'bg-red-500/10 border-red-500/30' },
    2: { label: 'Media', color: 'text-amber-600 dark:text-amber-400',   bg: 'bg-amber-500/10 border-amber-500/30' },
    3: { label: 'Baja',  color: 'text-emerald-600 dark:text-emerald-400', bg: 'bg-emerald-500/10 border-emerald-500/30' },
}

const TYPE_MAP: Record<string, { label: string; color: string; icon: any }> = {
    bug:     { label: 'Bug',     color: 'bg-red-600 text-white border-red-400 shadow-sm shadow-red-900/40',          icon: Bug },
    feature: { label: 'Feature', color: 'bg-violet-600 text-white border-violet-400 shadow-sm shadow-violet-900/40', icon: Sparkles },
    mejora:  { label: 'Mejora',  color: 'bg-sky-600 text-white border-sky-400 shadow-sm shadow-sky-900/40',          icon: Wrench },
    tarea:   { label: 'Tarea',   color: 'bg-secondary text-secondary-foreground border-border shadow-sm',       icon: ListTodo },
}

interface Cliente { id: string; nombre: string }
interface Ticket {
    id: string; id_ticket_display: string; cliente_id: string | null
    ...
    prioridad: number; estado: string; tipo: string; cliente?: { nombre: string } | null
}

const EMPTY_FORM = { cliente_id: '', tipo: 'tarea', nombre_solicitante: '', requerimiento: '', observaciones: '', prioridad: 2, fecha_entrega: '' }
...
export function SoporteClient({ tickets: initial, clientes, userRole }: {
    ...
    const [editForm, setEditForm] = useState({ nombre_solicitante: '', requerimiento: '', observaciones: '', prioridad: 2, tipo: 'tarea' })
    ...
    async function handleEditSave() {
        ...
        setEditSaving(true)
        const res = await updateSoporteTicket(editingTicket.id, editingTicket.cliente_id, editForm as any)
        setEditSaving(false)
        if (res.error) { alert('Error: ' + res.error); return }
        ...
        <div className="space-y-6">
            ...
            {showForm && (
                <div className="rounded-xl border border-indigo-500/30 border-t-4 border-t-indigo-500 bg-card p-6 animate-in slide-in-from-top-3 duration-300">
                    <h2 className="text-base font-semibold text-foreground mb-4">Registrar Nuevo Ítem del Roadmap</h2>
                    <form onSubmit={handleCreate} className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div className="space-y-1.5">
                            <label className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">Cliente <span className="text-muted-foreground/70 normal-case font-normal">(opcional)</span></label>
                            <select
                        ...
                        <div className="md:col-span-2 flex justify-end gap-3 pt-2">
                            <Button type="button" variant="ghost" onClick={() => setShowForm(false)} className="text-muted-foreground hover:text-foreground">Cancelar</Button>
            ...
            <div className="flex flex-col sm:flex-row gap-3">
                ...
                <Select value={tipoFilter} onValueChange={setTipoFilter}>
                    ...
                    <SelectContent className="bg-popover border-border">
                        ...
                        {Object.entries(TYPE_MAP).map(([k, tp]) => (
                            <SelectItem key={k} value={k} className="text-sm">{tp.label}</SelectItem>
                        ))}
                    </SelectContent>
                </Select>
                <Select value={statusFilter} onValueChange={setStatusFilter}>
                    ...
                    <SelectContent className="bg-popover border-border">
                        ...
                        <SelectItem value="abierto" className="text-sm">Planeados</SelectItem>
                        <SelectItem value="en_progreso" className="text-sm">En desarrollo</SelectItem>
                        <SelectItem value="completado" className="text-sm">Lanzados</SelectItem>
                        <SelectItem value="cancelado" className="text-sm">Descartados</SelectItem>
                    </SelectContent>
                </Select>
            </div>
                ...
                {filtered.length === 0 ? (
                    ...
                    <div className="divide-y divide-border">
                        {filtered.map(t => {
                            ...
                            return (
                                <div key={t.id} className="grid grid-cols-1 gap-3 px-4 py-3.5 hover:bg-accent/50 transition-colors sm:grid-cols-[1fr_2fr_1fr] sm:items-start">
                                    {/* Left: IDs */}
                                    <div className="flex flex-col gap-0.5">
                                        <span className="font-mono text-sm font-bold text-indigo-600 dark:text-indigo-400">{t.id_ticket_display}</span>
                                        <span className="text-sm font-semibold text-blue-600 dark:text-blue-400">{t.cliente?.nombre ? `🏢 ${t.cliente.nombre}` : '🌐 Interno'}</span>
                                    ...
                                    <div className="flex-1 min-w-0">
                                        <p className="text-sm font-medium text-foreground leading-snug">{t.requerimiento}</p>
                                        {t.observaciones && (
                                    ...
                                    <div className="flex flex-wrap items-center gap-2 sm:flex-col sm:items-end sm:gap-1.5 sm:justify-self-end sm:w-full">
                                        {/* Tipo */}
                                        <span className={`inline-flex items-center gap-1 text-xs font-bold uppercase px-2 py-0.5 rounded border ${TYPE_MAP[t.tipo]?.color ?? TYPE_MAP.tarea.color}`}>
                                            {React.createElement((TYPE_MAP[t.tipo] ?? TYPE_MAP.tarea).icon, { className: 'w-3 h-3' })}
                                        ...
                                        {/* Edit */}
                                        {isTeam && (
                                            <button onClick={() => { setEditForm({ nombre_solicitante: t.nombre_solicitante, requerimiento: t.requerimiento, observaciones: t.observaciones || '', prioridad: t.prioridad, tipo: t.tipo || 'tarea' }); setEditingTicket(t) }}
                                                className="text-muted-foreground/70 hover:text-indigo-600 dark:hover:text-indigo-400 transition mt-0.5" title="Editar">
            ...
            <Dialog open={!!editingTicket} onOpenChange={open => { if (!open) setEditingTicket(null) }}>
                <DialogContent className="bg-background border-border text-foreground max-w-lg">
                    ...
                    <div className="space-y-4 py-2">
                        <div className="space-y-1.5">
                            <label className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">Solicitante</label>
                            <Input value={editForm.nombre_solicitante} onChange={e => setEditForm({ ...editForm, nombre_solicitante: e.target.value })} className="bg-card border-input text-foreground" />
                        </div>
                        <div className="space-y-1.5">
                            <label className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">Requerimiento</label>
                            <textarea rows={3} value={editForm.requerimiento} onChange={e => setEditForm({ ...editForm, requerimiento: e.target.value })} className="w-full bg-card border border-input rounded-md p-3 text-sm text-foreground outline-none focus:border-indigo-500" />
                        </div>
                        <div className="space-y-1.5">
                            <label className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">Observaciones</label>
                            <textarea rows={2} value={editForm.observaciones} onChange={e => setEditForm({ ...editForm, observaciones: e.target.value })} className="w-full bg-card border border-input rounded-md p-3 text-sm text-foreground outline-none focus:border-indigo-500" />
                        </div>
                        <div className="space-y-1.5">
                            <label className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">Tipo</label>
                            <div className="flex flex-wrap gap-2">
                                {Object.entries(TYPE_MAP).map(([k, tp]) => {
                                    const Icon = tp.icon
                                    return (
                                        <button key={k} type="button" onClick={() => setEditForm({ ...editForm, tipo: k })}
                    ...
                    <DialogFooter>
                        <Button variant="ghost" onClick={() => setEditingTicket(null)} className="text-muted-foreground hover:text-foreground">Cancelar</Button>
