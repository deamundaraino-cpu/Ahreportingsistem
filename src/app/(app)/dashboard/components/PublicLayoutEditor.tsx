...
import {
    Globe, X, GripVertical, Save, Loader2, Check, Plus, Database, BarChart3
} from 'lucide-react'
...
const EXECUTIVE_PRESETS: { label: string; formula: string; prefix?: string; suffix?: string; color?: CardColor }[] = [
    ...
    { label: 'Sesiones Web', formula: 'ga_sessions', color: 'blue' },
...
export function PublicLayoutEditor({
    ...
    async function handleSave() {
        setLoading(true)
        const res = await savePublicLayout(clienteId, { tarjetas, graficos })
        setLoading(false)
