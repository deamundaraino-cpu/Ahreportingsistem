        ...
        try {
            do {
                ...
                if (json?.code !== 0 || !json?.data) {
                    log(`[reconcile] TikTok ${advertiser_id} error: ${json?.message ?? JSON.stringify(json)}`)
                    ok = false
                    break
                }
                for (const row of (json.data.list ?? [])) {
                    ...
                    if (!day) continue
                    porFecha.set(day, (porFecha.get(day) ?? 0) + (parseFloat(row?.metrics?.spend || '0') || 0))
                }
...
async function run(request: Request) {
    ...
    const plataformasPedidas: Plataforma[] = platformsParam
        ? (platformsParam.split(',').map(p => p.trim().toLowerCase()).filter(p => p === 'meta' || p === 'tiktok') as Plataforma[])
        : ['meta', 'tiktok']
    ...
    const log = (m: string) => { console.log(m); debugLogs.push(m) }
    ...
    for (const cliente of (clientes ?? [])) {
        ...
        for (const plataforma of plataformas) {
            const { dias: accountDays, ok } = plataforma === 'meta'
            ...
            if (heal) {
                for (const rango of reparables) {
                    ...
                    if (job) { encoladosCliente++; jobsEncolados++ }
                }
            }
