...
export async function POST(request: Request) {
    ...
    try {
        ...
        if (!body.tipo) {
            return NextResponse.json({ error: 'Falta `tipo` (o usa ?plan=diario|intradia|cierre_mes)' }, { status: 400 })
        }
        ...
        return NextResponse.json({ ok: true, encolados: job ? 1 : 0, duplicado: !job, cola: await queueStats(db) })
    } catch (e: any) {
