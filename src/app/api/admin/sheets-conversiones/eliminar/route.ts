...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    return NextResponse.json({ error: err.message || 'Error al consultar el sheet' }, { status: 500 })
  }
}
  ...
  try {
    ...
    for (const tabla of TABLAS) {
      for (let i = 0; i < PAGINAS_POR_TANDA; i++) {
        const { data, error } = await db.from(tabla).select('id')
          .eq('cliente_id', clientId).eq('sheet_id', sheetId).limit(PAGINA)
        ...
        if (delErr) return NextResponse.json({ error: `${tabla}: ${delErr.message}` }, { status: 500 })
        borradas += ids.length
      }
    }

    const restantes = (await Promise.all(TABLAS.map(t => contar(db, t, clientId, sheetId))))
      .reduce((a, b) => a + b, 0)
    ...
    const { data: cliente } = await db.from('clientes')
      .select('config_api').eq('id', clientId).single()
    ...
    await db.from('conversiones_offline_sync_log')
      .delete().eq('cliente_id', clientId).eq('sheet_id', sheetId)
    ...
    return NextResponse.json({ error: err.message || 'Error al eliminar el sheet' }, { status: 500 })
  }
}
