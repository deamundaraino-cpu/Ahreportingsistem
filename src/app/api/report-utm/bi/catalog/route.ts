...
import { BASE_REGISTRY, isAdditive, isPivotable, fieldCrossesDimension } from '@/lib/report-utm/bi/registry'
import { CANONICAL_TO_LEGACY_MEASURE, LEGACY_DIMENSION_IDS } from '@/lib/report-utm/bi/legacy-tokens'
...
export async function GET(req: NextRequest) {
    ...
    try {
        ...
        const hasPublicLink = clienteId
            ? (await resolvePublicClienteId(clienteId)) !== null
            : true
        ...
        const fuentes: FuenteSalida[] = reg.sources.map(src => {
            ...
            const fields: CampoSalida[] = src.fields.map(f => {
                ...
                if (f.kind === 'measure') {
                    ...
                    return {
                        ...
                        conflictsWith: m.conflictsWith?.map(
                            id => CANONICAL_TO_LEGACY_MEASURE[id] ?? id),
            ...
            const conCruce = dimension
                ? fields.map(f => ({
                    ...f,
                    crossesDimension: f.kind === 'dimension'
                        ? true
                        : fieldCrossesDimension(reg, f.canonicalId, dimension),
                }))
                : fields
