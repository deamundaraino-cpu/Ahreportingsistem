...
export async function GET(req: NextRequest) {
    ...
    try {
        ...
        const { data: cliente } = await admin
            .from('clientes').select('config_api').eq('id', publicId).maybeSingle()
        ...
        interface ColDef { col_name?: string; type?: string; label?: string; include?: boolean }
        interface TabCfg { sheet_name?: string; custom_columns?: Record<string, ColDef> }
        interface SheetCfg { name?: string; custom_columns?: Record<string, ColDef>; tabs?: TabCfg[] }
        ...
        const sheets: SheetCfg[] = Array.isArray(raw)
            ? raw as SheetCfg[]
            : raw && typeof raw === 'object' ? [raw as SheetCfg] : []
