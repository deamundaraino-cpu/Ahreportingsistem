...
import { parseCartPandaPayload } from '@/lib/report-utm/cartpanda-parser'
...
export async function POST(
    ...
    // Buscar integración CartPanda del cliente
    ...
    // Validar firma HMAC (CartPanda usa x-cartpanda-hmac-sha256)
    const { valid } = verifyWebhookSignature({
        rawBody,
        secret: integration.webhook_secret,
        signatureHeader: req.headers.get('x-cartpanda-hmac-sha256'),
        hottokHeader: null,
        hottokQuery: null,
        payload,
    })
    ...
    if (!valid) {
        console.warn('[cartpanda webhook] invalid signature', { clienteId })
        return NextResponse.json({ error: 'Invalid signature' }, { status: 401 })
    }

    // Parsear payload
    const parsed = parseCartPandaPayload(payload)
    if ('error' in parsed) {
        await trackingDb
            .from('integrations')
    ...
    // Insertar venta (dedup por cliente_id + platform + platform_sale_id)
    ...
    // Emitir webhooks salientes (fire-and-forget)
    ...
    if (outboundType) {
        ...
        }).catch((err) => console.error('[cartpanda webhook] outbound emit failed', err))
    ...
    // Meta CAPI dispatch para ventas aprobadas
        ...
        after(async () => {
            try {
                ...
                if (
                    ...
                    if (!result.ok) {
                        console.error('[cartpanda webhook] Meta CAPI failed', result.error)
                    }
                }
            } catch (err) {
                console.error('[cartpanda webhook] Meta CAPI dispatch error', err)
            }
        })
    }
        ...
        after(async () => {
            try {
                ...
                if (
                    ...
                    if (!result.ok) {
                        console.error('[cartpanda webhook] Google Ads conversion failed', result.error)
                    }
                }
            } catch (err) {
                console.error('[cartpanda webhook] Google Ads dispatch error', err)
            }
        })
    }
