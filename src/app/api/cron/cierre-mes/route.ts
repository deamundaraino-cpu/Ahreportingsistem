...
export async function POST(request: Request) {
  ...
  if (!start || !end) {
    return NextResponse.json(
      { error: 'Faltan `start` y `end` (primer y último día del mes)' },
      { status: 400 }
    );
  }
  ...
  if (end >= hoy) {
    return NextResponse.json(
      { error: `El período ${periodo} aún no ha terminado (fin ${end} >= hoy ${hoy})` },
      { status: 400 }
    );
  }
  ...
  for (const id of clienteIds) {
    ...
    if (snapError) {
      resultados.push({
        cliente_id: id,
        filas: filas.length,
        estado: `error snapshot: ${snapError.message}`,
      });
      continue;
    }
