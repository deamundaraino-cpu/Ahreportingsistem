...
export default async function ReportUtmOverview() {
    ...
    const [{ data: clientes, error }, { count: totalEvents }, { data: revenue7d }, { data: linksAgg }] =
        await Promise.all([
            supabase
                .from('clientes')
    ...
    return (
        <div className="space-y-8">
            ...
            {error && (
                <div className="rounded-lg border border-amber-200 dark:border-amber-500/30 bg-amber-50 dark:bg-amber-500/10 p-4">
                    ...
                    <ol className="mt-2 ml-4 list-decimal text-xs text-amber-700 dark:text-amber-400 space-y-1">
                        <li>Correr la migration <code>012_report_utm_schema.sql</code> en Supabase.</li>
                        <li>Agregar <code>report_utm</code> en Supabase Studio → Settings → API → Exposed schemas.</li>
                    </ol>
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                ...
                <StatCard label="Ventas trackeadas" value={eventsCount.toLocaleString()} icon={ShoppingBag} accent="violet" />
                <StatCard label="Revenue (7d)" value={`$${revenueLast7d.toFixed(0)}`} icon={Activity} accent="emerald" hint="aprobado" />
