...
interface Resumen { criticos: number; avisos: number; ok: number }

const ESTILO: Record<Gravedad, { chip: string; borde: string; Icono: typeof AlertTriangle; texto: string }> = {
    critico: {
            ...
            <header className="flex items-start justify-between gap-4">
                <div>
                    ...
                    <p className="text-sm text-muted-foreground mt-1 max-w-2xl">
                        Un informe con una fuente muerta no se ve roto: se ve vacío. Aquí sale lo
                        que no genera síntomas — integraciones caídas, fuentes paradas y cruces
                        degradados.
                    </p>
                </div>
                <button
            ...
            <div className="space-y-3">
                {clientes.map(c => {
                    ...
                    return (
                        <section
                            key={c.clienteId}
                            className={`rounded-lg border border-l-4 ${est.borde} p-4`}
                        >
                            <div className="flex items-center gap-3">
                                <est.Icono className="h-4 w-4 shrink-0" aria-hidden />
                                <h2 className="font-medium flex-1">{c.nombre}</h2>
                                <span className={`text-xs px-2 py-0.5 rounded ${est.chip}`}>
                                    {est.texto}
                                </span>
                            </div>
                                    ...
                                    {c.hallazgos.map((h, i) => (
                                        <li key={i} className="text-sm">
                                            <div className="flex flex-wrap items-baseline gap-x-2">
                                                <span className={`text-[11px] px-1.5 py-0.5 rounded ${ESTILO[h.gravedad].chip}`}>
                                                    {h.ambito}
                                                </span>
                                                <span>{h.titulo}</span>
                                            </div>
