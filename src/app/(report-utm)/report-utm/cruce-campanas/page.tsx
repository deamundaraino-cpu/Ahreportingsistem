...
import { Link2, Loader2, Plus, Trash2, AlertTriangle, CheckCircle2, RefreshCw, Sparkles, Wand2, Bug, Search, ListChecks } from 'lucide-react'
...
export default function CruceCampanasPage() {
        ...
        fetch(`/api/report-utm/bi/campaign-map?${p}`)
            ...
            .then(j => {
                ...
                setPicks(Object.fromEntries(
                    rows.filter(r => r.suggestion)
                        .map(r => [r.value, campKeyOf(r.suggestion!.platform, r.suggestion!.campaign_id, r.suggestion!.campaign_name)])
                ))
            })
            .catch(() => {})
    ...
    async function postOverride(value: string, camp: { platform: string; campaign_id: string | null; name: string }) {
    ...
    async function confirmAllSuggestions() {
        const targets = breakdown.filter(r => !r.matched_campaign && !r.invalid_reason && r.suggestion && r.suggestion.confidence >= BULK_CONFIDENCE)
        if (targets.length === 0) return
        setBulkBusy(true)
        try {
            for (const r of targets) {
                await postOverride(r.value, { platform: r.suggestion!.platform, campaign_id: r.suggestion!.campaign_id, name: r.suggestion!.campaign_name })
            }
        } finally {
            setBulkBusy(false)
            load()
    ...
    const uncrossedCount = useMemo(() => breakdown.filter(r => !r.matched_campaign).length, [breakdown])
    const bulkCount = useMemo(() => breakdown.filter(r => !r.matched_campaign && !r.invalid_reason && r.suggestion && r.suggestion.confidence >= BULK_CONFIDENCE).length, [breakdown])
    ...
    const filtered = useMemo(() => {
        ...
        return breakdown.filter(r => {
            ...
            if (q && !r.value.toLowerCase().includes(q) && !(r.matched_campaign ?? '').toLowerCase().includes(q)) return false
            return true
        })
        ...
        <div className="space-y-6">
            <div>
                ...
                <p className="mt-1 text-sm text-muted-foreground">
                    Cada <code className="font-mono px-1 py-0.5 rounded bg-muted">utm_campaign</code> de tus leads y a qué campaña real de Meta/TikTok cruzó. Lo que no cruce automáticamente, mapéalo aquí.
                </p>
            </div>
            ...
            {loading ? (
                ...
                <>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-4 gap-4">
                        ...
                        <Stat label="UTMs sin cruzar" value={String(uncrossedCount)} icon={AlertTriangle} tone={uncrossedCount > 0 ? 'amber' : 'emerald'} />
                    ...
                    <div className="rounded-2xl border border-border bg-card overflow-hidden">
                        <div className="px-6 py-4 border-b border-border flex flex-wrap items-center gap-3">
                            ...
                            <div className="ml-auto flex items-center gap-3">
                                <label className="flex items-center gap-1.5 text-[11px] text-muted-foreground cursor-pointer select-none">
                                    <input type="checkbox" checked={onlyUncrossed} onChange={e => setOnlyUncrossed(e.target.checked)} className="accent-emerald-500" />
                                    Solo sin cruzar
                                </label>
                        ...
                        {breakdown.length === 0 ? (
                            <div className="px-6 py-10 text-center text-sm text-muted-foreground">No hay leads en el rango seleccionado.</div>
                        ) : (
                            <div className="overflow-x-auto">
                                        ...
                                        {filtered.map(row => {
                                            ...
                                            return (
                                                <tr key={row.value} className="hover:bg-accent align-top">
                                                    <td className="px-6 py-3 text-xs font-mono text-foreground max-w-[320px] truncate" title={row.value}>{row.value}</td>
                                                    <td className="px-6 py-3 text-right text-xs font-mono tabular-nums text-foreground">{row.count}</td>
                                                    <td className="px-6 py-3">
                                                        {row.matched_campaign ? (
                                                            <div className="flex items-center gap-2">
                                                                <CheckCircle2 className="h-3.5 w-3.5 text-emerald-500 shrink-0" />
                                                                <span className="text-xs text-foreground max-w-[260px] truncate" title={row.matched_campaign}>
                                                                    {row.distinct_campaigns > 1 ? `Varias campañas (${row.distinct_campaigns})` : row.matched_campaign}
                                                                </span>
                                                            ...
                                                            <span className="inline-flex items-center gap-1.5 text-xs text-amber-600 dark:text-amber-400">
                                                                <Bug className="h-3.5 w-3.5" /> {row.invalid_reason === 'macro_no_renderizado' ? 'Macro sin renderizar' : 'Sin UTM'}
                                                            </span>
                                                        ) : (
                                                            <span className="inline-flex items-center gap-1.5 text-xs text-amber-600 dark:text-amber-400">
                                                        ...
                                                        {mappable ? (
                                                            <div className="flex items-center gap-2">
                                                                <select
                                                                    ...
                                                                    onChange={e => setPicks(p => ({ ...p, [row.value]: e.target.value }))}
                                ...
                                {filtered.length === 0 && (
                                    <div className="px-6 py-8 text-center text-xs text-muted-foreground">Sin resultados con el filtro actual.</div>
                                )}
                            </div>
                        )}
                    </div>
