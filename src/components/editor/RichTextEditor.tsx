...
export function RichTextEditor({ content, onChange, placeholder, clientId }: RichTextEditorProps) {
    ...
    const editor = useEditor({
        ...
        content,
        editorProps: {
            attributes: { class: 'ProseMirror bitacora-content text-sm text-foreground px-3 py-2 focus:outline-none' },
        },
        onUpdate: ({ editor }) => {
        ...
        <div className="border border-input rounded-md overflow-hidden bg-background">
            ...
            <div className="flex items-center gap-0.5 px-2 py-1.5 border-b border-input bg-muted/30 flex-wrap">
                ...
                <ToolbarButton
                    ...
                    onClick={() => setLinkInput(editor.isActive('link') ? (editor.getAttributes('link').href ?? '') : '')}
                    title="Enlace"
                >
