            ...
            <div className="flex items-start justify-between gap-3">
                <div className="min-w-0">
                    ...
                    <div className="mt-0.5 space-y-0.5">
                        {lead.lead_email && <p className="text-[11px] text-muted-foreground font-mono break-all">{lead.lead_email}</p>}
                        {lead.lead_phone && <p className="text-[11px] text-muted-foreground">{lead.lead_phone}</p>}
                        {!hasContact && <p className="text-[11px] text-muted-foreground italic">Sin datos de contacto</p>}
                    </div>
                </div>
...
function UtmChip({ label, value, accent, mono }: { label: string; value: string | null; accent?: boolean; mono?: boolean }) {
    const v = dec(value)
    if (!v) return null
    return (
        <span
        ...
        >
            ...
            <span className={`truncate ${mono ? 'font-mono' : ''} ${accent ? 'text-emerald-600 dark:text-emerald-400 font-medium' : 'text-foreground'}`}>
                {v}
            </span>
        </span>
    )
