...
const ACCENT_OPTIONS = ['#3b82f6', '#10b981', '#8b5cf6', '#f59e0b', '#ef4444', '#06b6d4', '#f97316', '#ec4899']
...
export function BiClienteBrandingCard({
    ...
    async function handleUpload(e: React.ChangeEvent<HTMLInputElement>) {
        ...
        if (!file) return
        if (file.size > 2 * 1024 * 1024) { setError('El logo excede el máximo de 2MB.'); return }
        ...
        try {
            ...
            const { data: { publicUrl } } = supabase.storage.from('bitacoras-images').getPublicUrl(filePath)
        ...
        <div className="rounded-2xl border border-border bg-card p-5">
            <div className="flex items-start gap-3 mb-4">
                ...
                <div>
                    ...
                    <p className="text-xs text-muted-foreground">Logo y color que verá el cliente en los informes compartidos.</p>
                </div>
            </div>

            {/* Logo */}
            <div className="mb-4">
                <label className="block text-xs font-medium text-muted-foreground mb-1.5">Logo del cliente</label>
                <div className="flex items-center gap-3">
                    ...
                    <div className="flex flex-col gap-1.5">
                        ...
                        {logoUrl && (
                            <button onClick={() => setLogoUrl('')} className="inline-flex items-center gap-1 text-[11px] text-muted-foreground hover:text-red-500 transition-colors w-fit">
