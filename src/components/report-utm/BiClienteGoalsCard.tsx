...
export function BiClienteGoalsCard({ clienteId, initialGoals = {} }: Props) {
    const [goals, setGoals] = useState<Record<string, string>>(() =>
        Object.fromEntries(FIELDS.map(f => [f.key, initialGoals[f.key] != null ? String(initialGoals[f.key]) : '']))
    )
    ...
    async function handleSave() {
        ...
        try {
            ...
            if (!res.ok) { setError(json?.error ?? 'No se pudieron guardar las metas.'); return }
    ...
    return (
        <div className="rounded-2xl border border-border bg-card p-5">
            ...
            <p className="text-xs text-muted-foreground mb-4">
                Los indicadores de los informes se pintan en verde, amarillo o rojo según estas metas.
                Deja un campo vacío para no evaluarlo.
            </p>
                    ...
                    <div key={f.key}>
                        ...
                        <div className="flex items-center gap-1">
                            ...
                            <input
                                ...
                                step="any"
                                value={goals[f.key] ?? ''}
                                onChange={e => { setGoals(prev => ({ ...prev, [f.key]: e.target.value })); setSaved(false) }}
                                placeholder="—"
