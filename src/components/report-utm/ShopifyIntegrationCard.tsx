...
export function ShopifyIntegrationCard({
  ...
  const [shopDomain, setShopDomain] = useState(String(integration?.config?.shop_domain ?? ''));
  ...
  const onActivate = () => {
    if (!shopDomain.trim()) {
      setError('Ingresá el Shop Domain primero');
      return;
    }
    setError(null);
  ...
  const onRotate = () => {
    if (!confirm('¿Rotar el webhook secret? El antiguo dejará de funcionar inmediatamente.'))
      return;
    setError(null);
    startTransition(async () => {
        ...
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1">
            ...
            <p className="mt-1 text-xs text-muted-foreground">
              Recibí eventos <code className="font-mono text-xs">orders/paid</code> de tu tienda
              Shopify con atribución multi-touch.
            </p>
  ...
  return (
    <div className="rounded-2xl border border-border bg-card p-6 space-y-5">
      ...
      {revealedSecret ? (
        <div className="rounded-lg border border-emerald-200 dark:border-emerald-500/30 bg-emerald-50/40 dark:bg-emerald-500/5 p-4">
          ...
          <p className="mt-2 text-xs text-emerald-700 dark:text-emerald-400">
            Solo se muestra una vez. En Shopify Admin → Settings → Notifications → Webhooks, pegá
            este secret y la URL de arriba.
          </p>
        </div>
      ) : (
      ...
      <div className="text-xs text-muted-foreground space-y-1 pt-1">
        <p>
          Topic recomendado:{' '}
          <code className="font-mono px-1 py-0.5 rounded bg-muted">orders/paid</code>. La firma de
          Shopify es HMAC-SHA256 en Base64 (diferente a Hotmart/CartPanda).
        </p>
        <p>
          UTMs leídos de{' '}
          <code className="font-mono px-1 py-0.5 rounded bg-muted">note_attributes</code> y del
          campo <code className="font-mono px-1 py-0.5 rounded bg-muted">landing_site</code> como
          fallback.
        </p>
      </div>
    </div>
