...
export function LeadCamposCard({ clienteId }: { clienteId: string }) {
  ...
  async function guardar(borrador: Partial<LeadCampoDef>) {
    ...
    try {
      ...
      if (!res.ok) {
        setErrorDialogo(json?.error ?? 'No se pudo guardar.');
        return;
      }
      setEditando(null);
      await cargar();
  ...
  async function guardarSegmento(borrador: Partial<LeadSegmentoDef>) {
    ...
    try {
      ...
      const json = await res.json().catch(() => ({}));
      if (!res.ok) {
        setError(json?.error ?? 'No se pudo guardar el segmento.');
        return;
      }
      await cargar();
    } finally {
  ...
  async function borrarSegmento(id: string) {
    if (
      !confirm('¿Borrar el segmento? Las tarjetas y fórmulas que lo usen dejarán de tener datos.')
    )
      return;
    ...
    <div className="rounded-2xl border border-border bg-card p-5">
      ...
      <p className="text-xs text-muted-foreground mb-4">
        ...
        {detectadas.length > 0 && (
          <>
            {' '}
            Se detectaron <strong>{detectadas.length}</strong> preguntas en {leadsEscaneados} leads
            del último año.
          </>
        )}
      </p>
                ...
                <div className="flex items-start gap-3">
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="text-[10px] text-muted-foreground/70 break-words mt-0.5">
                      {campo.claves_origen.length} pregunta
                      {campo.claves_origen.length === 1 ? '' : 's'}
                      {valores.length > 0 &&
                        ` · ${valores.length} valores: ${valores.slice(0, 4).join(' · ')}`}
                      {valores.length > 4 && ` …`}
                    </p>
