...
export function BiFieldPicker({
    kind, value, onChange, clienteId, dimension,
    alreadyChosen = [], onlyFunnelStages, onlyPivotable,
}: Props) {
    ...
    const solapaCon = (f: CatalogField) =>
        (f.conflictsWith ?? []).filter(id => yaElegidos.has(id))
...
function FieldRow({
    field, sourceLabel, disabled, selected, overlaps, onPick,
}: {
    field: CatalogField
    sourceLabel?: string
    disabled?: boolean
