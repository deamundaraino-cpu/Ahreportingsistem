...
import type {
    EstadoVenta, HotmartCompra, HotmartComision, HotmartComprador, HotmartProducto,
    HotmartWebhook, ItemComisiones, ItemHistorial, ResultadoParseo, VentaHotmart,
} from './tipos'
...
function comisionDe(
    ...
    const propias = comisiones.filter(
        c => String(c?.source ?? '').trim().toUpperCase() === fuente,
    )
    ...
    for (const c of propias) {
        ...
        moneda =
            moneda ??
            texto(c.commission?.currency_code) ?? texto(c.commission?.currency_value) ??
            texto(c.currency_value) ?? texto(c.currency_code)
    }
    return { valor, moneda }
}
...
function extraerOrigen(compra: HotmartCompra | undefined, datos: AnyObj) {
    ...
    const buscar = (...claves: string[]): string | null => {
        for (const k of claves) {
            const v =
                texto(custom[k]) ?? texto(tracking[k]) ?? texto(origin[k]) ??
                texto(compraObj[k]) ?? texto(datos[k])
            if (v) return v
        }
        return null
    }
...
function extraerComprador(buyer: HotmartComprador | undefined) {
    return {
        ...
        comprador_pais:
            texto(buyer?.address?.country_iso) ??
            texto(buyer?.address?.country) ??
            texto(buyer?.country),
    }
}
...
function base(
    ...
    VentaHotmart,
    | 'transaction_id' | 'parent_transaction_id' | 'fecha_venta' | 'aprobada_at'
    | 'orden_at' | 'estado' | 'evento_ts' | 'reembolsada_at' | 'tipo'
    | 'clasificacion_origen' | 'tab_id' | 'oferta_codigo' | 'es_order_bump'
    | 'pago_tipo' | 'pago_cuotas' | 'checkout_pais' | 'origen'
> | null {
...
export function parsearWebhook(payload: unknown, ahora: Date = new Date()): ResultadoParseo {
    ...
    const nombreEvento =
        texto(wh.event) ?? texto((wh as AnyObj).event_type) ?? ''
    ...
    const productor = comisionDe(compra.commissions ?? datos.commissions as HotmartComision[], 'PRODUCER')
    const afiliado = comisionDe(compra.commissions ?? datos.commissions as HotmartComision[], 'AFFILIATE')
    const coproductor = comisionDe(compra.commissions ?? datos.commissions as HotmartComision[], 'COPRODUCER')
