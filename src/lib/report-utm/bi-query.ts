...
export async function runBiQuery(params: BiQueryParams): Promise<BiQueryRow[]> {
  ...
  const { campos: leadCampos, segmentos: leadSegmentos } = await loadLeadCamposSiHaceFalta(
    supabase,
    params
  );
  ...
  for (const m of params.metrics) {
    ...
    if (met) {
      sheetReqs.set(m, { kind: 'campo', campoClave: met.clave, clave: met.clave, outKey: m });
      continue;
    }
  ...
  if (needsLeads) {
    leadsData = await queryLeadsDirect(
      supabase,
      params,
      dateFrom,
      dateTo,
  ...
  if (sheetPorEntidad) {
    ...
    sheetData = await querySheetLeadsDirect(
      params,
      dateFrom,
      dateTo,
      sheetFields,
...
function hasAnyEntityFilter(
  ...
  return (Object.values(ENTITY_FILTER_FIELD) as string[]).some((f) =>
    hasEntityFilter(filters, advancedFilter, f)
  );
}

// ── Campos de formulario: tipos de agregación ─────────────────────────

interface FieldMetricReq {
  agg: FieldAgg;
  key: string;
  outKey: string;
}
...
function fieldAggValue(acc: FieldAcc | undefined, agg: FieldAgg): number {
  ...
  switch (agg) {
    ...
    case 'avg':
      return acc.n > 0 ? round2(acc.sum / acc.n) : 0;
    case 'min':
      return acc.n > 0 ? round2(acc.min) : 0;
    case 'max':
      return acc.n > 0 ? round2(acc.max) : 0;
    case 'count':
      return acc.present;
    default:
      return 0;
...
export function evalAdvancedRow(
  ...
  for (const g of af.groups) {
    ...
    for (const c of conds) {
      ...
      applicable = true;
      if (matchFilterCondition(cell, c.op, c.value)) {
        pass = true;
        break;
      }
    }
...
export function collectAdvancedColumns(
  af: AdvancedFilter | undefined,
  table: AdvTable
): { cols: string[]; needsRawFields: boolean } {
...
function buildEntityFilterPlan(
  ...
  for (const kind of ENTITY_KINDS) {
    ...
    if (hasEntityFilter(params.filters, params.advancedFilter, field)) {
      matchers.push({
        kind,
        test: entityNameFilterPredicate(params.filters, params.advancedFilter, field),
      });
    }
  }
...
async function querySalesDirect(
  ...
  let data = await fetchAllRows(() =>
    applyBase(
      supabase.schema('report_utm').from('sales_events').select(Array.from(selectCols).join(','))
    )
  );
...
> = {
  campaign: {
    rank: 1,
    metaCol: 'meta_campaigns',
    tiktokCol: 'tiktok_campaigns',
...
const adsDailyCoberturaCache = new Map<
  string,
  { desde: string; hasta: string; ts: number } | null
>();
...
async function adsDailyCubre(
  ...
  if (!rango || now - rango.ts > ADS_DAILY_COBERTURA_TTL_MS) {
    ...
    rango = {
      desde: String(min.fecha).slice(0, 10),
      hasta: String(max.fecha).slice(0, 10),
      ts: now,
    };
...
async function queryAdsScalar(
  ...
  for (const r of data as unknown as Record<string, unknown>[]) {
    ...
    entry.clicks +=
      (platform === 'tiktok' ? 0 : Number(r.meta_clicks ?? 0)) +
      (platform === 'meta' ? 0 : Number(r.tiktok_clicks ?? 0));
    entry.impressions +=
      (platform === 'tiktok' ? 0 : Number(r.meta_impressions ?? 0)) +
...
async function queryAdsFromJsonb(
  ...
  for (const r of data as unknown as Record<string, unknown>[]) {
    const dateKey =
      params.dimension === 'date' ? truncateDate(String(r.fecha ?? ''), grouping) : 'total';
...
async function querySheetLeadsDirect(
  ...
  for (const raw of filas) {
    ...
    if (plan.inMemory && !rowPassesEntityFilters(rec as Record<string, unknown>, plan, resolver!))
      continue;
 ...
 */
async function querySubsLatest(
  params: BiQueryParams,
  _dateFrom: string,
  dateTo: string
  ...
  const { data } = await db
    ...
    .select(
      'active_count,delayed_count,canceled_count,total_count,active_recurring_value,captured_date'
    )
    .eq('cliente_id', publicId)
...
function mergeResults(
  ...
  for (const key of keys) {
    ...
    if (params.metrics.includes('cpl'))
      row.cpl = leads_count > 0 && spend > 0 ? round2(spend / leads_count) : null;
    if (params.metrics.includes('cpa'))
      row.cpa = sales_count > 0 && spend > 0 ? round2(spend / sales_count) : null;
    if (params.metrics.includes('roas')) row.roas = spend > 0 ? round2(revenue / spend) : null;
    if (params.metrics.includes('conversion_rate'))
      row.conversion_rate = leads_count > 0 ? round2((sales_count / leads_count) * 100) : null;
    if (params.metrics.includes('cpc'))
      row.cpc = clicks > 0 && spend > 0 ? round2(spend / clicks) : null;
    if (params.metrics.includes('cpm'))
      row.cpm = impressions > 0 && spend > 0 ? round2((spend / impressions) * 1000) : null;
    ...
    if (params.metrics.includes('ctr'))
      row.ctr = impressions > 0 ? round2((clicks / impressions) * 100) : null;
    ...
    if (params.metrics.includes('ga_avg_session_duration'))
      row.ga_avg_session_duration = gaAvgDuration;
    ...
    if (params.metrics.includes('hotmart_roas'))
      row.hotmart_roas = spend > 0 && hotmartRevenue > 0 ? round2(hotmartRevenue / spend) : null;
    if (params.metrics.includes('hotmart_cpa'))
      row.hotmart_cpa = spend > 0 && hotmartSales > 0 ? round2(spend / hotmartSales) : null;
    if (params.metrics.includes('hotmart_roi'))
      row.hotmart_roi =
        spend > 0 && hotmartRevenue > 0 ? round2(((hotmartRevenue - spend) / spend) * 100) : null;
    ...
    if (params.metrics.includes('offline_revenue'))
      row.offline_revenue = round2(off?.offline_revenue ?? 0);
  ...
  if (params.dimension === 'date') {
    rows.sort((a, b) =>
      String(a.dimension_value ?? '').localeCompare(String(b.dimension_value ?? ''))
    );
  ...
  if (leadFieldClave !== null) {
    ...
    if (campo && campo.valores_orden.length > 0) {
      const orden = ordenarBuckets(
        campo,
        rows.map((r) => String(r.dimension_value ?? ''))
      );
...
function getDimValue(
  ...
  if (leadClave !== null) {
    ...
    return (
      bucketDeLeadRaw(campo, row.raw_fields as Record<string, unknown> | null) ?? '(sin valor)'
    );
  }
...
export function applyDimFilters(
  q: any,
  filters?: Record<string, string>,
  isAllowed: (k: string) => boolean = isValidDimKey
): any {
  if (!filters) return q;
...
async function calcularValores(params: ParamsValores): Promise<ResultadoValores> {
  ...
  const supabase = await createAdminClient();
  const dateFrom =
    params.date_from ?? new Date(Date.now() - 90 * 86400_000).toISOString().slice(0, 10);
  const dateTo = params.date_to ?? new Date().toISOString().slice(0, 10);
  ...
  try {
    ...
    if (leadClave !== null) {
      if (!params.cliente_id) return sinValores('sin_cliente');
      const campos = await loadLeadCampos(supabase.schema('report_utm'), params.cliente_id, {
        soloActivos: true,
      });
...
export async function runFunnelQuery(params: {
  ...
  const [adsResult, leadsResult, salesResult] = await Promise.all([
    adsMetrics.length
      ? runBiQuery({
          ...queryParams,
          metrics: adsMetrics as BiMetric[],
          dimension: 'none',
          date_from: dateFrom,
          date_to: dateTo,
        })
    ...
    leadMetrics.length
      ? runBiQuery({
          ...queryParams,
          metrics: leadMetrics as BiMetric[],
          dimension: 'none',
          date_from: dateFrom,
          date_to: dateTo,
        })
