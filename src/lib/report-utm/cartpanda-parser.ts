...
export function parseCartPandaPayload(
    raw: unknown,
): ParsedCartPandaEvent | PayloadError {
    ...
    const eventName =
        asString(get(raw, 'event')) ??
        asString(get(raw, 'type')) ??
        asString(get(raw, 'status'))
    ...
    const currency =
        asString(get(data, 'currency')) ??
        asString(get(data, 'currency_code')) ??
        'BRL'
    ...
    const customer_email =
        ...
        asString(get(data, 'customer_email')) ??
        null
    const customer_phone =
        asString(get(customer, 'phone')) ??
        asString(get(customer, 'cellphone')) ??
        null
    const customer_country =
        asString(get(customer, 'country')) ??
        asString(get(customer, 'country_code')) ??
        null
    ...
    const transaction_type =
        asString(get(data, 'transaction_type')) ??
        asString(get(data, 'type')) ??
        'principal'
    ...
    const utmTerm =
        get(data, 'utm', 'utm_term') ??
        get(data, 'tracking', 'utm_term') ??
        get(data, 'utm_term')
    const utmId =
        get(data, 'utm', 'utm_id') ??
        get(data, 'tracking', 'utm_id') ??
        get(data, 'utm_id')
