...
export function parseShopifyPayload(payload: unknown): ParsedShopifyEvent | PayloadError {
  ...
  // Monto y moneda
  const rawAmount =
    asString(get(payload, 'total_price')) ?? asString(get(payload, 'subtotal_price'));
  ...
  const status: ParsedShopifyEvent['status'] = FINANCIAL_STATUS_MAP[financialStatus] ?? 'pending';

  // Timestamp
  const sale_timestamp =
    asString(get(payload, 'processed_at')) ?? asString(get(payload, 'created_at')) ?? null;
  ...
  const product_id =
    asString(get(firstItem, 'product_id')) ?? asString(get(firstItem, 'variant_id'));
  const product_name = asString(get(firstItem, 'name')) ?? asString(get(firstItem, 'title'));

  // Cliente
  const customer = get(payload, 'customer');
  const customer_email =
    asString(get(customer, 'email')) ?? asString(get(payload, 'email')) ?? null;
  const customer_name =
  ...
  const utm_source = noteAttrs['utm_source'] ?? landingParams?.get('utm_source') ?? null;
  const utm_medium = noteAttrs['utm_medium'] ?? landingParams?.get('utm_medium') ?? null;
  const utm_campaign = noteAttrs['utm_campaign'] ?? landingParams?.get('utm_campaign') ?? null;
  const utm_content = noteAttrs['utm_content'] ?? landingParams?.get('utm_content') ?? null;
  const utm_term = noteAttrs['utm_term'] ?? landingParams?.get('utm_term') ?? null;
  const utm_id = noteAttrs['utm_id'] ?? landingParams?.get('utm_id') ?? null;
