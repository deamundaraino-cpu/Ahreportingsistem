...
export function readVisitor(req: NextRequest): { visitorId: string; sessionId: string } {
    ...
    return {
        visitorId: existingVid && /^[a-f0-9]{32}$/.test(existingVid) ? existingVid : crypto.randomBytes(16).toString('hex'),
        sessionId: existingSid && /^[a-f0-9]{16}$/.test(existingSid) ? existingSid : crypto.randomBytes(8).toString('hex'),
    }
}
...
export function buildTouchFromUrl(url: URL, referrer: string | null): Touch | null {
    ...
    const fields: Touch = {
        ...
        click_id:
            sp.get('fbclid') ?? sp.get('gclid') ?? sp.get('ttclid') ?? sp.get('click_id') ?? null,
        ts: new Date().toISOString(),
        referrer,
    }
    // Solo guardamos un touch si hay alguna señal de atribución
    const hasSignal =
        fields.source || fields.campaign || fields.click_id || fields.medium
