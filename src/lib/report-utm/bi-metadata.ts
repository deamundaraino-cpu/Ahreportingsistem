...
export const DEFAULT_FUNNEL_STAGES: BiMetric[] = [
  'impressions',
  'clicks',
  'leads_count',
  'sales_count',
...
export const METRIC_META: Record<BiMetric, MetricMetaEntry> = {
  ...
  ga_bounce_rate: {
    label: 'Tasa de rebote (GA4)',
    format: 'percent',
    group: 'ga4',
    breakdown: 'total',
  },
  ga_avg_session_duration: {
    label: 'Duración sesión GA4 (s)',
    format: 'number',
    group: 'ga4',
    breakdown: 'total',
  },
  ...
  ventas_principal: {
    label: 'Ventas principal (neto)',
    format: 'currency',
    group: 'hotmart',
    breakdown: 'total',
  },
  ventas_bump: {
    label: 'Ventas bump (neto)',
    format: 'currency',
    group: 'hotmart',
    breakdown: 'total',
  },
  ventas_upsell: {
    label: 'Ventas upsell (neto)',
    format: 'currency',
    group: 'hotmart',
    breakdown: 'total',
  },
  ...
  ventas_principal_bruto: {
    label: 'Ventas principal (bruto)',
    format: 'currency',
    group: 'hotmart',
    breakdown: 'total',
  },
  ventas_bump_bruto: {
    label: 'Ventas bump (bruto)',
    format: 'currency',
    group: 'hotmart',
    breakdown: 'total',
  },
  ...
  hm_roas: {
    label: 'ROAS (Hotmart real)',
    format: 'ratio',
    group: 'hotmart',
    breakdown: 'campaign',
  },
  hm_cpa: {
    label: 'CPA (Hotmart real)',
    format: 'currency',
    group: 'hotmart',
    breakdown: 'campaign',
  },
  ...
  offline_ventas: {
    label: 'Ventas offline (Sheet)',
    format: 'number',
    group: 'offline',
    breakdown: 'total',
  },
  offline_revenue: {
    label: 'Revenue offline (Sheet)',
    format: 'currency',
    group: 'offline',
    breakdown: 'total',
  },
  ...
  subs_total: {
    label: 'Suscripciones (total)',
    format: 'number',
    group: 'subs',
    breakdown: 'global',
  },
  subs_mrr: {
    label: 'MRR (valor recurrente)',
    format: 'currency',
    group: 'subs',
    breakdown: 'global',
  },
...
export function platformFromUtm(
  source?: string | null,
  medium?: string | null
): 'meta' | 'tiktok' | null {
...
export function parseOfflineFieldMetric(
  token: string
): { type: OfflineFieldType; key: string } | null {
    ...
    groups: groups.map((g) => {
      ...
      return {
        conditions: Array.isArray(conds)
          ? conds
              .filter(
                (c): c is FilterCondition => !!c && typeof (c as FilterCondition).field === 'string'
              )
              .map((c) => ({
                field: c.field,
                op: (c.op ?? 'eq') as FilterOp,
...
export function serializeAdvancedFilter(af: AdvancedFilter): string {
  const groups = (af.groups ?? [])
    .map((g) => ({
      conditions: (g.conditions ?? []).filter((c) => c.field && c.value && c.value.trim()),
    }))
...
export function appendAdvancedFilter(
  ...
  const combined: AdvancedFilter = {
    groups: [...(report.groups ?? []), ...(widgetFilter?.groups ?? [])],
  };
...
export function matchFilterCondition(cell: string, op: FilterOp, target: string): boolean {
  ...
  switch (op) {
    case 'eq':
      return parts.some((x) => v === x);
    case 'neq':
      return !parts.some((x) => v === x);
    case 'contains':
      return lc.includes(lt);
    case 'ncontains':
      return !lc.includes(lt);
    case 'starts':
      return lc.startsWith(lt);
    case 'ends':
      return lc.endsWith(lt);
    default:
      return true;
  }
}
...
export function matchFilterConditionNorm(cell: string, op: FilterOp, target: string): boolean {
  ...
  switch (op) {
    ...
    case 'neq':
      return !parts.some((x) => v === x);
    case 'contains':
      return v.includes(lt);
    case 'ncontains':
      return !v.includes(lt);
    case 'starts':
      return v.startsWith(lt);
    case 'ends':
      return v.endsWith(lt);
    default:
      return true;
  }
}
 ...
 */
export const NON_ATTRIBUTABLE_FIELDS: ReadonlySet<string> = new Set([
  'ip_country',
  'form_name',
  'form_plugin',
  'attribution_method',
 ...
 */
export function splitEntityGroups(af: AdvancedFilter | undefined): {
  entityOnly: AdvancedFilter | undefined;
  rest: AdvancedFilter | undefined;
} {
...
export function hasNonAttributableFilter(
  ...
  for (const [k, v] of Object.entries(filters ?? {})) {
    ...
    if (NON_ATTRIBUTABLE_FIELDS.has(k) || isFieldDim(k) || isLeadFieldDim(k) || isSheetDim(k))
      return true;
  }
  return !!advancedFilter?.groups?.some((g) =>
    g.conditions?.some(
      (c) =>
        c.value &&
        c.value.trim() &&
        (NON_ATTRIBUTABLE_FIELDS.has(c.field) ||
          isFieldDim(c.field) ||
          isLeadFieldDim(c.field) ||
          isSheetDim(c.field))
    )
  );
}
...
export const METRIC_GLOSSARY: Record<string, string> = {
  ...
  frequency:
    'Cuántas veces vio el anuncio, en promedio, cada persona. Si sube mucho, el público se está saturando.',
...
const GROUP_GLOSSARY: Record<MetricGroup, string> = {
  ...
  hotmart:
    'Dato de facturación de Hotmart en el período. Es del total de la cuenta: no se reparte por campaña.',
