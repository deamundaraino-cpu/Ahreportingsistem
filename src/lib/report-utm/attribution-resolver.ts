...
export async function resolveAttribution(
  ...
  const { data: history } = await db
    .from('pixel_events')
    .select(
      'visitor_id, event_type, page_url, referrer, utm_source, utm_medium, utm_campaign, utm_content, utm_term, click_id, created_at'
    )
    .eq('cliente_id', sale.cliente_id)
...
export function dedupTouches<T>(
  ...
  const igual = first !== null && last !== null && JSON.stringify(first) === JSON.stringify(last);
