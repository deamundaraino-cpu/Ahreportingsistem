...
export const SALES_SOURCE: DataSource = {
    ...
    fields: [
        ...
        money(S, 'revenue', 'Revenue', 'Dinero total facturado por las ventas del período.', 'ventas',
            { column: 'amount', recommended: true }),

        derived(S, 'cpa', 'CPA',
            'Costo por Adquisición: cuánto cuesta, en promedio, conseguir una venta. Cuanto MÁS BAJO, mejor.',
            'ventas', 'ads.spend / sales.count',
            { format: 'currency', nullUnless: ['sales.count'], recommended: true, direction: 'down', goal: 'cpa_max' }),
        derived(S, 'roas', 'ROAS',
            ...
            'ventas', 'sales.revenue / ads.spend',
