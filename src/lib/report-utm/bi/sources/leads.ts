...
export const LEADS_SOURCE: DataSource = {
    ...
    fields: [
        ...
        measure(S, 'count', 'Leads (contactos)',
            ...
            'leads',
            {
                agg: 'count', column: 'id', recommended: true, funnelStage: 70,
                goal: 'leads_target',
        ...
        derived(S, 'cpl', 'CPL',
            ...
            'leads', 'ads.spend / leads.count',
            { format: 'currency', nullUnless: ['leads.count'], recommended: true, direction: 'down', goal: 'cpl_max' }),

        derived(S, 'conversion_rate', 'Conv. Rate',
            'Porcentaje de leads que terminaron comprando.',
            'leads', 'sales.count / leads.count',
        ...
        dimension(S, 'campaign', 'Campaña', 'campaign', 'leads',
            { column: 'utm_campaign', resolve: 'entity' }),
        dimension(S, 'content', 'Contenido (anuncio)', 'ad', 'leads',
            { column: 'utm_content', resolve: 'entity' }),
        dimension(S, 'term', 'Término (conjunto)', 'adset', 'leads',
            { column: 'utm_term', resolve: 'entity' }),
        ...
        dimension(S, 'utm_campaign_raw', 'Campaña UTM (crudo)', 'lead_column', 'leads',
            { column: 'utm_campaign', resolve: 'entity_raw' }),
