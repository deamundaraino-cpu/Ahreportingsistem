...
export function applyDerived(compiled: Compiled[], row: RowValues): void {
  for (const c of compiled) {
    ...
    row[c.field.id] = bloqueado ? null : evalExpr(c.ast, row, { onMissing: 'null', decimals: 2 });
  }
}
...
export function compileCalculated(
  specs: CalcSpec[]
): Array<{ spec: CalcSpec; ast: Node; refs: string[] } | null> {
