...
export function parseExpr(src: string): ParsedExpr | ExprError {
    ...
    const parseFactor = (): Node => {
        ...
        if (c === '-') { i++; return { t: 'neg', v: parseFactor() } }
...
export function rewriteRefs(ast: Node, map: (id: string) => string): Node {
    switch (ast.t) {
        case 'num': return ast
        case 'ref': return { t: 'ref', id: map(ast.id) }
        case 'neg': return { t: 'neg', v: rewriteRefs(ast.v, map) }
        case 'bin': return { t: 'bin', op: ast.op, l: rewriteRefs(ast.l, map), r: rewriteRefs(ast.r, map) }
    }
}
...
export function serializeExpr(ast: Node): string {
    ...
    const walk = (n: Node): string => {
        switch (n.t) {
            ...
            case 'neg': return `-${prec(n.v) < 3 ? `(${walk(n.v)})` : walk(n.v)}`
            case 'bin': {
                const p = prec(n)
