...
async function ultimaFecha(
  ...
  const { data, error } = await q.order(columnaFecha, { ascending: false }).limit(1);
...
export async function recogerSenales(cliente: {
  ...
  const integraciones: SenalIntegracion[] = ((integRaw ?? []) as Record<string, unknown>[]).map(
    (i) => ({
      tipo: String(i.tipo ?? ''),
  ...
  const fuentes: SenalFuente[] = [
    ...
    },
    {
      ...
      configurada: tiene('meta_token') || tiene('tiktok_accounts'),
      requierePuente: true,
    },
    {
      id: 'cuenta',
      label: 'Cuenta (día)',
      ...cuenta,
      toleranciaDias: TOLERANCIA_DIAS.cuenta,
      configurada: Boolean(pid),
      requierePuente: true,
    },
    {
      id: 'offline',
      ...
      ...offline,
      toleranciaDias: TOLERANCIA_DIAS.offline,
      configurada: tiene('google_sheets_conversiones'),
      requierePuente: true,
    },
    {
      id: 'sheet',
      label: 'Sheet (filas)',
      ...sheet,
      toleranciaDias: TOLERANCIA_DIAS.sheet,
      configurada: tiene('google_sheets') || tiene('google_sheets_conversiones'),
      requierePuente: true,
    },
    {
      id: 'subs',
...
async function medirSheetAjeno(
  ...
  const { data } = await db.from('sheet_filas').select('valores').eq('cliente_id', pid).limit(500);
  const filas = (data ?? []) as Array<{ valores: Record<string, string> }>;
  if (filas.length === 0) return null;

  const conId = filas.map((f) => atribucionDeFila(f.valores).campaign_id).filter(Boolean);
