...
export function validateEnvVars(required: string[]): void {
  ...
  if (missing.length > 0) {
    throw new ApiError('INVALID_CONFIG', 'Missing required environment variables', 500, {
      missing,
    });
  }
}
...
export function validateSupabaseClient(
  client: unknown
): asserts client is { from: (table: string) => unknown } {
  if (!client || typeof client !== 'object' || !('from' in client)) {
    throw new ApiError('INVALID_CONFIG', 'Supabase client not properly initialized', 500);
  }
}
