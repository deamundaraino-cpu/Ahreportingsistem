 ...
 * qué campaña vienen.
 */
function filtroVacio(keyword: AnyCampaignFilter, campaignFilter: CampaignFilterSpec | undefined): boolean {
...
export function fechasEnRango(
    ...
    return Object.keys(porFecha).filter(f =>
...
export function agregarRespuestas(
    ...
    if (def.showDelta && opts?.prev) {
        ...
        if (iPrev !== -1) {
            ...
            const { porBucket: crudo } = sumarPorBucket(
                prev, iPrev, permPrev, Object.keys(prev.porFecha))
    ...
    if (filas.length > topN) {
        ...
        const top = new Set(
            [...filas].sort((a, b) => b.leads - a.leads).slice(0, topN).map(f => f.label))
    ...
    const buckets: BucketAgregado[] = filas.map(f => {
        ...
        const delta = leadsPrevio && leadsPrevio > 0
            ? ((f.leads - leadsPrevio) / leadsPrevio) * 100
            : null
        return {
            label: f.label,
            leads: f.leads,
