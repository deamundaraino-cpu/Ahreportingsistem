...
function isTabFilter(f: AnyCampaignFilter): f is TabCampaignFilter {
  return (
    ...
    f !== null &&
    'conditions' in f &&
    Array.isArray((f as TabCampaignFilter).conditions)
  );
}
...
function campaignMatchesGroup(
  ...
  for (const mapping of groupMappings) {
    ...
    if (
      ...
      campaignMatchesPattern(campaign, mapping.campaign_name_pattern)
    ) {
      return true;
    }
  }
...
function isEmptyFilter(filter: AnyCampaignFilter): boolean {
  ...
  if (isTabFilter(filter)) {
    return filter.conditions.every((c) =>
      Array.isArray(c.value) ? c.value.length === 0 : c.value === ''
    );
  }
...
type CampaignGroupLike = {
  id?: string;
  campaign_group_mappings?: Array<{ campaign_id?: string; campaign_name_pattern?: string }>;
};
...
export function filterCampaignList(
  ...
  if (isTabFilter(filter)) {
    ...
    const conds = filter.conditions.filter((c) =>
      Array.isArray(c.value) ? c.value.length > 0 : c.value !== ''
    );
  ...
  return campaigns.filter((c: any) =>
    campaignMatchesOne(c, filter as string | CampaignFilterSpec, campaignGroups)
  );
}
...
export function enrichMetaRow(row: any, filter: AnyCampaignFilter, campaignGroups?: any[]): any {
  ...
  const ri = (field: string) =>
    matching.reduce((s: number, c: any) => s + (parseInt(c[field] || '0') || 0), 0);
  const rf = (field: string) =>
    matching.reduce((s: number, c: any) => s + (parseFloat(c[field] || '0') || 0), 0);
 ...
 */
export function enrichTikTokRow(row: any, filter: AnyCampaignFilter, campaignGroups?: any[]): any {
...
export function serializeTabFilter(f: string | TabCampaignFilter | null | undefined): string {
  ...
  const conds = f.conditions.filter((c) =>
    Array.isArray(c.value) ? c.value.length > 0 : String(c.value ?? '') !== ''
  );
  ...
  return (
    TAB_FILTER_PREFIX + JSON.stringify({ mode: f.mode === 'or' ? 'or' : 'and', conditions: conds })
  );
}
