...
export function construirSystem(ctx: AgentContext, contextoExtra?: string): string {
  const base = [
    ...
    '',
    'Tiempo real frente a sincronizado:',
    '',
    '· Los LEADS y las VENTAS del CRM entran por webhook y están al día al segundo. Para',
    '  «¿cuántos leads van hoy?» usa `get_leads`, no `meta_leads` de get_metrics (eso es lo que',
    '  Meta reportó en la última sincronización).',
    '· El GASTO, las impresiones, los clics, las conversiones de Meta, Hotmart y GA4 se leen de la',
    '  última sincronización. Si te preguntan por el gasto de HOY, mira `get_sync_status`; si la',
    '  última sincronización es de hace más de unas horas, propón `trigger_sync` y di con qué corte',
