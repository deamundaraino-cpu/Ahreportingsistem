 ...
 */
export async function preloadUsdRates(
    db: any,
    monedas: string[],
    fecha: string,
): Promise<void> {
    const wanted = Array.from(new Set(
        monedas.map(m => String(m || '').toUpperCase()).filter(m => m && m !== 'USD')
    ))
...
export async function getUsdRate(db: any, moneda: string, fecha: string): Promise<FxLookup> {
    ...
    const promise = (async (): Promise<FxLookup> => {
        ...
        if (fresh) {
            await db.from('fx_rates')
                .upsert([{ fecha, moneda: cur, usd_rate: fresh, fuente: 'open.er-api.com' }], { onConflict: 'fecha,moneda' })
