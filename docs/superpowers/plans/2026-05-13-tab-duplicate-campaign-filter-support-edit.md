...
export function enrichMetaRow(row: any, keywordFilter: string, campaignGroups?: any[]): any {
  ...
  if (keywordFilter && campaignGroups && campaignGroups.length > 0) {
    ...
    if (selectedGroup && selectedGroup.campaign_group_mappings) {
      ...
      matching = row.meta_campaigns.filter(
        (c: any) => kw === '' || c.name?.toLowerCase().includes(kw)
      );
    }
  ...
  return {
    ...
    meta_cost_per_complete_registration:
      ri('complete_registration') > 0 ? spend / ri('complete_registration') : 0,
    ...
    meta_cost_per_initiate_checkout:
      ri('initiates_checkout') > 0 ? spend / ri('initiates_checkout') : 0,
    meta_landing_page_views: ri('landing_page_views'),
    meta_cost_per_landing_page_view:
      ri('landing_page_views') > 0 ? spend / ri('landing_page_views') : 0,
...
async function handleDuplicate() {
  ...
  if (res.error) {
    alert('Error al duplicar: ' + res.error);
    return;
  }
  onClose();
}
