...
const conAmbas = (fecha: string) => ({
    cliente_id: CLIENTE, fecha, sync_hash: 'h',
    meta_spend: 1039587, meta_impressions: 231418, meta_clicks: 6072, meta_campaigns: [],
    tiktok_spend: 237866, tiktok_campaigns: [],
    synced_at: 'now', is_partial: false,
})
/** Meta omitida a propósito: debe conservarse lo que ya hay en BD. */
const soloTikTok = (fecha: string) => ({
    cliente_id: CLIENTE, fecha, sync_hash: 'h',
    tiktok_spend: 179364, tiktok_campaigns: [],
    synced_at: 'now', is_partial: false,
})

const lote = [
    soloTikTok('2026-07-24'), soloTikTok('2026-07-25'),
    soloTikTok('2026-07-26'), soloTikTok('2026-07-27'),
    conAmbas('2026-07-28'), conAmbas('2026-07-29'),
]
...
check('cada grupo es homogéneo (misma firma en todas sus filas)',
...
check('la firma no depende del orden de las claves',
...
check('sin omisiones cuando vienen las dos', plataformasOmitidas(conAmbas('2026-07-28')).length === 0)
check('detecta las dos omitidas',
...
// TikTok en ads_daily).
check('la limpieza de ads_daily está acotada por plataforma',
    /\.from\('ads_daily'\)\s*\n\s*\.delete\(\)[\s\S]{0,200}?\.eq\('plataforma', plataforma\)/.test(worker),
...
for (const tipo of [
    ...
    'onsite_conversion.lead', 'onsite_conversion.lead_grouped', 'onsite_web_lead',
]) {
...
check('el agregado por familia sigue siendo MAX (no suma)',
...
check('el catálogo de TikTok guarda la jerarquía',
    /padresAds/.test(worker) && /padresAdgroups/.test(worker))
check('las filas de nivel ad/adgroup adjuntan campaign_id',
    /campaign_id:\s*campaignId/.test(worker))
check('la campaña se resuelve a nombre desde el catálogo',
...
const adgroupFila = mapElemento('tiktok_adgroups', {
    ...
    spend: 126835, clicks: 1060, impressions: 67055, conversions: 52,
    account_id: '7387511059776798737',
}, { cliente_id: CLIENTE, fecha: '2026-08-04' })!

check('ads_daily recibe la campaña del conjunto TikTok',
    adgroupFila.campana_id === '1871435133624754' &&
    adgroupFila.campana_nombre?.includes('[LSP] - [EVS]') === true)
check('el conjunto conserva su propia identidad',
    adgroupFila.nivel === 'adset' && adgroupFila.entidad_id === '1871435133625346')
check('la campaña NO se cuela como evento numérico',
    !('campaign_id' in adgroupFila.eventos) && !('campaign_name' in adgroupFila.eventos))

const adFila = mapElemento('tiktok_ads', {
    ad_id: '99', ad_name: 'AD 1',
    campaign_id: '1871435133624754', campaign_name: 'C1',
    adset_id: '1871435133625346', adset_name: 'AG 1',
    spend: 10, account_id: '7387511059776798737',
}, { cliente_id: CLIENTE, fecha: '2026-08-04' })!
check('el anuncio TikTok cuelga de campaña y conjunto',
...
const sinPadre = mapElemento('tiktok_adgroups', {
    adgroup_id: '1', adgroup_name: 'X', campaign_id: null, campaign_name: null, spend: 5,
}, { cliente_id: CLIENTE, fecha: '2026-08-04' })
check('sin campaña conocida la fila sigue existiendo',
...
const filasEspejo = expandirFila({
    ...
    tiktok_adgroups: [{ adgroup_id: 'g1', adgroup_name: 'G1', campaign_id: 'c1', campaign_name: 'C1', spend: 2 }],
    tiktok_ads: [{ ad_id: 'a1', ad_name: 'A1', campaign_id: 'c1', campaign_name: 'C1', adset_id: 'g1', spend: 3 }],
}, { hoy: '2026-08-05' })
check('el espejo genera los 3 niveles de TikTok',
    new Set(filasEspejo.map(f => f.nivel)).size === 3)
check('los 3 niveles comparten la misma campaña',
...
function dbStub(filas: Array<Record<string, unknown>>) {
    ...
    const q: any = {
        select: () => q,
        eq: (col: string, val: unknown) => { aplicados.push({ col, val }); return q },
        neq: (col: string, val: unknown) => { aplicados.push({ col: `!${col}`, val }); return q },
        gte: (col: string, val: unknown) => { aplicados.push({ col: `>=${col}`, val }); return q },
        limit: () => Promise.resolve({ data: filas, error: null }),
        update: (patch: Record<string, unknown>) => { aplicados.push({ col: 'update', val: patch }); return q },
    }
...
async function comprobarCola() {
    ...
    check('releaseJob devuelve el job a pending sin gastar intentos',
        patch?.estado === 'pending' && patch?.locked_at === null && patch?.locked_by === null &&
        !('intentos' in patch) && !('last_error' in patch))

    const runner = leerFuente('src/lib/sync/runner.ts')
    check('el runner comprueba si el cliente está ocupado antes de ejecutar',
        /clienteOcupado\(db, job\.cliente_id, job\.id, opts\.workerId/.test(runner))
    check('el job devuelto a la cola no cuenta como reclamado',
        runner.indexOf('await releaseJob(db, job.id)') < runner.indexOf('result.claimed++'))
    check('no gira en vacío: corta si vuelve el mismo job liberado',
