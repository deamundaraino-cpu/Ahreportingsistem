...
async function main() {
  ...
  console.log(
    `Resumen: ${criticos} cliente(s) con algo crítico · ${avisos} con avisos · ` +
  ...
  for (const c of clientes) {
    ...
    if (n > 0) {
      ...
      ruta =
        `webhook de ${pasarelas.join('/')} dado de alta pero sin entregar ` +
        '— revisar alta en la plataforma, secreto y URL';
    } else if (pareceCrm) {
      ruta =
        'la venta se cierra fuera de una pasarela y vive en el CRM del Sheet ' +
