...
async function main() {
    ...
    if (huecos.length === 0) {
        ...
        console.log(`  ${pad('CLIENTE', 22)} ${pad('PLAT', 7)} ${pad('FECHA', 12)} ${pad('MOTIVO', 20)} PISTA`)
        console.log(`  ${'─'.repeat(84)}`)
        for (const h of huecos) {
            console.log(`  ${pad(h.cliente.trim(), 22)} ${pad(h.plataforma, 7)} ${pad(h.fecha, 12)} ${pad(h.motivo, 20)} ${h.otra_plataforma ?? ''}`)
        }
    ...
    const descuadres = await sql<{ cliente: string; plataforma: string; fecha: string; columna: string; array: string }>(`
        WITH d AS (
    ...
    if (descuadres.length === 0) {
        ...
        for (const d of descuadres) {
            console.log(`  ${pad(d.cliente.trim(), 22)} ${pad(d.plataforma, 7)} ${d.fecha}  columna=${d.columna}  campañas=${d.array}`)
        }
    ...
    if (porCliente.size > 0) {
        ...
        for (const [id, e] of porCliente) {
            ...
            console.log(`  GET /api/worker?client_id=${id}&start=${ord[0]}&end=${ord[ord.length - 1]}&force=1\n`)
        }
    }
