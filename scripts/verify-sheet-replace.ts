...
async function main() {
  ...
  for (const pasada of [1, 2]) {
    ...
    for (const c of conSheets) {
      ...
      for (const r of results) {
        ...
        console.log(
          `  ${c.nombre} · ${r.name}: ${r.rowsProcessed} filas, ${r.rawProcessed} crudas${err}`
        );
  ...
  for (const [k, v] of Object.entries(final)) {
    ...
    check(
      v.lotes.size === 1,
      `${tabla}: un solo lote`,
      `${v.lotes.size} lote(s), ${v.filas} filas`
    );
  }
  ...
  check(
    repetidas === 0,
    'sheet_filas: sin coordenadas repetidas',
    `${repetidas} repetida(s) de ${vistas.size}`
  );
  ...
  check(
    borrados === (viejos.count ?? 0),
    'borra exactamente los eventos fuera de retención',
    `${borrados} borrado(s), ${viejos.count} fuera de retención`
  );
  check(
    (despues.count ?? 0) === (antes.count ?? 0) - borrados,
    'no toca los eventos dentro de retención',
    `${antes.count} → ${despues.count}`
  );

  const quedanViejos = await rutm
    .from('pixel_events')
    .select('id', { count: 'exact', head: true })
    .lt('created_at', corte);
  check(
    (quedanViejos.count ?? 0) === 0,
    'no quedan eventos fuera de retención',
    String(quedanViejos.count)
  );
