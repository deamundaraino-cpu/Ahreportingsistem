...
import {
    plegarConteos, ordenarPorFrecuencia, recortar, aValoresPlanos,
    parseSeleccion, serializarSeleccion, tieneComa, esSeleccionPorCasillas,
    componerFilas, alternarValor, sinValores, VACIO_HONESTO,
...
const splitViejo = (s: string) => s.split(',').map(x => x.trim()).filter(Boolean)
...
for (const g of guardadosReales) {
    ...
    check(`«${g || '(vacío)'}» se lee igual que antes`,
...
check('ida y vuelta lo conserva entero',
...
check('el split anterior SÍ lo partía (el fallo que se corrige)',
    splitViejo(conComa).length === 2)

check('varios valores, uno con coma',
    JSON.stringify(parseSeleccion(serializarSeleccion(['fb', conComa, 'ig'])))
        === JSON.stringify(['fb', conComa, 'ig']))
check('un valor que ya trae una barra invertida sobrevive',
...
check('los espacios de sobra se recortan',
    JSON.stringify(serializarSeleccion([' fb ', 'ig'])) === JSON.stringify('fb,ig'))
check('un valor vacío no genera una entrada',
    serializarSeleccion(['fb', '', '  ']) === 'fb')
...
check('alternar un valor con coma no rompe la lista',
...
// resuelven a lo mismo tienen que SUMAR, no competir.
{
    ...
    check('una etiqueta vacía también descarta',
...
// ════════════════════════════════════════════════════════════
{
    ...
    check('empate → alfabético estable',
        orden[1].valor === 'a' && orden[2].valor === 'c')
    ...
    const conOrden = ordenarPorFrecuencia(
        new Map([['20-100', 5], ['1-10', 2], ['11-20', 99]]),
        ['1-10', '11-20', '20-100'])
    check('el orden del analista gana a la frecuencia',
...
// total se dice.
{
    ...
    check('son los MÁS FRECUENTES, no los primeros que llegaron',
...
// que seguir funcionando.
{
    const r: ResultadoValores = {
        valores: [{ valor: 'b', n: 9 }, { valor: 'a', n: 3 }],
...
for (const m of ['sin_cliente', 'dimension_no_listable', 'error_consulta', 'timeout'] as const) {
    ...
    check(`«${m}» viene con lista vacía y motivo`,
        r.valores.length === 0 && r.motivo === m && r.total === 0 && !r.truncado)
}
check('un vacío legítimo NO lleva motivo',
...
{
    const listados: ValorConConteo[] = [{ valor: 'a', n: 10 }, { valor: 'b', n: 5 }]
    const filas = componerFilas(listados, ['b', 'raro'])
    check('el seleccionado ausente aparece', filas.some(f => f.valor === 'raro'))
    ...
    check('el seleccionado presente conserva su conteo',
        filas.find(f => f.valor === 'b')?.n === 5)
    check('los marcados se marcan',
        filas.filter(f => f.marcado).map(f => f.valor).sort().join(',') === 'b,raro')
