...
check(
  'un lead de anuncio pone el adId en utm_id (cruza por el paso 3 de la cascada)',
  utmCtwa.utm_id === '120239585076400410'
);
check('y lo marca como tráfico de pago', utmCtwa.utm_medium === 'paid_social');
check(
  'la fuente de un Click-to-WhatsApp es facebook, no whatsapp',
  utmCtwa.utm_source === 'facebook'
);
check(
  'el nombre del anuncio va a utm_content',
  utmCtwa.utm_content === '🚨 ¿Reportado en centrales?'
);
...
check(
  'un adId manda sobre la etiqueta "Social media" de GHL',
  utmMal.utm_medium === 'paid_social',
  `dio ${utmMal.utm_medium}`
);
check('y ese lead también cruza por utm_id', utmMal.utm_id === '120249141702310410');

const utmOrg = deriveUtms(ORGANICO_IG);
check(
  'un lead orgánico se queda sin utm_id (cae en «(sin campaña)» con gasto 0)',
  utmOrg.utm_id === null
);
...
const camposUtmOrg = Object.entries(utmOrg).filter(
  ([k]) => k.startsWith('utm_') || k === 'click_id'
);
check(
  'mediumId NO aparece en ningún campo utm_* ni en click_id',
...
check(
  'unas UTMs reales se respetan tal cual',
  utmLanding.utm_source === 'google' && utmLanding.utm_medium === 'cpc'
);
check(
  'el nombre de campaña real llega a utm_campaign',
  utmLanding.utm_campaign === 'Marca — Búsqueda'
);
...
check(
  'sin ninguna señal, todos los utm quedan en null',
  [utmVacio.utm_source, utmVacio.utm_medium, utmVacio.utm_campaign, utmVacio.utm_id].every(
    (v) => v === null
  )
);
...
check(
  'un campo normal entra en raw_fields con el NOMBRE del campo',
  campos.raw_fields['Huéspedes'] === '4'
);
check(
  'y conserva el acento (fieldKey lo pierde: contact.huspedes)',
  Object.keys(campos.raw_fields).includes('Huéspedes')
);
check(
  'un campo de texto largo NO entra en raw_fields',
...
check(
  'el nombre compuesto se arma si no viene contactName',
...
check(
  'un campo de lead encuentra la respuesta que vino de GHL',
...
check(
  'y la encuentra aunque el catálogo se escribiera sin acento',
...
check(
  'external_id lleva el prefijo que evita chocar con Meta',
  fila.external_id === `${GHL_EXTERNAL_PREFIX}XTYuz69fWsNx5r9yTHW6`
);
check(
  'source y form_plugin identifican la fuente',
  fila.source === GHL_SOURCE && fila.form_plugin === GHL_SOURCE
);
check('el plugin tiene etiqueta legible en la UI', typeof PLUGIN_LABELS[GHL_SOURCE] === 'string');
check(
  'created_at es dateAdded, no la fecha de ingesta',
  fila.created_at === '2026-08-10T14:32:00.000Z'
);
check('el país ISO-2 alimenta la dimensión País', fila.ip_country === 'CO');
check(
  'los datos de contacto se mapean',
...
check(
  'no se inventan datos de navegación',
  fila.visitor_id === undefined && fila.page_url === undefined
);

const custom = fila.custom_data as Record<string, unknown>;
check(
  'custom_data guarda el id del contacto para poder volver a GHL',
  custom.ghl_contact_id === 'XTYuz69fWsNx5r9yTHW6'
);
check('y la atribución cruda, por si hay que auditar el cruce', custom.attribution_source !== null);
check(
  'el texto largo vive en custom_data, no en raw_fields',
...
check(
  'sin dateAdded no se fuerza created_at (lo pone el DEFAULT de la tabla)',
...
check('la fila orgánica tampoco lleva el mediumId en utm_id', filaOrganica.utm_id === null);
