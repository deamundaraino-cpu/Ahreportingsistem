...
function validar(): string[] {
  ...
  for (const p of PLANTILLAS) {
    ...
    for (const w of p.layout) {
      ...
      const metricas = String(w.config.metric ?? '')
        .split(',')
        .map((s) => s.trim())
        .filter(Boolean);
      if (!metricas.length) {
        errores.push(`${donde}: sin métrica`);
        continue;
      }
...
async function main() {
  ...
  console.log(
    `· ${PLANTILLAS.length} plantillas válidas (${PLANTILLAS.reduce((n, p) => n + p.layout.length, 0)} widgets)`
  );
...
async function probar(db: any) {
  ...
  if (!cid) {
    console.log('⚠ sin clientes con leads');
    return;
  }
    ...
    for (const w of p.layout) {
      ...
      if (w.type === 'funnel') {
        ...
        const rows = await runBiQuery({
          ...base,
          metrics: String(w.config.metric)
            .split(',')
            .map((s) => s.trim()) as any,
        ...
        muestra = primera
          ? Object.entries(primera)
              .filter(([k]) => k !== 'dimension_value' && k !== '__nocross')
              .map(([k, v]) => `${k}=${v}`)
              .join(' ')
          : '';
      }
      if (n === 0) vacios++;
      console.log(
        `    ${n > 0 ? '✓' : '✗'} ${w.title.padEnd(26)} ${String(n).padStart(4)} · ${muestra.slice(0, 90)}`
      );
    }
  }
  console.log(
