...
import {
    BASE_REGISTRY as REG, isAdditive, isPivotable, fieldCrossesDimension, funnelStages,
} from '../src/lib/report-utm/bi/registry'
import {
    CANONICAL_TO_LEGACY_MEASURE, LEGACY_DIMENSION_IDS, migrateMeasureId, migrateDimensionId,
...
async function main() {
    ...
    check(`las ${medidas.length} medidas se guardan con un id que el motor ya entiende`,
        noReconocidas.length === 0, noReconocidas.join(', '))
    ...
    check(`las ${dims.length} dimensiones se guardan con un id conocido`,
        dimsNoReconocidas.length === 0, dimsNoReconocidas.join(', '))
    const dimIdaVuelta = dimsGuardadas.filter((id, i) => migrateDimensionId(id) !== dims[i].id)
    check('ida y vuelta de dimensiones es exacta', dimIdaVuelta.length === 0, dimIdaVuelta.join(', '))
    ...
    const camposAfectados = dependientes.reduce(
        (a, s) => a + s.fields.filter(f => f.kind === 'measure').length, 0)
    console.log(`  · ${dependientes.map(s => s.label).join(', ')}`)
    check(`sin enlace se atenúan ${dependientes.length} fuentes de golpe (${camposAfectados} medidas)`,
    ...
    // ninguna explicación.
    check('leads y ventas NO dependen del enlace',
    ...
    const conPais = REG.measures().filter(m => !fieldCrossesDimension(REG, m.id, 'leads.ip_country'))
    console.log(`  · agrupando por País, ${conPais.length} de ${medidas.length} medidas caerían en el total`)
    check('agrupando por país se marca el gasto', conPais.some(m => m.id === 'ads.spend'))
    check('agrupando por país se marca el CPL (hereda del gasto)',
        conPais.some(m => m.id === 'leads.cpl'))
    check('agrupando por país NO se marca leads.count',
        !conPais.some(m => m.id === 'leads.count'))

    const conCampana = REG.measures().filter(m => !fieldCrossesDimension(REG, m.id, 'leads.campaign'))
    ...
    check('agrupando por campaña GA4 se marca', conCampana.some(m => m.id === 'cuenta.ga_sessions'))
    ...
    check(`el embudo ofrece ${etapas.length} etapas, no las 72 medidas`,
        etapas.length > 5 && etapas.length < medidas.length, String(etapas.length))
    check('las etapas del embudo son conteos, nunca importes',
        etapas.every(e => e.format === 'number'),
        etapas.filter(e => e.format !== 'number').map(e => e.id).join(', '))

    const pivotables = medidas.filter(m => isPivotable(REG, m.id))
    check(`la tabla dinámica ofrece ${pivotables.length} medidas (solo grano de fila)`,
        pivotables.length === 3, pivotables.map(m => m.id).join(', '))

    const aditivas = medidas.filter(isAdditive)
    check(`${aditivas.length} medidas admiten fila de totales`, aditivas.length > 40)
    check('el alcance NO admite total (personas únicas)',
        !aditivas.some(m => m.id === 'ads.reach'))
    check('ningún ratio admite total',
        !aditivas.some(m => m.format === 'ratio' || m.format === 'percent'),
        aditivas.filter(m => m.format === 'ratio' || m.format === 'percent').map(m => m.id).join(', '))
    ...
    check('las tres métricas de "leads" se declaran entre sí',
        ['leads.count', 'ads.leads_form', 'offline.leads'].every(id =>
            (REG.measure(id)?.conflictsWith?.length ?? 0) >= 2))
    ...
    check('todos los solapamientos apuntan a campos reales', solapeRoto.length === 0,
        solapeRoto.join(', '))
