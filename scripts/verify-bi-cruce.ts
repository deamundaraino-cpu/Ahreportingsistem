...
async function main() {
  ...
  for (const c of conEnlace) {
    ...
    check(
      `${nombre}: el gasto cruzado no supera el total`,
      s.matched <= s.total + 0.01,
      `${s.matched} > ${s.total}`
    );
    check(
      `${nombre}: el porcentaje está entre 0 y 100`,
      s.pct === null || (s.pct >= 0 && s.pct <= 100),
      String(s.pct)
    );
    check(`${nombre}: sin gasto → pct null (no 0%)`, s.total > 0 ? s.pct !== null : s.pct === null);
    ...
    if (s.total > 0) {
      ...
      check(
        `${nombre}: el gasto de las campañas sin leads cabe en lo no cruzado`,
        gastoHuerfano <= s.total - s.matched + 0.01,
        `huérfanas ${money(gastoHuerfano)} > no cruzado ${money(s.total - s.matched)}`
      );
      check(
        `${nombre}: ninguna huérfana tiene gasto 0`,
        s.orphans.every((o) => o.spend > 0)
      );
      check(
        `${nombre}: las huérfanas vienen ordenadas por gasto`,
    ...
    check(
      `${nombre}: los métodos de cruce suman el total de leads`,
      sumaMetodos === leadsTotal,
      `${sumaMetodos} ≠ ${leadsTotal}`
    );
  }
  ...
  for (const c of conEnlace) {
    ...
    check(
      `${c.nombre.trim()}: ningún valor está a la vez en mapeables e inválidos`,
      !d.suggestions.some((s) => d.invalid.some((i) => i.value === s.value))
    );
  }
  console.log(
    `  · en total: ${vistosMapeables} valores mapeables · ${vistosInvalidos} no mapeables`
  );
