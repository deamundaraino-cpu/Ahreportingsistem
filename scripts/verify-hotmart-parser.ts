...
import { parsearWebhook, parsearApi, instanteISO, comisionesPendientes } from '../src/lib/hotmart/parser'
...
check('evento desconocido no se marca aprobada',
...
check('el mapa de eventos no reconoce un status de la API',
    estadoDeEvento('APPROVED') === null)
check('el mapa de la API no reconoce un nombre de evento',
    estadoDeStatusApi('PURCHASE_APPROVED') === null)
check('cada mapa reconoce lo suyo',
...
check('un payload que no es objeto es ilegible',
    (() => { const r = parsearWebhook('hola', AHORA); return !r.ok && r.motivo === 'ilegible' })())
...
const vBump = venta(parsearWebhook(webhook({}, {
...
const vUpsell = venta(parsearWebhook(webhook({}, {
...
check('is_bump/is_upsell (que no existen en Hotmart) se ignoran',
...
const sinTel = venta(parsearWebhook({
    ...webhook(),
    data: {
        ...webhook().data,
...
const conUtmReal = venta(parsearWebhook(webhook({}, {
...
const nocturna = venta(parsearWebhook(webhook({}, {
    approved_date: Date.parse('2026-08-08T02:00:00Z'),
}), AHORA))!
check('02:00 UTC cuenta como el día anterior en Colombia',
    nocturna.fecha_venta === '2026-08-07', nocturna.fecha_venta)
const diurna = venta(parsearWebhook(webhook({}, {
    approved_date: Date.parse('2026-08-08T18:00:00Z'),
}), AHORA))!
check('18:00 UTC cuenta como el mismo día', diurna.fecha_venta === '2026-08-08', diurna.fecha_venta)

// ════════════════════════════════════════════════════════════
seccion('Estados y reembolsos')
// ════════════════════════════════════════════════════════════
for (const [ev, esperado] of [
    ['PURCHASE_APPROVED', 'aprobada'], ['PURCHASE_COMPLETE', 'completa'],
    ['PURCHASE_REFUNDED', 'reembolsada'], ['PURCHASE_CHARGEBACK', 'chargeback'],
    ['PURCHASE_CANCELED', 'cancelada'], ['PURCHASE_EXPIRED', 'expirada'],
...
check('las claves coinciden exactamente',
...
for (const [status, esperado] of [
    ['APPROVED', 'aprobada'], ['COMPLETE', 'completa'], ['REFUNDED', 'reembolsada'],
    ['CHARGEBACK', 'chargeback'], ['CANCELLED', 'cancelada'], ['EXPIRED', 'expirada'],
] as const) {
    const r = parsearApi({ ...itemHist, purchase: { ...itemHist.purchase, status } }, undefined, { ahora: AHORA })
    check(`status ${status} → ${esperado}`, r.ok && r.venta.estado === esperado)
}
check('la API sin transacción es ilegible',
    (() => { const r = parsearApi({ purchase: { status: 'APPROVED' } }, undefined, { ahora: AHORA }); return !r.ok })())
