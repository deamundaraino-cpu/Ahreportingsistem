...
import {
    mapElemento, expandirFila, normNombre, nivelesParaFecha, FORMAS,
...
const NOMBRES = [
    ...
    'promo_verano', 'Promo Verano', 'PROMO-VERANO',
    'Campaña Otoño', '  espacios   raros  ', 'LSP Ranco II', 'LSP EVS', '',
]
const divergentes = NOMBRES.filter(n => normNombre(n) !== normLabel(n))
check(`las dos normalizaciones coinciden en ${NOMBRES.length} nombres reales`,
...
check('dos escrituras del mismo nombre dan la MISMA clave (no duplica)',
    sinId.entidad_key === sinId2.entidad_key)

check('sin id NI nombre se descarta (no se inventa clave)',
    mapElemento('meta_ads', { spend: 10 }, CTX) === null)
check('una columna desconocida se descarta',
...
check('en nivel campaña, la entidad ES la campaña',
    camp.campana_id === 'c9' && camp.campana_nombre === 'Camp 1')
check('nivel correcto', camp.nivel === 'campaign' && camp.plataforma === 'meta')

const adset = mapElemento('meta_adsets', {
    adset_id: 's1', adset_name: 'Conjunto 1', campaign_id: 'c9', campaign_name: 'Camp 1', spend: 50,
}, CTX)!
check('en nivel conjunto se guarda su campaña',
    adset.campana_id === 'c9' && adset.adset_id === 's1' && adset.adset_nombre === 'Conjunto 1')

const ad = mapElemento('meta_ads', {
    ad_id: 'a1', ad_name: 'Anuncio 1', adset_id: 's1', adset_name: 'Conjunto 1',
    campaign_id: 'c9', campaign_name: 'Camp 1', account_id: 'act_1', spend: 25,
}, CTX)!
check('en nivel anuncio se guarda toda la jerarquía',
    ad.campana_id === 'c9' && ad.adset_id === 's1' && ad.entidad_id === 'a1' && ad.cuenta_id === 'act_1')
...
check('un anuncio de TikTok sin campaign_id queda con campana_id null (limitación real)',
...
const conEventos = mapElemento('meta_ads', {
    ad_id: 'a1', ad_name: 'A', spend: '12.5', impressions: 1000, clicks: 50,
    reach: 800, link_clicks: 40, purchases: 3, view_content: 20,
    // Cola larga: sin columna propia.
    search: 7, donate: 2, find_location: 1, customize_product: 4,
    custom_conversions: { lead_calificado: 5 },
    // Ratios: NO se guardan, se recalculan sobre los totales.
    cpc: 0.25, cpm: 12.5, ctr: 5, frequency: 1.25,
}, CTX)!
check('el gasto en texto se convierte', conEventos.spend === 12.5)
check('las columnas propias se llenan',
    conEventos.impressions === 1000 && conEventos.clicks === 50 &&
    conEventos.reach === 800 && conEventos.purchases === 3 && conEventos.view_content === 20)
check('la cola larga va a eventos',
    conEventos.eventos.search === 7 && conEventos.eventos.donate === 2 &&
    conEventos.eventos.find_location === 1)
check('custom_conversions se conserva como objeto',
...
// con ningún día.
check('los ratios NO se guardan',
    !('cpc' in conEventos.eventos) && !('ctr' in conEventos.eventos) &&
    !('frequency' in conEventos.eventos))
check('los eventos en 0 no ocupan sitio en el JSONB',
    mapElemento('meta_ads', { ad_id: 'x', ad_name: 'x', search: 0 }, CTX)!.eventos.search === undefined)
...
check('nivel campaña: `leads` alimenta leads_form',
    mapElemento('meta_campaigns', { name: 'C', leads: 42 }, CTX)!.leads_form === 42)
check('nivel anuncio: `leads_form` manda sobre `leads`',
    mapElemento('meta_ads', { ad_id: 'a', ad_name: 'a', leads: 1, leads_form: 9 }, CTX)!.leads_form === 9)
check('`leads` no se duplica en eventos',
...
check('hoy incluye los tres niveles',
    JSON.stringify(nivelesParaFecha('2026-08-03', HOY)) === '["campaign","adset","ad"]')
check(`hace ${VENTANA_NIVEL_AD_DIAS} días todavía incluye anuncio`,
    nivelesParaFecha('2026-07-04', HOY).includes('ad'))
check('hace 31 días ya NO incluye anuncio',
    !nivelesParaFecha('2026-07-03', HOY).includes('ad'))
check('pero campaña y conjunto se conservan SIEMPRE',
...
const filaReciente = {
    cliente_id: 'c1', fecha: '2026-08-01',
...
check('expande los tres niveles en fecha reciente', expReciente.length === 5,
    String(expReciente.length))
check('los niveles NO se mezclan',
...
const sumaCampana = expReciente.filter(r => r.nivel === 'campaign').reduce((a, r) => a + r.spend, 0)
check('sumar sin filtrar nivel infla el gasto (140 → 260): de ahí el aviso',
    sumaCampana === 140 && sumaTodo === 260, `campaña=${sumaCampana} todo=${sumaTodo}`)

const filaVieja = { ...filaReciente, fecha: '2025-01-01' }
const expVieja = expandirFila(filaVieja, { hoy: HOY })
check('en fecha vieja se omite el nivel anuncio',
...
check('las métricas aditivas se suman',
    dedup[0].spend === 25 && dedup[0].impressions === 300)
check('el alcance toma el MÁXIMO, no la suma (personas únicas)',
    dedup[0].reach === 90, String(dedup[0].reach))

seccion('Cobertura de las 6 columnas')
check('las 6 columnas JSONB están mapeadas', Object.keys(FORMAS).length === 6)
check('cada plataforma tiene sus 3 niveles',
