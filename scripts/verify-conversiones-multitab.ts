...

{
  ...
  check(
    'la pestaña hereda el nombre',
    cfgs[0].tabs?.[0].sheet_name === 'Hoja1',
    cfgs[0].tabs?.[0].sheet_name
  );
  check(
    'la pestaña hereda el mapeo',
    cfgs[0].tabs?.[0].col_fecha === 'Día',
    cfgs[0].tabs?.[0].col_fecha
  );
}

{
  ...
  check(
    'ids derivados de la posición',
    cfgs[0].id === 'sheet_0' && cfgs[1].id === 'sheet_1',
    `${cfgs[0].id}/${cfgs[1].id}`
  );
  check('ids únicos entre sheets', cfgs[0].id !== cfgs[1].id);
  check(
    'cada sheet estrena una pestaña',
    cfgs.every((c) => c.tabs?.length === 1)
  );
...

{
  const tabs: SheetTabConfig[] = [
    {
      ...
      enabled: true,
      custom_columns: {
        citas: { col_name: 'Citas', type: 'count', label: 'Citas', include: true },
      },
    },
    {
      id: 't2',
  ...
  check(
    'mergeTabCustomColumns une columnas de varias pestañas',
    Object.keys(merged).sort().join(',') === 'citas,ticket',
...

{
  ...
  check(
    'cuenta la cantidad <= 0',
    quality.cantidad_invalida === 1,
    String(quality.cantidad_invalida)
  );
  ...
  check(
    'marca el sheet de origen',
    parsed.every((r) => r.sheet_id === 'sheet-a')
  );
  check(
    'marca la pestaña de origen',
    parsed.every((r) => r.tab_name === 'Datos')
  );
  check(
    'columna extra detectada por defecto',
    parsed[0].custom_fields[sanitizeColName('Citas Agendadas')] === 3
  );
}

{
  ...
  check(
    'avisa de columna adicional ausente',
    quality.warnings.some((w) => w.includes('No Existe')),
    quality.warnings.join(' | ')
  );
}

{
  ...
  try {
    parseRowsForTab(
      headers,
      fakeRows(headers, [['a', 'b']]),
      { id: 't', sheet_name: 'X', enabled: true },
      's',
      'X'
    );
  } catch {
...

{
  ...
  for (const [texto, esperado] of casos) {
    ...
    const { rows: parsed } = parseRowsForTab(
      headers,
      rows,
      { id: 't', sheet_name: 'V', enabled: true },
      's',
      'V'
    );
    if (parsed[0]?.valor !== esperado) {
      ok = false;
      detalles.push(`${texto} → ${parsed[0]?.valor} (esperado ${esperado})`);
    }
  }
...

{
  ...
  const crudo = parseRowsForTab(
    headers,
    rows,
    { id: 't', sheet_name: 'F', enabled: true, col_fecha: 'created_time' },
    's',
    'F'
  );
  check(
    'sin cantidad se descartan todas las filas',
    crudo.rows.length === 0,
    String(crudo.rows.length)
  );
  check(
    'el descarte se explica en los avisos',
    crudo.quality.warnings.some((w) => w.includes('cantidad 0 o vacía')),
    crudo.quality.warnings.join(' | ')
  );
  ...
  check(
    'cada fila cuenta como 1',
    parsed.every((r) => r.cantidad === 1)
  );
  check(
    'tipo_fijo marca las filas como lead',
    parsed.every((r) => r.tipo === 'lead')
  );
  check(
    'fecha con espacio se recorta a YYYY-MM-DD',
    parsed[0].fecha === '2026-04-15',
    parsed[0].fecha
  );
  check('fecha ISO con T también', parsed[2].fecha === '2026-04-16', parsed[2].fecha);
  check(
    'no avisa de cantidad cuando count_rows está activo',
    !quality.warnings.some((w) => w.includes('cantidad')),
    quality.warnings.join(' | ')
  );
  check(
    'no avisa de tipo cuando hay tipo_fijo',
    !quality.warnings.some((w) => w.includes('de tipo')),
    quality.warnings.join(' | ')
  );
  ...
  check(
    'el tipo lead permite que sume en offline_leads',
    agg.every((a) => a.tipo === 'lead')
  );
}
...
sec('parseDate — descarta fechas imposibles');
{
  ...
  check(
    '29 de febrero bisiesto SÍ vale',
    parse('29/02/2024')[0]?.fecha === '2024-02-29',
    parse('29/02/2024')[0]?.fecha
  );
  check('29 de febrero no bisiesto se descarta', parse('29/02/2026').length === 0);
  check(
    'una fecha normal sigue funcionando',
    parse('15/04/2026')[0]?.fecha === '2026-04-15',
    parse('15/04/2026')[0]?.fecha
  );
  check(
    'ISO normal sigue funcionando',
    parse('2026-04-15')[0]?.fecha === '2026-04-15',
    parse('2026-04-15')[0]?.fecha
  );
  ...
  check(
    'ambigua: 05/13 se reinterpreta como mm/dd',
    parse('05/13/2026')[0]?.fecha === '2026-05-13',
    parse('05/13/2026')[0]?.fecha
  );
...

{
  ...
  check(
    'suma columnas de moneda',
    dia1.custom_fields.ticket === 30,
    String(dia1.custom_fields.ticket)
  );
  // (50*2 + 100*8) / 10 = 90 — no el promedio simple 75.
  check(
    'promedia porcentajes ponderando por cantidad',
...

{
  ...
  const a = parseRowsForTab(
    headers,
    fakeRows(headers, [['2026-07-01', 'lead', '2', '0', 'wa']]),
    tabA,
    's1',
    'A'
  ).rows;
  const b = parseRowsForTab(
    headers,
    fakeRows(headers, [['2026-07-01', 'lead', '3', '0', 'wa']]),
    tabB,
    's1',
    'B'
  ).rows;
  ...
  check(
    'el agregado suma ambas pestañas',
    agg[0].total_cantidad === 5,
    String(agg[0].total_cantidad)
  );
  check('las filas conservan su pestaña de origen', a[0].tab_name === 'A' && b[0].tab_name === 'B');
}
...

{
  ...
  const { conversiones, crudas, quality } = parseTabPayload(
    headers,
    rows,
    tab,
    'sheet-a',
  ...
  check(
    'la fila sin fecha no llega a la capa cruda',
    crudas.every((c) => c.fecha === '2026-07-01')
  );
  check(
    'solo_crudas cuenta las que no son conversión',
    quality.solo_crudas === 1,
    String(quality.solo_crudas)
  );
  check(
    'guarda todas las columnas, también las estándar',
    Object.keys(crudas[0].valores).sort().join(',') ===
      'cantidad,email,fecha,rango_de_ingresos,tipo',
    Object.keys(crudas[0].valores).join(',')
  );
  check(
    'el valor se guarda sin convertir',
    crudas[0].valores.rango_de_ingresos === '20 a 100',
    crudas[0].valores.rango_de_ingresos
  );
  check(
    'marca sheet y pestaña de origen',
    crudas.every((c) => c.sheet_id === 'sheet-a' && c.tab_name === 'Form A')
  );
  check(
    'fila_num es el número de fila de datos',
    crudas[0].fila_num === 1 && crudas[1].fila_num === 2,
    crudas.map((c) => c.fila_num).join(',')
  );
  check(
    'columnas_crudas refleja lo guardado',
    quality.columnas_crudas === 5,
    String(quality.columnas_crudas)
  );
}

{
  ...
  check(
    'parseRowsForTab sigue devolviendo las mismas conversiones',
    JSON.stringify(viejo.rows) === JSON.stringify(nuevo.conversiones)
  );
  check(
    'los contadores de calidad no cambian',
...

{
  ...
  const declaradas = parseTabPayload(
    headers,
    rows(),
    {
      ...base,
  ...
  check(
    'raw_mode "declared" solo guarda lo declarado con include',
...
{
  check(
    'detecta columnas de PII',
    esColumnaSensible('Correo electrónico') && esColumnaSensible('Teléfono')
  );
  check('detecta identificadores', esColumnaSensible('lead_id') && esColumnaSensible('id'));
  check(
    'no marca columnas de negocio',
    !esColumnaSensible('Rango de ingresos') && !esColumnaSensible('Ciudad')
  );
}
...

{
  ...
  const filas = fakeRows(headersReales, [
    ['2026-07-01', 'Ana', 'AGENDAMIENTO DIRECTO', '2026-07-05'],
  ]);
  const r = parseTabPayload(headersReales, filas, tab, 's', 'GESTION LEADS');
  check(
    'adivina la columna cuando ninguna se llama "fecha"',
    r.conversiones.length === 1 && r.conversiones[0].fecha === '2026-07-01',
    JSON.stringify(r.conversiones)
  );
  check(
    'avisa de qué columna eligió',
    r.quality.warnings.some((w) => w.includes('FECHA DE AGENDA')),
    r.quality.warnings.join(' | ')
  );
  check(
    'la fila entra como lead (count_rows + tipo_fijo)',
  ...
  const rAlias = parseTabPayload(
    conAlias,
    fakeRows(conAlias, [['2026-07-02', '1']]),
    { id: 't2', sheet_name: 'F', enabled: true },
    's',
    'F'
  );
  check(
    'resuelve el alias "created_time" sin avisar',
    rAlias.conversiones[0]?.fecha === '2026-07-02' &&
      !rAlias.quality.warnings.some((w) => w.includes('Sin columna de fecha')),
    rAlias.quality.warnings.join(' | ')
  );

  // Configurada explícitamente: manda la config, no la adivinanza.
  const rFijo = parseTabPayload(
    headersReales,
    filas,
    { ...tab, col_fecha: 'FECHA DE INVERSIÓN' },
    's',
    'GESTION LEADS'
  );
  check(
    'la columna configurada gana a la auto-detección',
    rFijo.conversiones[0]?.fecha === '2026-07-05',
...

{
  ...
  check(
    'parsea tipo y clave',
    p?.type === 'currency' && p?.key === 'ticket_promedio',
    JSON.stringify(p)
  );
  check('formato de moneda', offlineFieldFormat(tok) === 'currency');
  check(
    'formato de porcentaje',
    offlineFieldFormat(makeOfflineFieldMetric('percentage', 'tasa')) === 'percent'
  );
  check(
    'formato de conteo',
    offlineFieldFormat(makeOfflineFieldMetric('count', 'citas')) === 'number'
  );
  ...
  check(
    'etiqueta legible sin catálogo',
    offlineFieldLabel(tok) === 'Ticket promedio (Sheet)',
    String(offlineFieldLabel(tok))
  );
  check(
    'etiqueta usa el label configurado',
    offlineFieldLabel(tok, [
      { key: 'ticket_promedio', label: 'Ticket $', type: 'currency', sources: [] },
    ]) === 'Ticket $ (Sheet)'
  );
...

{
  ...
  check(
    'extrae las claves correctas',
    found
      .map((f) => f.key)
      .sort()
      .join(',') === 'citas_agendadas,ticket',
    found.map((f) => f.key).join(',')
  );
  check(
    'no extrae identificadores normales',
    extractOfflineFieldAliases('spend / leads_count').length === 0
  );
}
