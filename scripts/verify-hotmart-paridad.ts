...
type Agregado = { principal_n: number; bump_n: number; principal: number; bump: number; upsell: number }
...
async function main() {
    ...
    const { data: ventas, error: e1 } = await db
        ...
        .limit(20000)
    if (e1) { check('lectura de hotmart_ventas', false, e1.message); process.exit(1) }

    if ((ventas ?? []).length === 0) {
        // No es un fallo: la tabla es nueva y hasta que corra el backfill está
        // legítimamente vacía. Callarlo sí sería un problema.
        console.log('  · sin ventas en el rango: ejecuta `npm run backfill:hotmart` para poder comparar.')
    ...
    for (const v of ventas ?? []) {
        ...
        if (v.estado === 'reembolsada' || v.estado === 'chargeback') {
            ...
            if (v.tipo === 'principal') { a.principal_n++; a.principal += neto }
            else if (v.tipo === 'bump') { a.bump_n++; a.bump += neto }
            else if (v.tipo === 'upsell') { a.upsell += neto }
        }
        nuevo.set(k, a)
    }
    ...
    const { data: diarias, error: e2 } = await db
        ...
        .select('cliente_id, fecha, ventas_principal, ventas_bump, ventas_upsell, ventas_principal_count, ventas_bump_count')
        .gte('fecha', desde).lte('fecha', hasta)
        .limit(20000)
    if (e2) { check('lectura de metricas_diarias', false, e2.message); process.exit(1) }
    ...
    check('paridad exacta en los días SIN reembolsos',
