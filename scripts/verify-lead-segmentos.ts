...
import {
    segmentoIncluyeBucket, cuentaEnSegmento, bucketsAcumulados, indexarRawFields,
...
import {
    makeLeadSegMetric, isLeadSegMetric, parseLeadSegMetric, leadSegAlias,
    extractLeadSegAliases, leadSegLabel, isLeadFieldDim, isSheetToken, isFieldMetric,
    isAdditiveMetric, supportsPivot, esEtapaDeEmbudo,
    extractFieldMetricAliases, extractSheetAliases, extractOfflineFieldAliases,
    hasNonAttributableFilter, makeLeadFieldDim,
} from '../src/lib/report-utm/bi-metadata'
import {
    clavesDelDia, clavesDeCampo, claveSinRespuesta, claveSegmento,
    camposEnFormula, formulaUsaRespuestas, CLAVE_TOTAL_LEADS, PREFIJO_SEGMENTO,
...
function campo(over: Partial<LeadCampoDef> = {}): LeadCampoDef {
    return {
        id: 'campo-1', cliente_id: 'c', clave: 'rango_de_ingresos', nombre: 'Rango de ingresos',
        descripcion: null, claves_origen: ['cual_es_tu_rango_de_ingresos'],
        valores_map: MAP_PRESENTACION, valores_orden: BUCKETS,
...
function seg(over: Partial<LeadSegmentoDef> = {}): LeadSegmentoDef {
    return {
        id: 'seg-1', cliente_id: 'c', campo_id: 'campo-1', campo_clave: 'rango_de_ingresos',
        clave: 'desde_2m', nombre: 'Desde 2M', descripcion: null,
        operador: 'in', valores: ['Más de $2.000.000'], activo: true, orden: 0,
        ...over,
    }
}
...
check('un lead que no respondió TAMPOCO cuenta en `not_in`', !cuentaEnSegmento(excepto, c, sinRespuesta))
...
check('sin `valores_map`, el bucket es el valor normalizado y no coincide',
...
for (const crudo of ['entre_$1600000_y_$2000000', 'Entre $1.600.000 a $2.000.000']) {
    check(`la variante "${crudo}" cae en el segmento por su bucket`,
...
check('desde el penúltimo devuelve dos buckets',
    bucketsAcumulados(c, 'Entre $1.600.000 y $2.000.000').join('|') ===
    'Entre $1.600.000 y $2.000.000|Más de $2.000.000')
check('desde el primero devuelve todos', bucketsAcumulados(c, BUCKETS[0]).length === BUCKETS.length)
check('desde el último devuelve solo ese', bucketsAcumulados(c, BUCKETS[4]).length === 1)
check('un bucket que no está en el orden no produce acumulado',
...
check('sin `valores_orden` no se inventa un acumulado',
...
check('ida y vuelta del token', token === 'leadseg:desde_2m' && parseLeadSegMetric(token) === 'desde_2m')
check('reconoce el suyo', isLeadSegMetric(token))
check('no confunde con una dimensión de campo', !isLeadSegMetric(makeLeadFieldDim('rango_de_ingresos')))
...
check('la etiqueta lleva la pregunta delante',
    leadSegLabel(token, [{
        clave: 'desde_2m', nombre: 'Desde 2M', campo_clave: 'rango_de_ingresos',
        campo_nombre: 'Rango de ingresos', operador: 'in', valores: [], cobertura: 0,
...
const mezcla = 'spend / lseg__desde_2m + f_sum__edad + sf__calificados + sv__leads_20_100 + off__ventas + lf__rango__2m'
...
check('no captura sf__/sv__', !segs.some(s => s.clave.includes('calificados') || s.clave.includes('leads_20_100')))
check('no captura lf__', !segs.some(s => s.clave.includes('rango')))

check('extractFieldMetricAliases ignora lseg__', extractFieldMetricAliases('lseg__desde_2m').length === 0)
check('extractSheetAliases ignora lseg__', extractSheetAliases('lseg__desde_2m').length === 0)
check('extractOfflineFieldAliases ignora lseg__', extractOfflineFieldAliases('lseg__desde_2m').length === 0)
...
check('el selector del dashboard no lo esconde', !esSumandoDeSheet(claveSegmento({ clave: 'desde_2m' })))
...
check('un segmento como MÉTRICA no anula el gasto',
    !hasNonAttributableFilter({ }, undefined))
check('un filtro por campo de lead SÍ anula el gasto',
    hasNonAttributableFilter({ [makeLeadFieldDim('rango_de_ingresos')]: 'Más de $2.000.000' }, undefined))
...
const ds: LeadAnswerDatasetLite = {
    ...
    campanaIds: [null, 'c1'],
    campos: [{
        clave: 'rango_de_ingresos', nombre: 'Rango de ingresos',
        buckets: BUCKETS, claves_origen: ['cual_es_tu_rango_de_ingresos'],
        origen: 'catalogo', cobertura: 15,
    ...
    porFecha: { '2026-08-01': [[[0, 1, 10], [4, 1, 5]]] },
...
const suma = c0.reduce((n, k) => n + (claves[k.clave] ?? 0), 0) + claves[claveSinRespuesta(ds.campos[0])]
check('respuestas + sin_respuesta == utm_leads', suma === claves[CLAVE_TOTAL_LEADS], `${suma} vs ${claves[CLAVE_TOTAL_LEADS]}`)
check('el total es el del cubo', claves[CLAVE_TOTAL_LEADS] === 20)
check('el `(sin respuesta)` no sale negativo pese al solape',
...
check('una fórmula con solo un segmento carga su campo padre',
    camposEnFormula('meta_spend / lseg__desde_2m', ['rango_de_ingresos'], relacion)
        .join() === 'rango_de_ingresos')
check('una fórmula con lf__ sigue resolviendo por la clave del campo',
    camposEnFormula('lf__rango_de_ingresos__2m', ['rango_de_ingresos'], relacion)
        .join() === 'rango_de_ingresos')
check('no se duplica si la fórmula usa los dos',
    camposEnFormula('lf__rango_de_ingresos__2m + lseg__desde_2m', ['rango_de_ingresos'], relacion).length === 1)
check('una fórmula sin nada de esto no carga campos',
    camposEnFormula('meta_spend / meta_leads', ['rango_de_ingresos'], relacion).length === 0)

check('formulaUsaRespuestas reconoce un segmento', formulaUsaRespuestas('meta_spend / lseg__desde_2m'))
...
check('el prefijo del dashboard es el mismo alias que el del BI',
