...
import {
    evaluarCliente, ordenarPorGravedad, diasEntre,
    TOLERANCIA_DIAS, UMBRAL_CRUCE,
...
function fuente(p: Partial<SenalFuente> = {}): SenalFuente {
    return {
        id: 'ads', label: 'Anuncios (gasto)',
        ultimaFecha: HOY, estado: 'con_datos',
        toleranciaDias: TOLERANCIA_DIAS.ads,
        configurada: true, requierePuente: true,
        ...p,
    }
}
function senales(p: Partial<SenalesCliente> = {}): SenalesCliente {
    return {
        clienteId: 'c1', nombre: 'Cliente', tienePuente: true, hoy: HOY,
        fuentes: [], integraciones: [], pctLeadsCruzados: 90,
        sheetFilasAjenas: null,
        ...p,
    }
}
const titulos = (s: SenalesCliente) => evaluarCliente(s).hallazgos.map(h => h.titulo).join(' | ')
...
{
    const r = evaluarCliente(senales({
        tienePuente: false,
        fuentes: [
    ...
    check('un solo hallazgo, no uno por fuente', r.hallazgos.length === 1,
...
// ════════════════════════════════════════════════════════════
check('una fuente al día no genera nada',
    evaluarCliente(senales({ fuentes: [fuente()] })).hallazgos.length === 0)

check('una fuente NO configurada se ignora',
    evaluarCliente(senales({
        fuentes: [fuente({ configurada: false, estado: 'vacia', ultimaFecha: null })],
    })).hallazgos.length === 0)

check('configurada y vacía → crítico',
    titulos(senales({ fuentes: [fuente({ estado: 'vacia', ultimaFecha: null })] }))
        .includes('sin un solo dato'))
...
{
    const retrasada = (dias: number) => evaluarCliente(senales({
        fuentes: [fuente({
    ...
    check('al triple de la tolerancia → crítico', retrasada(TOLERANCIA_DIAS.ads * 3 + 1).gravedad === 'critico')
}
...
// `count` exacto agotaba el tiempo límite y el error se tragaba.
{
    ...
    check('no poder leer NO se reporta como fuente vacía',
    ...
    check('el texto dice que no se pudo comprobar',
...
// ════════════════════════════════════════════════════════════
{
    ...
    check('dice cuántos días lleva', r.hallazgos[0].titulo.includes('50 día(s)'),
        r.hallazgos[0].titulo)
    check('incluye el mensaje de la plataforma',
        r.hallazgos[0].titulo.includes('Páginas accesibles'))
}
check('una integración sana no genera nada',
    evaluarCliente(senales({
...
check('sin leads no se inventa un 0 %',
...
check('ninguna fila cruza → crítico',
    evaluarCliente(senales({ sheetFilasAjenas: { total: 500, sinCruce: 500 } })).gravedad === 'critico')
check('un Sheet propio no avisa',
...
check('con muy pocas filas no se pronuncia',
    evaluarCliente(senales({ sheetFilasAjenas: { total: 10, sinCruce: 10 } })).hallazgos.length === 0)
check('sin datos de Sheet no se pronuncia',
...
}
{
    ...
    check('dentro de un cliente, lo crítico va antes que el aviso',
...
// sin salida se deja de mirar a la semana.
{
    ...
    check(`los ${r.hallazgos.length} hallazgos traen una acción`,
