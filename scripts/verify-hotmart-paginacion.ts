...
const correr = () =>
  paginarHotmart<unknown>({
    ruta: RUTA,
    params: [
      ['start_date', '1786770000000'],
      ['max_results', '100'],
    ],
...
async function main() {
  ...
  check(
    'se clasifica como respuesta inválida',
    r.familia === 'respuesta_invalida',
    String(r.familia)
  );
  ...
  check(
    'el motivo incluye la query enviada',
    /start_date=1786770000000/.test(r.motivo ?? ''),
    r.motivo
  );
  ...
  check(
    'toda familia trae etiqueta legible',
    [400, 401, 403, 404, 422, 429, 500, 418].every(
      (s) => clasificarErrorHotmart(s).etiqueta.length > 10
    )
  );
  check(
    'un 400 y un 401 NO dicen lo mismo',
