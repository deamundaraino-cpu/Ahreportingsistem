...
async function main() {
  ...
  const { detectarCamposDeLeads, loadLeadCampos } =
    await import('../src/lib/report-utm/lead-campos-db');
  const { cargarRespuestasLead, campoSintetico } =
    await import('../src/lib/report-utm/lead-answers-db');
  ...
  // los leads de TODOS y agota el statement_timeout sin probar nada.
  {
    ...
    check(
      'una clave con comillas y un intento de inyección no rompen la consulta',
      !error,
      String((error as { message?: string } | null)?.message ?? '')
    );
  }
  const { count: sigueViva } = await rutm
    .from('lead_events')
  ...
  // funcionaba con los formularios de Meta, que ya mandan snake_case.
  {
    ...
    for (const m of muestras) {
      const { data: fila, error: e } = await rutm.rpc('norm_clave', { p: m });
      const enSql = e ? `<error: ${e.message}>` : String(fila ?? '');
      const enNode = normalizarClaveLead(m);
      check(
        `"${m.slice(0, 45)}" normaliza igual`,
        enSql === enNode,
  ...
  if (!refCliente) {
    check(
      'hay algún cliente con preguntas de opción en el rango',
      false,
      'ninguno: revisa el rango DESDE/HASTA del script'
    );
  } else {
    console.log(
      `  · cliente de referencia: ${refCliente.nombre.trim()} · claves: ${refClaves.join(', ')}`
    );
    ...
    const nueva = await rutm.rpc('bi_respuestas_por_dia', {
      p_cliente_id: refCliente.id,
      p_desde: bounds.gte,
      p_hasta: bounds.lt,
      p_claves_json: refClaves,
      p_limite: 60000,
    });
    ...
    const crudas = await fetchAllRows(() =>
      rutm
        .from('lead_events')
    ...
    const totalNode = (crudas as any[]).reduce(
      (s, r) => s + (bucketDeLead(campoRef, indexarRawFields(r.raw_fields)) ? 1 : 0),
      0
    );

    check(
      'la RPC cuenta exactamente los mismos leads que bucketDeLead',
    ...
    check(
      'cada respuesta cuadra valor a valor',
      desajustes.length === 0,
    ...
    const { count: leadsEnRango } = await rutm
      .from('lead_events')
      ...
      .lt('created_at', bounds.lt);
    check(
      'los leads contados no superan a los del rango',
    ...
    if (refClaves.length >= 2) {
      const porSeparado = await Promise.all(
        refClaves.map((k) =>
          rutm.rpc('bi_respuestas_por_dia', {
            p_cliente_id: refCliente!.id,
            p_desde: bounds.gte,
      ...
      check(
        'el COALESCE evita el doble conteo (juntas ≤ suma por separado)',
  ...
  for (const c of clientes) {
    const { claves, leads } = await detectarCamposDeLeads(rutm, c.id, {
      dateFrom: DESDE,
      dateTo: HASTA,
    });
    ...
    const { data, error } = await rutm.rpc('bi_respuestas_por_dia', {
      ...
      p_desde: bounds.gte,
      p_hasta: bounds.lt,
      p_claves_json: opcion.map((k) => k.clave_norm),
      p_limite: 60000,
    });
    ...
    console.log(
      `  · ${c.nombre.trim()}: ${leads} leads → ${filas} filas (factor ${factor.toFixed(2)}) en ${ms} ms`
    );
    ...
    check(
      `${c.nombre.trim()}: no se acerca al tope de la consulta`,
      filas < 60000 * 0.5,
      `${filas} filas`
    );
    ...
    if (leads >= 1000) {
      check(
        `${c.nombre.trim()}: el plegado sigue comprimiendo`,
        factor <= 0.5,
  ...
  check(
    'un id público inventado devuelve null, no lanza',
  ...
  if (refCliente) {
    ...
    check(
      'el índice 0 del diccionario es siempre (sin campaña)',
      ds.campanas[0] === '(sin campaña)',
      ds.campanas[0]
    );
    check(
      'el diccionario de ids es paralelo al de nombres',
    ...
    check(
      'todos los días están dentro del rango',
    ...
    check(
      'cada día trae una entrada por campo',
      fechas.every((f) => ds.porFecha[f].length === ds.campos.length)
    );
    check(
      'ningún triplete apunta fuera del diccionario',
      fechas.every((f) =>
        ds.porFecha[f].every((porCampo) =>
          porCampo.every(([b, c]) => c >= 0 && c < ds.campanas.length && b >= 0)
        )
      )
    );
    check(
      'ningún triplete apunta a un bucket inexistente',
      fechas.every((f) =>
        ds.porFecha[f].every((porCampo, i) =>
          porCampo.every(([b]) => b < ds.campos[i].buckets.length)
        )
      )
    );

    const totalCubo = fechas.reduce(
      (s, f) =>
        s + ds.porFecha[f].reduce((s2, pc) => s2 + pc.reduce((s3, [, , n]) => s3 + n, 0), 0),
      0
    );
    const coberturaDeclarada = ds.campos.reduce((s, c) => s + c.cobertura, 0);
    check(
      'la cobertura declarada cuadra con la suma del cubo',
      totalCubo === coberturaDeclarada,
      `cubo=${totalCubo} cobertura=${coberturaDeclarada}`
    );
  }
  ...
  for (const c of clientes) {
    ...
    const totalCubo = Object.values(ds.totalesPorFecha).reduce(
      (s, pares) => s + pares.reduce((s2, [, n]) => s2 + n, 0),
      0
    );
    ...
    const { count } = await rutm
      ...
      .lt('created_at', bounds.lt);
    check(
      `${c.nombre.trim()}: el total diario cuadra con los leads del rango`,
      totalCubo === (count ?? 0),
      `cubo=${totalCubo} tabla=${count}`
    );
  }
  ...
  // provoca, para que nadie "simplifique" la paginación creyéndola de más.
  {
    ...
    if (grande) {
      ...
      if (totalReal > 1000) {
        check(
          `${grande.nombre.trim()}: una sola página se queda corta (por eso se pagina)`,
        ...
        check(
          `${grande.nombre.trim()}: paginando se recuperan todos los días`,
          dias > 0 && !ds.incompleto,
          `dias=${dias} incompleto=${ds.incompleto}`
        );
      } else {
  ...
  if (refCliente) {
    ...
    check(
      'en ningún día responden más leads de los que se registraron',
      desbordes === 0,
      `${desbordes} días desbordados`
    );

    check('el cubo trae totales diarios', Object.keys(ds.totalesPorFecha).length > 0);
    check(
      'todo día con respuestas tiene su total',
  ...
  // comprobación existe para enterarnos si algún día empieza a ocurrir.
  {
    ...
    for (const c of clientes) {
      ...
      if (huerfanos.length > 0) {
        ...
        console.log(
          `  · ${c.nombre.trim()}: ${huerfanos.length} días con leads y sin fila de métricas`
        );
      }
    }
    check(
      'ningún día con contactos se queda sin fila de métricas',
      diasHuerfanos === 0,
