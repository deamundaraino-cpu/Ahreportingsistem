...
async function inventario() {
    ...
    const { data: diarios } = await db.from('leads_diarios')
        .select('client_id, date').order('date', { ascending: false }).limit(5000)
    ...
    for (const tabla of ['layouts_reporte', 'clientes_layouts', 'cliente_tabs']) {
        ...
        const usados = (data ?? []).filter((r: any) =>
            patron.test(JSON.stringify([r.columnas, r.tarjetas, r.graficos, r.custom_metrics, r.ranking_tables]))
        )
...
async function crearCampos() {
    ...
    for (const c of clientes) {
        ...
        if (!qualityField) {
            console.log('  Sin campo de calidad configurado: sus leads ya se cuentan como offline_leads.')
            continue
        }

        const columna = sanitizarColumna(qualityField)
        const { data: cliente } = await db.from('clientes')
            .select('config_api').eq('id', c.id).maybeSingle()
        const sheetMigrado = ((cliente as any)?.config_api?.google_sheets_conversiones ?? [])
            .find((s: any) => s?.migrado_de === 'google_sheets')
        ...
        if (yaCampo) {
            ...
            const { data, error } = await db.from('sheet_campos').insert({
                cliente_id: c.id,
                clave: CAMPO_CLAVE,
...
async function cuadre() {
    ...
    for (const c of clientes) {
        ...
        const { data: legacy } = await db.from('leads_diarios')
            .select('date, leads_totales, leads_calificados')
            .eq('client_id', c.id).gte('date', desde).order('date')

        const { data: nuevo } = await db.from('sheet_campo_valores_diarios')
            .select('fecha, valor, filas')
            .eq('campo_id', campo.id).gte('fecha', desde)
