...
check(
  `${todosIds.length} campos registrados en ${STATIC_SOURCES.length} fuentes`,
  todosIds.length > 0
);

const noCanonicos = todosIds.filter((id) => !isCanonicalId(id));
check(
  'todos los ids tienen la forma <fuente>.<campo>',
...
check(
  'toda medida tiene texto de ayuda propio',
  sinHelp.length === 0,
...
check(
  'todo weighted_avg declara su peso',
  sinPeso.length === 0,
  sinPeso.map((m) => m.id).join(', ')
);
...
const derivadaSinHojas = REG.measures().filter(
  (m) => m.formula && leafDeps(REG, m.id).length === 0
);
check(
  'toda derivada llega a medidas físicas',
  derivadaSinHojas.length === 0,
...
check(
  'toda métrica de METRIC_META tiene id canónico',
...
check(
  'todo id canónico existe en el registro',
  traduceANada.length === 0,
...
check(
  'toda dimensión de DIMENSION_META tiene id canónico',
  dimSinTraducir.length === 0,
  dimSinTraducir.join(', ')
);
...
check(
  'todo id de dimensión existe en el registro',
  dimTraduceANada.length === 0,
  dimTraduceANada.map(([d, id]) => `${d} → ${id}`).join(', ')
);

// Idempotencia: reejecutar la migración tiene que ser un no-op.
const noIdempotente = [...legacyMetrics.map((m) => LEGACY_MEASURE_IDS[m]!)].filter(
  (id) => migrateMeasureId(id) !== id
);
...
check(
  'migrateDimensionId es idempotente',
  dimNoIdempotente.length === 0,
  dimNoIdempotente.join(', ')
);

check(
  'leads_total (borrada en la mig. 045) sigue resolviendo',
  migrateMeasureId('leads_total') === 'leads.count'
);
check(
  'el alias histórico dimension:campaign resuelve',
  migrateDimensionId('campaign') === migrateDimensionId('utm_campaign')
);
check(
  'un token desconocido se devuelve tal cual (no se descarta)',
...
check(
  'leadfield:<k>',
  migrateDynamicToken('leadfield:rango_ingresos') === 'leads.campo__rango_ingresos'
);
check(
  'offfield:<tipo>:<k>',
  migrateDynamicToken('offfield:currency:margen') === 'offline.custom__margen'
);
...
check(
  'un token dinámico traducido es canónico',
...
check(
  'los 72 desgloses se DERIVAN correctamente',
  difBreakdown.length === 0,
  difBreakdown.join(' | ')
);
...
// una fuente falle aquí en vez de fallar en un informe de un cliente.
{
  ...
  check(
    'ninguna medida existe solo en el registro',
...
const aditivasDerivadas = aLegacy(
  REG.measures()
    .filter(isAdditive)
    .map((m) => m.id)
);
...
check(
  'ADDITIVE_METRICS se deriva igual',
  aditivasDerivadas.size === aditivasActuales.size &&
    [...aditivasActuales].every((m) => aditivasDerivadas.has(m)),
  difSets(aditivasDerivadas, aditivasActuales)
);

const pivotDerivadas = aLegacy(
  REG.measures()
    .filter((m) => isPivotable(REG, m.id))
    .map((m) => m.id)
);
check(
  'PIVOT_METRICS se deriva igual',
  pivotDerivadas.size === PIVOT_METRICS.length && PIVOT_METRICS.every((m) => pivotDerivadas.has(m)),
  difSets(pivotDerivadas, new Set(PIVOT_METRICS as string[]))
);

const funnelDerivadas = aLegacy(funnelStages(REG).map((m) => m.id));
check(
  'FUNNEL_STAGE_METRICS se deriva igual (mismo conjunto)',
...
check(
  'FUNNEL_STAGE_METRICS conserva el ORDEN del embudo',
...
check(
  'RECOMMENDED_METRICS se deriva igual',
...
const bajarMejorDerivadas = aLegacy(
  REG.measures()
    .filter((m) => isLowerBetterField(REG, m.id))
    .map((m) => m.id)
);
check(
  'LOWER_IS_BETTER se deriva igual',
...
check(
  `los ${Object.keys(METRIC_GLOSSARY).length} textos del glosario se conservan`,
  glosarioMal.length === 0,
  glosarioMal.join(', ')
);
...
check(
  'AD_SCALAR_METRICS → «cuenta» (y tiktok_conversions → «ads»)',
  escalaresMal.length === 0,
  escalaresMal.join(', ')
);
...
check(
  'leads.count → solo la fuente leads',
  JSON.stringify(nombresDe(['leads.count'])) === '["leads"]'
);
check('ads.spend → solo la fuente ads', JSON.stringify(nombresDe(['ads.spend'])) === '["ads"]');
// El caso que antes se codificaba REPITIENDO `cpl` en needsLeads y en needsAds.
check(
  'leads.cpl → ads + leads (deducido de la fórmula)',
  JSON.stringify(nombresDe(['leads.cpl'])) === '["ads","leads"]'
);
check('sales.roas → ads + sales', JSON.stringify(nombresDe(['sales.roas'])) === '["ads","sales"]');
check(
  'cuenta.hotmart_roas → ads + cuenta',
  JSON.stringify(nombresDe(['cuenta.hotmart_roas'])) === '["ads","cuenta"]'
);
check(
  'leads.conversion_rate → leads + sales',
  JSON.stringify(nombresDe(['leads.conversion_rate'])) === '["leads","sales"]'
);
// Un promedio ponderado necesita traer también su peso.
check(
  'cuenta.ga_bounce_rate arrastra su peso (ga_sessions)',
  sourcesFor(REG, ['cuenta.ga_bounce_rate']).length === 1 && leafDepsIncluyePeso()
);
...
check(
  'una fórmula NO arrastra fuentes por palabras sueltas',
...
check('gasto × campaña SÍ cruza', fieldCrossesDimension(REG, 'ads.spend', 'leads.campaign'));
check('gasto × fecha SÍ cruza', fieldCrossesDimension(REG, 'ads.spend', 'leads.date'));
check(
  'gasto × país NO cruza (metricas_diarias no tiene UTM)',
  !fieldCrossesDimension(REG, 'ads.spend', 'leads.ip_country')
);
check('gasto × formulario NO cruza', !fieldCrossesDimension(REG, 'ads.spend', 'leads.form_name'));
check('leads × país SÍ cruza', fieldCrossesDimension(REG, 'leads.count', 'leads.ip_country'));
check(
  'GA4 × campaña NO cruza',
  !fieldCrossesDimension(REG, 'cuenta.ga_sessions', 'leads.campaign')
);
check('GA4 × fecha SÍ cruza', fieldCrossesDimension(REG, 'cuenta.ga_sessions', 'leads.date'));
check(
  'suscripciones NO cruzan ni por fecha (son una foto)',
  !fieldCrossesDimension(REG, 'subs.active', 'leads.date')
);
check('suscripciones SÍ valen en el Total', fieldCrossesDimension(REG, 'subs.active', 'none'));
check(
  'CPL hereda el límite del gasto (× país NO cruza)',
  !fieldCrossesDimension(REG, 'leads.cpl', 'leads.ip_country')
);
check('CPL × campaña SÍ cruza', fieldCrossesDimension(REG, 'leads.cpl', 'leads.campaign'));
check(
  'Conv. Rate × país SÍ cruza (leads y ventas son de grano fila)',
  fieldCrossesDimension(REG, 'leads.conversion_rate', 'leads.ip_country')
);
check(
  'un campo de Sheet NO cruza con el gasto',
...
check(
  'las tres métricas de "leads" se declaran incompatibles entre sí',
  solapes.length === 3,
  JSON.stringify(solapes)
);
check(
  'métricas no solapadas no generan aviso',
...
check(
  'toda fuente con fecha declara su tipo',
  sinTipoFecha.length === 0,
  sinTipoFecha.map((s) => s.id).join(', ')
);

const snapshotConFecha = STATIC_SOURCES.filter((s) => s.grainKind === 'snapshot' && s.dateColumn);
check(
  'un snapshot no declara columna de rango',
  snapshotConFecha.length === 0,
...
check(
  'leads y ventas cuelgan del cliente de report_utm',
  JSON.stringify(porReportUtm.sort()) === '["leads","sales"]',
  porReportUtm.join(', ')
);

// Solo el grano de fila puede ser eje de tabla dinámica.
const pivotNoFila = REG.measures().filter(
  (m) => isPivotable(REG, m.id) && REG.sourceOf(m.id)?.grainKind !== 'row'
);
check(
  'solo el grano «row» es pivotable',
  pivotNoFila.length === 0,
