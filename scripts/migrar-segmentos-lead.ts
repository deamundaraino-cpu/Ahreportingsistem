...
const goodpropRango: CampoPlan = {
    ...
    descripcion: 'Respuesta de rango de ingresos, unificando el formulario web y el de Meta Lead Ads.',
...
const goodprop: PlanCliente = {
    ...
    campos: [{
        ...
        segmentos: [
            {
                ...
                descripcion: 'Leads que contestaron la pregunta de ingresos. No es el total de contactos.',
                valores: GP_ORDEN,
            },
            { clave: 'ingresos_desde_1_3m', nombre: 'Desde 1.3M', valores: [GP.de13a16, GP.de16a2, GP.de2a4, GP.mas4] },
            { clave: 'ingresos_desde_1_6m', nombre: 'Desde 1.6M', valores: [GP.de16a2, GP.de2a4, GP.mas4] },
        ...
        campo: {
            ...
            descripcion: 'Cuándo quiere invertir o comprar. 2.448 leads y ningún campo configurado hasta ahora.',
        ...
        segmentos: [
            { clave: 'plazo_hasta_3_meses', nombre: 'Hasta 3 meses', valores: ['Cuanto antes', '1 a 3 meses'] },
            { clave: 'plazo_hasta_6_meses', nombre: 'Hasta 6 meses', valores: ['Cuanto antes', '1 a 3 meses', '3 a 6 meses'] },
        ],
    }],
...
const cris: PlanCliente = {
    ...
    campos: [{
        ...
        segmentos: [
            { clave: 'propiedades_web_desde_2', nombre: 'Desde 2 propiedades', valores: ['2 a 3', 'Más de 4'] },
        ...
        campo: {
            ...
            descripcion: 'Escala del formulario de Meta: 3-4 / 5-8 / más de 8. NO es la misma que la web.',
        ...
        segmentos: [
            { clave: 'propiedades_meta_desde_5', nombre: 'Desde 5 propiedades', valores: ['5 a 8', 'Más de 8'] },
            { clave: 'propiedades_meta_desde_9', nombre: 'Más de 8 propiedades', valores: ['Más de 8'] },
        ],
    }],
...
const somosRentable: PlanCliente = {
    ...
    campos: [{
        ...
        segmentos: [
            { clave: 'invertir_desde_15m', nombre: 'Desde $15M', valores: ['Entre $15M y $30M', 'Entre $30M y $50M', 'Más de $50M'] },
            { clave: 'invertir_desde_30m', nombre: 'Desde $30M', valores: ['Entre $30M y $50M', 'Más de $50M'] },
    ...
    campos: [{
        campo: {
            ...
            valores_map: mapa([
                ...
                [['Entre $2.000.000 a $3.000.000', 'Entre $2.000.000 – $3.000.000'], 'Entre $2.000.000 y $3.000.000'],
                [['Más de $3.000.000'], 'Más de $3.000.000'],
            ]),
            valores_orden: ['Menos de $2.000.000', 'Entre $2.000.000 y $3.000.000', 'Más de $3.000.000'],
            sin_mapear: 'otros',
        },
        segmentos: [
            { clave: 'renta_desde_2m', nombre: 'Desde $2M', valores: ['Entre $2.000.000 y $3.000.000', 'Más de $3.000.000'] },
        ...
        campo: {
            ...
            valores_map: mapa([
                ...
                [['Entre $12.000.000 a $15.000.000', 'Entre $12.000.000 – $15.000.000'], 'Entre $12.000.000 y $15.000.000'],
                [['Más de $15.000.000'], 'Más de $15.000.000'],
            ]),
            valores_orden: ['Menos de $12.000.000', 'Entre $12.000.000 y $15.000.000', 'Más de $15.000.000'],
            sin_mapear: 'otros',
        },
        segmentos: [
            { clave: 'pie_desde_12m', nombre: 'Desde $12M', valores: ['Entre $12.000.000 y $15.000.000', 'Más de $15.000.000'] },
...
const inspiraCampos: { clave: string; nombre: string; claveOrigen: string; valores: string[] }[] = [
    {
        clave: 'ubicacion', nombre: 'Where are you based?', claveOrigen: 'where_are_you_based',
        valores: ['Mexico City', 'Los Angeles', 'Baja California', 'San Francisco / Bay Area', 'Monterrey / Guadalajara', 'Other'],
    },
    {
        clave: 'prioridad', nombre: 'What matters most right now?', claveOrigen: 'what_matters_most_to_you_right_now',
        valores: ['Finding my place in Baja before it peaks', 'Just exploring, no rush', "The community — who's already in", 'Understanding how ownership works for non-Mexicans'],
    },
    {
        clave: 'canal', nombre: 'How did you hear about us?', claveOrigen: 'how_did_you_hear_about_munity_iii',
        valores: ['Instagram / Facebook', 'LinkedIn', 'Search / Online', 'A friend / colleague mentioned it', 'Other'],
    },
    {
        clave: 'perfil', nombre: 'How would you describe yourself?', claveOrigen: 'how_would_you_best_describe_yourself',
        valores: ['Creative / Director / Artist', 'Executive (C-level / Director)', 'Founder / Co-Founder', 'Investor / Family Office', 'Other'],
    },
]
...
const COLS_LAYOUT = [
    'columnas', 'tarjetas', 'graficos', 'text_blocks', 'custom_metrics',
    'tablas', 'ranking_tables', 'lead_answer_blocks',
]
...
async function reescribirLayouts(soloInforme: boolean): Promise<any[]> {
    ...
    for (const tabla of TABLAS_LAYOUT) {
        ...
        for (const fila of (data ?? []) as any[]) {
            ...
            console.log(`  ${soloInforme ? 'se reescribiría' : 'reescrito'} ${tabla} ${fila.id}` +
...
async function main() {
    ...
    const copia: any = { creado: new Date().toISOString(), campos: [], segmentosCreados: [], layouts: [] }
    const aRetirar: { cliente: string; clave: string }[] = []

    for (const plan of PLANES) {
        console.log(`\n──────── ${plan.nombre} ────────`)
        const { data: existentes } = await db
            .from('lead_campos').select('*').eq('cliente_id', plan.clienteId)
        ...
        for (const { campo, segmentos } of plan.campos) {
            ...
            console.log(`     ${previo ? 'existe' : 'NUEVO'} · ${Object.keys(campo.valores_map).length} valores → ${buckets.length} buckets`)
    ...
    for (const plan of PLANES) {
        ...
        for (const [i, { campo, segmentos }] of plan.campos.entries()) {
            ...
            if (previo) {
                ...
                if (error) { console.log(`✗ ${campo.clave}: ${error.message}`); continue }
                campoId = previo.id
                console.log(`✓ actualizado leadfield:${campo.clave}`)
            } else {
                const { data, error } = await db.from('lead_campos')
                    .insert({ ...payload, clave: campo.clave }).select('id').single()
                if (error || !data) { console.log(`✗ ${campo.clave}: ${error?.message}`); continue }
            ...
            for (const [j, s] of segmentos.entries()) {
                ...
                const { data, error } = await db.from('lead_campo_segmentos').upsert({
                    cliente_id: plan.clienteId,
                    campo_id: campoId,
                    clave,
                    nombre: s.nombre,
                    ...
                    activo: true,
                    orden: j,
                }, { onConflict: 'cliente_id,clave' }).select('id').single()
                if (error) { console.log(`  ✗ lseg__${clave}: ${error.message}`); continue }
...
async function revertir(ruta: string) {
    ...
    for (const l of copia.layouts ?? []) {
        ...
        console.log(error ? `✗ ${l.tabla} ${l.id}: ${error.message}` : `✓ ${l.tabla} ${l.id} restaurado`)
    }
