...
async function main() {
  ...
  // niveles triplica el gasto y el número parece plausible.
  {
    const { error } = await db.rpc('ads_daily_resumen', {
      p_cliente_id: clientes[0]?.id,
      p_nivel: null,
      p_desde: DESDE,
      p_hasta: HASTA,
    });
    check(
      'p_nivel = NULL da error explícito, no un total triplicado',
      Boolean(error),
      error ? '' : 'la llamada devolvió datos'
    );
  }
  {
    const { error } = await db.rpc('ads_daily_resumen', {
  ...
  for (const c of clientes) {
    ...
    if (error) {
      check(`${c.nombre.trim()} — la RPC responde`, false, error.message);
      continue;
    }

    const rpc = (rpcRaw ?? []) as Array<{ fecha: string; spend: number }>;
    const dailySpend = rpc.reduce((s, r) => s + Number(r.spend ?? 0), 0);
    const diasDaily = new Set(
      rpc.filter((r) => Number(r.spend) > 0).map((r) => String(r.fecha).slice(0, 10))
    );
    ...
    if (!ok) {
      ...
      check(
        `${c.nombre.trim()} — el gasto coincide`,
        false,
  ...
  // elección se hace por variable de entorno al arrancar.
  {
    ...
    check(
      `el motor da el mismo gasto por campaña por los dos caminos (${claves.size} campaña(s))`,
      distintas.length === 0,
      distintas.slice(0, 4).join(' | ')
    );
  }
  ...
  for (const c of clientes.slice(0, 3)) {
    ...
    const a = ((porDia ?? []) as Array<{ spend: number }>).reduce(
      (s, r) => s + Number(r.spend ?? 0),
      0
    );
    const b = ((agregado ?? []) as Array<{ spend: number }>).reduce(
      (s, r) => s + Number(r.spend ?? 0),
      0
    );
    if (a === 0 && b === 0) continue;
    check(
      `${c.nombre.trim()} — mismo total agregado o por día`,
      Math.abs(a - b) < 0.01,
