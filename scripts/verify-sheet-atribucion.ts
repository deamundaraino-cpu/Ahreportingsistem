...
check(
  'quita el prefijo de campaña',
  limpiarIdSheet('c:120235587549050178') === '120235587549050178'
);
check(
  'quita el prefijo de conjunto',
  limpiarIdSheet('as:120247665005500178') === '120247665005500178'
);
check(
  'quita el prefijo de anuncio',
  limpiarIdSheet('ag:120247737093830178') === '120247737093830178'
);
...
check(
  'un texto con dos puntos y espacio se respeta',
  limpiarIdSheet('nota: revisar') === 'nota: revisar'
);
...
check(
  'un prefijo conocido sí se recorta aunque el id sea corto',
  limpiarIdSheet('c:999') === '999'
);
check(
  'el teléfono conserva su formato tras quitar el prefijo',
...
check(
  'encuentra los tres ids y los limpia',
...
check(
  'una fila de CRM sin identidad publicitaria no es atribuible',
...
check(
  'gana el primer alias con valor',
...
check(
  'ningún alias está duplicado entre partes',
  (() => {
...
check(
  'utm_id toma el id de anuncio cuando existe',
  utmDeFilaSheet(filaMeta).utm_id === '120247737093830178'
);
check(
  'sin anuncio, cae al conjunto',
  utmDeFilaSheet({ adset_id: 'as:222', campaign_id: 'c:111' }).utm_id === '222'
);
check(
  'sin anuncio ni conjunto, cae a la campaña',
  utmDeFilaSheet({ campaign_id: 'c:111' }).utm_id === '111'
);
check('sin ningún id, utm_id es null', utmDeFilaSheet({ campaign_name: 'Promo' }).utm_id === null);
...
const campoMonto: SheetCampoDef = {
  ...
  nombre: 'Monto',
  agregacion: 'sum',
  origenes: [{ sheet_id: '*', tab_name: '*', columnas: ['monto_de_inversion'] }],
};
...
const filas: SheetRawRow[] = [
  {
    ...
    valores: {
      campaign_id: 'c:111',
      ad_id: 'ag:aaa',
      cual_es_tu_rango_de_ingresos: 'entre_1_y_2',
      monto_de_inversion: '1.000,50',
    },
  },
  {
    sheet_id: 's1',
    ...
    valores: {
      campaign_id: 'c:222',
      ad_id: 'ag:bbb',
      cual_es_tu_rango_de_ingresos: 'mas_de_4',
      monto_de_inversion: '2.000',
    },
  },
  {
    sheet_id: 's1',
...
for (const campo of campos) {
  ...
  const totalDiario = agregarDiarios(
    diarios.filter((d) => d.campo_id === campo.id),
    agg
  );
  const totalFila = agregarDiarios(
    [...porFila.values()].filter((d) => d.campo_id === campo.id),
    agg
  );
  check(
    `«${campo.nombre}» (${agg}): el total por fila coincide con el diario`,
    totalDiario === totalFila,
    `diario=${totalDiario} fila=${totalFila}`
  );
}
...
const porCampana = acumularPorFila(
  filas,
  (f) => atribucionDeFila(f.valores).campaign_id || '(sin campaña)'
);
for (const campo of campos) {
  const agg = campo.agregacion;
  const total = agregarDiarios(
    [...porFila.values()].filter((d) => d.campo_id === campo.id),
    agg
  );
  ...
  for (const dim of dims) {
    suma += agregarDiarios(
      [...porCampana.values()].filter((d) => d.campo_id === campo.id && d.dim === dim),
      agg
    );
  }
  check(
    `«${campo.nombre}» (${agg}): las campañas suman el total`,
    Math.abs(total - suma) < 1e-9,
    `total=${total} suma=${suma}`
  );
}

check(
  'la fila sin atribución cae en su propio cubo, no se pierde',
...
const accAvg = (() => {
  const rows = [...acumularPorFila(filas, () => 'total').values()].filter(
    (d) => d.campo_id === campoMonto.id
  );
  return rows.map((r) => ({ ...r, campo_id: campoAvg.id }));
})();
check(
  'el promedio sale de Σsuma/Σn (no del promedio de promedios)',
...
check(
  'declara los ejes de conjunto y anuncio',
  Boolean(sheet?.joinAxes.includes('adset') && sheet?.joinAxes.includes('ad'))
);
check(
  'conserva sus ejes propios (fecha y valor)',
...
check(
  'el gasto sigue sin cruzar por el valor de un campo de Sheet',
...
check(
  'un campo de Sheet no cruza con las columnas de lead',
