...
const meta: QueryDiagnostics = {
  ...
  skipped: [
    {
      sourceId: 'ads',
      sourceLabel: 'Anuncios (Meta / TikTok)',
      reason: { kind: 'no_public_link' },
    },
  ],
  hasPublicLink: false,
};
...
check('el texto nombra la fuente', na!.text.includes('Anuncios (Meta / TikTok)'), na!.text);
check('el texto dice dónde arreglarlo', na!.text.includes('ficha del cliente'), na!.text);

// El caso completo: el motor devolvió 0 (porque no pudo leer la fuente) pero el
// widget NO debe mostrar ese 0.
check('un 0 con motivo se muestra como hueco', readValue({ spend: 0 }, 'spend', na) === null);
check('sin motivo, ese mismo 0 se muestra como 0', readValue({ spend: 0 }, 'spend', null) === 0);
