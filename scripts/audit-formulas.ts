...
const KNOWN = new Set<string>([
    ...
    'dias_totales', 'dias_transcurridos', 'dias_restantes',
    'presupuesto_objetivo', 'presupuesto_diario_ideal', 'presupuesto_gastado_total',
    'presupuesto_restante', 'presupuesto_diario_sugerido',
])
...
type Finding = {
    cliente: string; contenedor: string; elemento: string; formula: string
    constantes: number[]; desconocidos: string[]; vacia: boolean; sugerencia: string | null
}

function analizarFormula(formula: string | null | undefined): Omit<Finding, 'cliente' | 'contenedor' | 'elemento'> | null {
    const f = (formula ?? '').trim()
    if (f === '') return { formula: '', constantes: [], desconocidos: [], vacia: true, sugerencia: null }
    ...
    return {
        formula: f, constantes: [...new Set(nums)], desconocidos, vacia: false,
...
function elementos(arr: unknown): { nombre: string; formula: string }[] {
    ...
    for (const el of arr as Record<string, unknown>[]) {
        ...
        if (typeof el?.targetFormula === 'string') out.push({ nombre: `${nombre} · target`, formula: el.targetFormula })
        // custom_metrics: { key: formula } o [{ key/name, formula }]
        if (el && typeof el === 'object' && typeof el.formula !== 'string' && typeof el.expression === 'string') {
...
async function main() {
    ...
    const pushFrom = (cliente: string, contenedor: string, source: Record<string, unknown>) => {
        for (const key of ARRAY_KEYS) {
            for (const { nombre, formula } of elementos(source?.[key])) {
                ...
                if (a) findings.push({ cliente, contenedor: `${contenedor}.${key}`, elemento: nombre, ...a })
            }
        }
    }
    ...
    const nombreCliente = new Map((clientes || []).map((c: { id: string; nombre: string }) => [c.id, c.nombre]))
    const { data: tabs } = await db.from('cliente_tabs').select('cliente_id, nombre, tarjetas, columnas, graficos, ranking_tables, custom_metrics')
    for (const t of tabs || []) pushFrom(nombreCliente.get(t.cliente_id) || '(cliente?)', `tab:${t.nombre}`, t)

    const { data: cl } = await db.from('clientes_layouts').select('cliente_id, nombre, tarjetas, columnas, graficos, ranking_tables, custom_metrics')
    for (const l of cl || []) pushFrom(nombreCliente.get(l.cliente_id) || '(cliente?)', `layout_cliente:${l.nombre}`, l)

    const { data: lr } = await db.from('layouts_reporte').select('nombre, tarjetas, columnas, graficos, ranking_tables, custom_metrics')
    for (const l of lr || []) pushFrom('(plantilla global)', `layout:${l.nombre}`, l)

    const { data: tt } = await db.from('tab_templates').select('nombre, tarjetas, columnas, graficos, ranking_tables, custom_metrics')
    ...
    for (const f of conConst) { const k = f.cliente; (porCliente.get(k) ?? porCliente.set(k, []).get(k)!).push(f) }
    for (const [cliente, fs] of porCliente) {
        console.log(`\n  ▸ ${cliente}`)
        for (const f of fs) {
            console.log(`    [${f.contenedor}] "${f.elemento}"`)
            console.log(`        fórmula: ${f.formula}`)
            console.log(`        constantes: ${f.constantes.join(', ')}${f.sugerencia ? `  → ${f.sugerencia}` : ''}`)
        }
    }
