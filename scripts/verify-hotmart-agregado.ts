...
async function main() {
  ...
  check(
    'principal',
    cuatro.principal === 10 && cuatro.principal_count === 1 && cuatro.principal_bruto === 20
  );
  ...
  check(
    'downsell',
    cuatro.downsell === 4 && cuatro.downsell_count === 1 && cuatro.downsell_bruto === 6
  );
  ...
  const extras = await agregar([
    venta({
      tipo: 'sin_clasificar',
      producto_nombre: 'Ebook suelto',
      neto_productor_usd: 2,
      bruto_usd: 4,
  ...
  check(
    'un producto sin nombre no se pierde',
    extras.extras.some((e) => e.product_name === '(Sin nombre)')
  );
  ...
  check(
    'suma solo lo suyo',
    porTab.by_tab[TAB].principal.count === 1 && porTab.by_tab[TAB].bump.count === 1
  );
  check(
    'el total incluye lo que no tiene pestaña',
    porTab.principal === 17 && porTab.principal_count === 2
  );
  ...
  check(
    'el precio configurado manda sobre el bruto de la API',
    conPrecio.principal_bruto === 97,
    String(conPrecio.principal_bruto)
  );
