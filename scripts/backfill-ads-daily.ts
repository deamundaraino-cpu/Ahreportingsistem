...
async function main() {
    ...
    for (;;) {
        let q = db.from('metricas_diarias')
            .select('cliente_id,fecha,meta_campaigns,meta_adsets,meta_ads,tiktok_campaigns,tiktok_adgroups,tiktok_ads')
            .gte('fecha', desde)
...
async function verificar(db: any, hoy: string) {
    ...
    for (const f of filas) {
        ...
        const { data: reales, error: e2 } = await db.from('ads_daily')
        ...
        for (const esp of esperadas) {
            ...
            if (Math.abs(real.spend - esp.spend) > 0.01) {
                ...
                if (detalles.length < 10) {
                    detalles.push(`GASTO ${f.fecha} ${k}: esperado ${money(esp.spend)} vs ${money(real.spend)}`)
                }
            }
        }
