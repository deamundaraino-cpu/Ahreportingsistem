...
function ds(porFecha: LeadAnswerDatasetLite['porFecha']): LeadAnswerDatasetLite {
    return {
        ...
        campanaIds: IDS,
        campos: [{
            clave: 'rango_ingresos', nombre: 'Rango de ingresos', buckets: BUCKETS,
            claves_origen: ['cual_es_tu_rango_de_ingresos', 'cual_es_tu_rango_aproximado_de_ingresos'],
            origen: 'catalogo', cobertura: 0,
        }],
        porFecha,
        incompleto: false,
    }
...
const BASE = ds({
    '2026-07-01': [[[0, 1, 10], [1, 1, 40], [1, 2, 20], [2, 2, 30]]],
...
const DEF: LeadAnswerBlockDef = {
    ...
    topN: 12, agruparResto: true, showDelta: false,
}
...
check('los leads sin campaña identificada SÍ cuentan sin filtro',
    (todo.buckets.find(b => b.label === 'Más de $5M')?.leads ?? 0) === 8)
check('los porcentajes suman 100',
    Math.abs(todo.buckets.reduce((s, b) => s + b.pct, 0) - 100) < 0.05,
    String(todo.buckets.reduce((s, b) => s + b.pct, 0)))
check('el orden es el del catálogo, no el de frecuencia',
...
check('declara los leads sin campaña que quedan fuera', soloBogota.sinCruce === 8,
    `sinCruce=${soloBogota.sinCruce}`)
check('con filtro, el bucket que solo tenía leads sin campaña queda en 0',
    (soloBogota.buckets.find(b => b.label === 'Más de $5M')?.leads ?? -1) === 0)
check('los porcentajes vuelven a sumar 100 sobre el nuevo total',
...
check('un filtro que roza la etiqueta sintética NO arrastra los leads sin campaña',
...
check('…y tampoco entra por un filtro de bloque',
    !campanasPermitidas(BASE, '', { type: 'keyword', operator: 'includes', value: 'camp' }, []).has(0))

// ════════════════════════════════════════════════════════════════
seccion('Recorte de fechas de la pestaña')

check('fechasEnRango recorta por comparación de strings',
    fechasEnRango(BASE.porFecha, '2026-07-01', '2026-07-31').sort().join(',') === '2026-07-01,2026-07-15')

const soloJulio = agregarRespuestas(BASE, DEF, '', [], {
    fechaInicio: '2026-07-01', fechaFin: '2026-07-31',
})
// 158 - (5+5) de agosto = 148
check('el bloque respeta fecha_inicio/fecha_finalizacion de la pestaña',
    soloJulio.total === 148, `total=${soloJulio.total}`)
...
const muchos: LeadAnswerDatasetLite = {
    ...
    porFecha: { '2026-07-01': [[[0, 1, 50], [1, 1, 40], [2, 1, 30], [3, 1, 20], [4, 1, 10]]] },
}
...
check('aparece el bucket sintético con su recuento',
...
check('los porcentajes con resto agrupado suman 100',
...
check('sin agrupar, los leads recortados se DECLARAN', sinResto.omitidos === 30,
    `omitidos=${sinResto.omitidos}`)
check('sin agrupar, el denominador excluye lo recortado', sinResto.total === 120,
    `total=${sinResto.total}`)
check('sin agrupar, los porcentajes siguen sumando 100',
    Math.abs(sinResto.buckets.reduce((s, b) => s + b.pct, 0) - 100) < 0.05)

// ════════════════════════════════════════════════════════════════
seccion('Comparativo con el período anterior')

const PREV = ds({ '2026-06-15': [[[0, 1, 5], [1, 1, 50], [2, 1, 30]]] })
...
check('el delta compara contra el mismo bucket del período anterior',
...
check('un denominador en cero devuelve null, no Infinity ni +100%',
    bMas5.delta === null && bMas5.leadsPrevio === 0,
    `delta=${bMas5.delta}`)

check('el recorte de fechas de la pestaña NO se aplica al período anterior',
    agregarRespuestas(BASE, { ...DEF, showDelta: true }, '', [],
        { prev: PREV, fechaInicio: '2026-07-01', fechaFin: '2026-07-31' })
...
const PREV_OTRO_ORDEN: LeadAnswerDatasetLite = {
    ...
    porFecha: { '2026-06-15': [[[0, 1, 30], [1, 1, 50]]] },
}
check('el emparejamiento del comparativo es por etiqueta, no por índice',
    agregarRespuestas(BASE, { ...DEF, showDelta: true }, '', [], { prev: PREV_OTRO_ORDEN })
        .buckets.find(b => b.label === '$2M a $3M')?.leadsPrevio === 50)
...
const campoUnificado = {
    ...campoSintetico('rango', 'Rango de ingresos',
        ['cual_es_tu_rango_de_ingresos', 'cual_es_tu_rango_aproximado_de_ingresos']),
...
check('las dos escrituras del mismo rango caen en el mismo bucket',
...
const unificado = ds({ '2026-07-01': [[[1, 1, 6815], [1, 2, 2099]]] })
check('el bloque suma las dos vías en una sola barra',
    agregarRespuestas(unificado, DEF, '', []).buckets.find(b => b.label === '$2M a $3M')?.leads === 8914)

// ════════════════════════════════════════════════════════════════
seccion('Campo sintético (pregunta auto-detectada)')

const auto = campoSintetico('auto:x', 'Rango de renta', ['rango_de_renta'])
check('sin agrupación configurada, el valor normalizado ES el bucket',
...
check("con sin_mapear='ignorar' el valor se descarta",
    bucketDeValor({ ...auto, sin_mapear: 'ignorar' }, 'lo que sea') === null)

// ════════════════════════════════════════════════════════════════
seccion('Estados degradados')

check('un dataset vacío se declara ausente, no como 0 leads',
    agregarRespuestas({ campanas: [], campanaIds: [], campos: [], porFecha: {}, incompleto: false },
        DEF, '', []).campoAusente)
check('un campo borrado del catálogo se declara ausente',
    agregarRespuestas(BASE, { ...DEF, clave: 'ya_no_existe' }, '', []).campoAusente)
check('un bloque auto sin claves no encuentra campo',
    agregarRespuestas(BASE, { ...DEF, origen: 'auto', clave: undefined, clavesOrigen: [] }, '', []).campoAusente)
check('un bloque auto localiza su campo por las claves de origen',
    !agregarRespuestas(
...
check('ocultarVacios quita los buckets en cero',
    !conVacios.buckets.some(b => b.leads === 0))
...
const CON_TOTALES: LeadAnswerDatasetLite = {
    ...ds({ '2026-07-01': [[[0, 1, 15], [1, 1, 5], [2, 1, 3]]] }),
...

{
    ...
    check('utm_leads es el total de contactos del día', k[CLAVE_TOTAL_LEADS] === 40, String(k[CLAVE_TOTAL_LEADS]))
    check('cada respuesta tiene su clave con su conteo',
        k[claves[0].clave] === 15 && k[claves[1].clave] === 5 && k[claves[2].clave] === 3,
        JSON.stringify(claves.map(c => [c.clave, k[c.clave]])))
    check('(sin respuesta) recoge a los que no contestaron',
        k[claveSinRespuesta(campo)] === 17, String(k[claveSinRespuesta(campo)]))

    // La invariante que hace legible la tabla diaria.
    const sumaBuckets = claves.reduce((s, c) => s + (k[c.clave] ?? 0), 0) + (k[claveSinRespuesta(campo)] ?? 0)
    check('respuestas + (sin respuesta) == utm_leads', sumaBuckets === k[CLAVE_TOTAL_LEADS],
    ...
    check('los buckets sin leads emiten 0 explícito',
        k[claves[3].clave] === 0, String(k[claves[3].clave]))
}

{
    ...
    check('utm_leads respeta el filtro de campaña', k[CLAVE_TOTAL_LEADS] === 30, String(k[CLAVE_TOTAL_LEADS]))
    check('las claves por respuesta también lo respetan',
        k[claves[0].clave] === 15 && k[claves[1].clave] === 0,
        JSON.stringify([k[claves[0].clave], k[claves[1].clave]]))
    check('(sin respuesta) se recalcula sobre el total filtrado',
...
// ════════════════════════════════════════════════════════════════
{
    ...
    check('el reparto por respuesta es el esperado',
        dias[0].porBucket.slice(0, 3).join(',') === '15,5,3', dias[0].porBucket.join(','))
    check('cada fila cierra: total = respuestas + sin responder',
...
}
{
    ...
    check('un día sin ninguna respuesta no desaparece de la tabla', dias.length === 2,
        dias.map(d => d.fecha).join(','))
    const mudo = dias.find(d => d.fecha === '2026-07-02')!
    check('ese día muestra su total y todo en (sin respuesta)',
        mudo.total === 12 && mudo.sinRespuesta === 12)
}
{
    const { dias } = serieDiaria(CON_TOTALES, DEF, 'Medellin', [])
    check('la tabla diaria respeta el filtro de campaña',
