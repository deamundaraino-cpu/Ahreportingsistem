...
async function main() {
    ...
    const { data: csRaw } = await db.schema('report_utm')
    ...
    const abortaCon = async (
        nombre: string, esperado: string, fn: () => PromiseLike<{ error: unknown }>
    ) => {
    ...
    await abortaCon('una tabla que no es lead/sales aborta', 'p_tabla debe ser', () =>
        rutm.rpc('bi_valores_conteo', {
            p_cliente_id: null, p_tabla: 'clientes',
            p_desde: `${DESDE}T00:00:00Z`, p_hasta: `${HASTA}T00:00:00Z`,
            p_columna: 'utm_source', p_claves_json: null, p_limite: 10,
        }))
    ...
    await abortaCon('una columna fuera de la lista blanca aborta', 'no permitida en lead_events', () =>
        rutm.rpc('bi_valores_conteo', {
            p_cliente_id: null, p_tabla: 'lead_events',
            p_desde: `${DESDE}T00:00:00Z`, p_hasta: `${HASTA}T00:00:00Z`,
            p_columna: 'lead_email', p_claves_json: null, p_limite: 10,
        }))

    await abortaCon('un intento de inyección por identificador aborta', 'no permitida en lead_events', () =>
        rutm.rpc('bi_valores_conteo', {
            p_cliente_id: null, p_tabla: 'lead_events',
            p_desde: `${DESDE}T00:00:00Z`, p_hasta: `${HASTA}T00:00:00Z`,
            p_columna: "utm_source; DROP TABLE report_utm.lead_events; --",
            p_claves_json: null, p_limite: 10,
        }))

    await abortaCon('raw_fields sobre sales_events aborta', 'raw_fields solo existe en lead_events', () =>
        rutm.rpc('bi_valores_conteo', {
            p_cliente_id: null, p_tabla: 'sales_events',
            p_desde: `${DESDE}T00:00:00Z`, p_hasta: `${HASTA}T00:00:00Z`,
            p_columna: null, p_claves_json: ['x'], p_limite: 10,
        }))
    ...
    await abortaCon('columna y claves a la vez aborta', 'no ambos ni ninguno', () =>
        rutm.rpc('bi_valores_conteo', {
            p_cliente_id: null, p_tabla: 'lead_events',
            p_desde: `${DESDE}T00:00:00Z`, p_hasta: `${HASTA}T00:00:00Z`,
            p_columna: 'utm_source', p_claves_json: ['x'], p_limite: 10,
        }))
    ...
    await abortaCon('una columna de Hotmart no permitida aborta', 'no permitida', () =>
        db.rpc('hotmart_valores_conteo', {
            ...
            p_columna: 'comprador_email', p_desde: DESDE, p_hasta: HASTA, p_limite: 10,
        }))

    // Un valor con comillas y comodines tiene que sobrevivir como literal.
    {
        const { error } = await rutm.rpc('bi_valores_conteo', {
            p_cliente_id: mayor.id, p_tabla: 'lead_events',
            p_desde: `${DESDE}T00:00:00Z`, p_hasta: `${HASTA}T00:00:00Z`,
            p_columna: null, p_claves_json: ["o'brien%_"], p_limite: 10,
        })
        check('una clave JSONB con comillas y comodines no rompe la consulta', !error,
            String((error as { message?: string } | null)?.message ?? ''))
    }

    const { count: sigueViva } = await db.schema('report_utm').from('lead_events')
    ...
    // ORDER BY y qué valores llegaban era indefinido.
    {
        const r = await runValores({
            cliente_id: mayor.id, dimension: 'utm_campaign',
            date_from: DESDE, date_to: HASTA, limit: 5,
        })
        check('devuelve como mucho el límite', r.valores.length <= 5, `${r.valores.length}`)
        const orden = r.valores.map(v => v.n)
        check('vienen ordenados por frecuencia descendente',
            orden.every((n, i) => i === 0 || orden[i - 1] >= n), orden.join(','))
        check('el total refleja el conjunto entero, no el recorte',
            r.total > r.valores.length, `total=${r.total} devueltos=${r.valores.length}`)
        check('se declara truncado', r.truncado)
        check('todos traen un recuento positivo', r.valores.every(v => v.n > 0))
    }
    ...
    {
        const r = await runValores({
            cliente_id: mayor.id, dimension: 'utm_source',
            date_from: DESDE, date_to: HASTA, limit: 500,
        })
        ...
        const { count: totalRango } = await db.schema('report_utm').from('lead_events')
            .select('id', { count: 'planned', head: true })
            .eq('cliente_id', mayor.id)

        check('la suma de los recuentos no supera los leads del cliente',
            suma <= (totalRango ?? Number.MAX_SAFE_INTEGER),
            `suma=${suma} leads=${totalRango}`)
        check('hay al menos un valor', r.valores.length > 0)
        check('sin truncar, el total es el número de valores',
    ...
    // ════════════════════════════════════════════════════════════
    {
        ...
        check('runDistinctValues devuelve string[]',
            Array.isArray(planos) && planos.every(v => typeof v === 'string'))
        check('y es exactamente la proyección del motor',
    ...
    // un slicer sobre ventas devolvía nombres de formulario igualmente.
    {
        const enVentas = await runValores({
            cliente_id: mayor.id, dimension: 'form_name',
            date_from: DESDE, date_to: HASTA, source: 'sales',
        })
        check('un campo de leads pedido sobre ventas no devuelve valores',
            enVentas.valores.length === 0)
        // Y lo dice: una lista vacía muda sería indistinguible de «no hay datos».
        check('y explica por qué', enVentas.motivo === 'dimension_no_listable',
            String(enVentas.motivo))

        const enLeads = await runValores({
            cliente_id: mayor.id, dimension: 'form_name',
            date_from: DESDE, date_to: HASTA, source: 'leads',
        })
    ...
    for (const [nombre, dim] of ramas) {
        const t0 = Date.now()
        const r = await runValores({
            cliente_id: mayor.id, dimension: dim as never,
            date_from: DESDE, date_to: HASTA,
        })
        const ms = Date.now() - t0
        check(`${nombre} (${dim}) — ${ms} ms · ${r.valores.length} valores`, ms < PRESUPUESTO_MS,
    ...
    // ════════════════════════════════════════════════════════════
    {
        const base = { cliente_id: mayor.id, dimension: 'utm_campaign' as never, date_from: DESDE, date_to: HASTA }
        ...
        check('y devuelven lo mismo',
            a.valores.length === b.valores.length && a.total === b.total)
    }
}
