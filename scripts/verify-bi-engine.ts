...
check(
  'una derivada anidada arrastra la intermedia',
  e3.derived.includes('cuenta.hotmart_revenue_calc'),
  JSON.stringify(e3.derived)
);
check(
  'y llega a las columnas físicas',
...
check(
  'un promedio ponderado arrastra su peso',
  e4.leaves.includes('cuenta.ga_sessions'),
  JSON.stringify(e4.leaves)
);
...
check(
  'un campo calculado arrastra sus operandos',
...
check(
  '«dias» NO arrastra ninguna fuente',
  JSON.stringify(e6.leaves) === '["ads.spend"]',
  JSON.stringify(e6.leaves)
);
...
check('leads.count → [leads]', JSON.stringify(planSourceIds(p(['leads.count']))) === '["leads"]');
check('ads.spend → [ads]', JSON.stringify(planSourceIds(p(['ads.spend']))) === '["ads"]');
...
check(
  'una tabla con 5 métricas de 3 fuentes → esas 3',
  JSON.stringify(
    planSourceIds(p(['ads.spend', 'leads.count', 'leads.cpl', 'sales.count', 'sales.roas']))
  ) === '["ads","leads","sales"]'
);
check('subs.active → [subs]', JSON.stringify(planSourceIds(p(['subs.active']))) === '["subs"]');
...
check(
  'CPL hereda el motivo de su operando',
  planSinEnlace.unavailable['leads.cpl']?.kind === 'no_public_link'
);
check(
  'leads.count NO queda marcado (sí se puede medir)',
  planSinEnlace.unavailable['leads.count'] === undefined
);

const todasPublicas = p(
  ['ads.spend', 'cuenta.ga_sessions', 'offline.leads', 'subs.active'],
  null,
  SIN_ENLACE
);
check(
  'las 4 fuentes públicas se caen juntas',
...
check(
  'agrupando por país, el gasto se cae',
  JSON.stringify(planSourceIds(porPais)) === '["leads"]'
);
check('con motivo grain_mismatch', porPais.skipped[0]?.reason.kind === 'grain_mismatch');
check(
  'y el eje que falla se nombra',
...
check(
  'una fuente sin configurar se reporta como not_configured',
  sinVentas.skipped[0]?.reason.kind === 'not_configured'
);
check(
  'ventas queda como desconocido, no como 0',
...
const dobleProblema = skipReasonFor(
  REG.source('cuenta')!,
  { hasPublicLink: false },
  REG.dimensionField('leads.campaign')!
);
check(
  'falta de enlace manda sobre el desajuste de grano',
...
check(
  'la intermedia se calcula ANTES de la que la usa',
  orden.indexOf('cuenta.hotmart_revenue_calc') < orden.indexOf('cuenta.hotmart_roas'),
  JSON.stringify(orden)
);
check(
  'una física no entra en el orden de derivadas',
...
check(
  'CPL con 0 leads → null (no Infinity, no 0)',
...
check(
  'ROI Hotmart sin facturación → null, NO −100%',
...
check(
  'un campo calculado sobre una derivada nula da null, no 0',
...
check(
  'ROAS con gasto 0 → null',
  calcular(['sales.roas'], { 'sales.revenue': 3000, 'ads.spend': 0 })['sales.roas'] === null
);
check(
  'Conv. Rate = ventas / leads',
  calcular(['leads.conversion_rate'], { 'sales.count': 5, 'leads.count': 100 })[
...
check(
  'CPL de una campaña sin gasto → null',
  unidas.find((r) => r.key === 'Promo C')!.values['leads.cpl'] === null
);
check(
  'CPL de una campaña completa se calcula',
...
check('el gasto de la fuente caída es null, no 0', unidasCaido[0].values['ads.spend'] === null);
...
check(
  'suma las aditivas',
  totales['ads.spend'] === 800 && totales['leads.count'] === 27,
...
check(
  'el CPL total se recalcula sobre los totales (800/27)',
  totales['leads.cpl'] === 29.63,
  String(totales['leads.cpl'])
);

const sinNada = totalsRow(REG, [{ key: 'a', values: { 'ads.spend': null } }], ['ads.spend']);
