...
const val = (v: string, filas: number): CampoValorCrudo => ({
  valor_crudo: v,
  valor_norm: normalizarValorCrudo(v),
  filas,
  origenes: [],
  ultima_fecha: null,
});
...
check(
  'makeLeadFieldDim/parse van y vuelven',
  parseLeadFieldDim(makeLeadFieldDim('rango_de_ingresos')) === 'rango_de_ingresos'
);
check(
  'un token de campo de lead NO se confunde con uno de clave cruda',
  isLeadFieldDim('leadfield:x') && !isFieldDim('leadfield:x')
);
check(
  'un token de clave cruda NO se confunde con uno de catálogo',
  isFieldDim('field:x') && !isLeadFieldDim('field:x')
);
check(
  'sin catálogo la etiqueta humaniza la clave',
  leadFieldLabel('leadfield:rango_de_ingresos') === 'Rango de ingresos'
);
check(
  'con catálogo manda el nombre del analista',
  leadFieldLabel('leadfield:r1', [
    {
      clave: 'r1',
...
check(
  'filtrar por un campo de lead anula el gasto (no es atribuible)',
  hasNonAttributableFilter({ 'leadfield:r1': '$2M a $3M' }, undefined)
);
check(
  'un campo de lead en el filtro avanzado también anula el gasto',
  hasNonAttributableFilter(undefined, {
    groups: [{ conditions: [{ field: 'leadfield:r1', op: 'eq', value: 'x' }] }],
  })
);
...
check(
  'el filtro de campo de lead viaja al endpoint',
  qs.get('filters[leadfield:r1]') === '$2M a $3M' && qs.get('filters[field:otra]') === 'y'
);
check(
  'la firma de filtros incluye los campos de lead',
...
check(
  'la clave se compara sin acentos, signos ni mayúsculas',
  normalizarClaveLead('¿Cuál es tu rango de ingresos?') ===
    normalizarClaveLead('cual_es_tu_rango_de_ingresos')
);
check(
  '"Rango de renta" y "rango_de_renta" son la misma clave',
...
check(
  'el lead de Meta cae en el bucket',
  bucketDeLeadRaw(goodprop, leadMeta) === '$2M a $4M',
  String(bucketDeLeadRaw(goodprop, leadMeta))
);
check(
  'el lead de la web cae en el MISMO bucket',
  bucketDeLeadRaw(goodprop, leadWeb) === '$2M a $4M',
  String(bucketDeLeadRaw(goodprop, leadWeb))
);
check('un lead que no respondió no cuenta', bucketDeLeadRaw(goodprop, { nombre: 'Ana' }) === null);
check('un lead sin raw_fields no cuenta', bucketDeLeadRaw(goodprop, null) === null);
...
check(
  '"a" y "–" caen en el mismo bucket',
  bucketDeLeadRaw(surProfundo, { 'rango de renta': 'Entre $2.000.000 a $3.000.000' }) ===
    '$2M a $3M' &&
    bucketDeLeadRaw(surProfundo, { 'rango de renta': 'Entre $2.000.000 – $3.000.000' }) ===
      '$2M a $3M'
);

check(
  'el índice de raw_fields se reutiliza entre campos',
  bucketDeLead(
    surProfundo,
    indexarRawFields({ 'Rango de renta': 'Entre $2.000.000 a $3.000.000' })
  ) === '$2M a $3M'
);

// sin_mapear
check(
  'sin_mapear=crudo deja el valor normalizado',
  bucketDeLeadRaw(campo({ claves_origen: ['x'] }), { x: 'Algo Nuevo' }) === 'algo nuevo'
);
check(
  'sin_mapear=otros agrupa lo no mapeado',
  bucketDeLeadRaw(campo({ claves_origen: ['x'], sin_mapear: 'otros' }), { x: 'Algo' }) === '(otros)'
);
check(
  'sin_mapear=ignorar descarta lo no mapeado',
...
check(
  'rangos DISTINTOS no se funden',
  firmaDeValorLead('Entre $2.000.000 a $3.000.000') !==
    firmaDeValorLead('Entre $2.000.000 a $4.000.000')
);
check(
  'no destroza palabras con "a" ("casa" sigue siendo "casa")',
...
check(
  'auto-agrupar propone el valor MÁS FRECUENTE como nombre del grupo',
  sugerido[normalizarValorCrudo('Entre $2.000.000 – $3.000.000')] ===
    'Entre $2.000.000 a $3.000.000',
  JSON.stringify(sugerido)
);
check(
  'auto-agrupar no toca los valores sin variantes',
...
check(
  'los rangos salen de menor a mayor, no alfabéticamente',
  JSON.stringify(ordenarBuckets(conOrden, ['Más de $3M', 'Menos de $2M', '$2M a $3M'])) ===
    JSON.stringify(['Menos de $2M', '$2M a $3M', 'Más de $3M'])
);
check(
  'un valor nuevo sin orden definido va al final',
  ordenarBuckets(conOrden, ['Más de $3M', 'Sin respuesta', 'Menos de $2M']).at(-1) ===
    'Sin respuesta'
);
check(
  'sin orden configurado, alfabético',
