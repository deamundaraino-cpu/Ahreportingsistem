...
check(
  '20:00 COL del 15 (= 01:00 UTC del 16) es el día 15',
  colombiaDateOf('2026-07-16T01:00:00+00:00') === '2026-07-15',
  colombiaDateOf('2026-07-16T01:00:00+00:00')
);
check(
  '23:59 COL del 15 sigue siendo el día 15',
  colombiaDateOf('2026-07-16T04:59:00Z') === '2026-07-15'
);
check('00:00 COL del 16 ya es el día 16', colombiaDateOf('2026-07-16T05:00:00Z') === '2026-07-16');
check(
  '19:00 COL del 15 (el borde exacto) es el día 15',
  colombiaDateOf('2026-07-16T00:00:00Z') === '2026-07-15'
);
check('18:59 COL del 15 es el día 15', colombiaDateOf('2026-07-15T23:59:00Z') === '2026-07-15');
...
check(
  '03:00 UTC del 15 es el día 14 en Colombia',
  colombiaDateOf('2026-07-15T03:00:00Z') === '2026-07-14'
);

seccion('colombiaDateOf: formatos y bordes');
check(
  'acepta el formato de PostgREST (+00:00)',
  colombiaDateOf('2026-07-15T18:30:00+00:00') === '2026-07-15'
);
check('acepta microsegundos', colombiaDateOf('2026-07-15T18:30:00.123456+00:00') === '2026-07-15');
check('acepta un objeto Date', colombiaDateOf(new Date('2026-07-16T01:00:00Z')) === '2026-07-15');
...
check(
  'un yyyy-MM-dd suelto se devuelve intacto (NO se retrasa)',
...
check(
  '01/08 01:00 UTC es el 31/07 en Colombia',
  colombiaDateOf('2026-08-01T01:00:00Z') === '2026-07-31'
);
check(
  '01/01 02:00 UTC es el 31/12 del año anterior',
  colombiaDateOf('2027-01-01T02:00:00Z') === '2026-12-31'
);
// Año bisiesto: 2028 lo es.
check(
  '01/03 03:00 UTC de un año bisiesto es el 29/02',
...
check('el límite inferior lleva la zona explícita', b.gte === '2026-07-01T00:00:00-05:00', b.gte);
...
check(
  'el límite superior es el día SIGUIENTE, exclusivo',
  b.lt === '2026-08-01T00:00:00-05:00',
  b.lt
);

// Comprobación de que los límites encierran exactamente el rango pretendido.
const dentro = (iso: string) =>
  Date.parse(iso) >= Date.parse(b.gte) && Date.parse(iso) < Date.parse(b.lt);
...
check('las 20:00 COL del 30 de junio ya NO se cuelan en julio', !dentro('2026-07-01T01:00:00Z'));

seccion('Rangos de un solo día');
const un = colombiaRangeBounds('2026-07-15', '2026-07-15');
const dentroUn = (iso: string) =>
  Date.parse(iso) >= Date.parse(un.gte) && Date.parse(iso) < Date.parse(un.lt);
check('un día cubre 24 horas exactas', Date.parse(un.lt) - Date.parse(un.gte) === 24 * 3600 * 1000);
...
check(
  'colombiaDateOf(ahora) === colombiaToday()',
...
check(
  'las 24 horas del día caen donde deben (corte en 05:00 UTC)',
  malas === 0,
  `${malas} hora(s) mal`
);
