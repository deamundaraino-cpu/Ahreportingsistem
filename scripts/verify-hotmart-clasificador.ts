...
import {
    clasificarVenta, matchesAny, leerFunnel, cobertura,
...
type EntradaVenta = Pick<
    VentaHotmart, 'oferta_codigo' | 'es_order_bump' | 'parent_transaction_id' | 'producto_nombre'
>
...
function clasificadorViejo(
    ...
    for (const f of funnels) {
        if (matchesAny(nombreProducto, f.principal_patterns)) return { tipo: 'principal', tab_id: f.tab_id }
...
const nombres = [
    'Camaradictos Pro', 'Camaradictos Basico', 'Bump Camaradictos', 'Upsell Camaradictos',
    'Tributario Pro', 'Plantillas Excel', 'Producto Suelto', '', 'CAMARADICTOS%raro',
]
let paridad = true
...
check('un funnel sin ofertas clasifica igual que el worker actual', paridad, divergencias.join(' | '))
check('el origen se reporta como «nombre»',
    clasificarVenta(v({ producto_nombre: 'Camaradictos Pro' }), funnelsHeredados).origen === 'nombre')
check('el primer funnel gana',
    clasificarVenta(v({ producto_nombre: 'Camaradictos Pro' }), funnelsHeredados).tab_id === 'tab-A')
check('sin match → sin_clasificar',
    clasificarVenta(v({ producto_nombre: 'Producto Suelto' }), funnelsHeredados).tipo === 'sin_clasificar')
...
check('renombrar el producto ya no rompe la clasificación',
...
const bumpConPadre = clasificarVenta(
    v({ es_order_bump: true, parent_transaction_id: 'HP000' }), [funnel()],
)
...
const bumpConNombre = clasificarVenta(
    v({ es_order_bump: true, producto_nombre: 'Camaradictos Pro' }), funnelsHeredados,
)
...
check('sin claves nuevas, las listas quedan vacías',
...
check('los códigos de oferta NO se pasan a minúsculas',
    nuevo!.principal_offers.includes('A1B2C3'))
