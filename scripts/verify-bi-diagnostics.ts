...
async function main() {
  ...
  const { explainSkipReason, sanitizeForClient } =
    await import('../src/lib/report-utm/bi/diagnostics');
  ...
  if (!sinEnlace) {
    ...
    check(
      'el gasto se marca como no disponible',
      r.meta?.unavailable['spend']?.kind === 'no_public_link',
      JSON.stringify(r.meta?.unavailable)
    );
    check(
      'CPL hereda el motivo (no muestra un 0 inventado)',
      r.meta?.unavailable['cpl']?.kind === 'no_public_link'
    );
    check(
      'los leads NO se marcan (sí se pueden medir)',
      r.meta?.unavailable['leads_count'] === undefined
    );
    check(
      'la fuente de anuncios se reporta como descartada',
      r.meta?.skipped.some((s) => s.sourceId === 'ads' && s.reason.kind === 'no_public_link') ===
        true
    );

    const motivo = r.meta?.skipped.find((s) => s.sourceId === 'ads');
    check(
      'el motivo tiene texto en español para la UI',
    ...
    check(
      'y en efecto el gasto salía 0 (de ahí el problema)',
      gasto === 0 || gasto === undefined,
      String(gasto)
    );
    ...
    check(
      'el motivo interno se neutraliza a «no configurado»',
  ...
  if (conEnlace) {
    ...
    check(
      'el gasto se marca con grain_mismatch',
      r.meta?.unavailable['spend']?.kind === 'grain_mismatch',
      JSON.stringify(r.meta?.unavailable)
    );
    check('los leads sí se desglosan por país', r.meta?.unavailable['leads_count'] === undefined);
    const m = r.meta?.skipped.find((s) => s.sourceId === 'ads');
    check(
      'el texto explica qué hacer en su lugar',
  ...
  if (conEnlace) {
    ...
    check(
      'no se marca nada como no disponible',
  ...
  if (conEnlace) {
    const r = await consultar({
      ...
      metrics: 'leads_count,spend,cpl',
      dimension: 'none',
      'filters[ip_country]': 'CO',
      ...RANGO,
    });
    const u = r.meta?.unavailable['spend'];
    check(
      'el gasto se marca como no atribuible',
      u?.kind === 'unattributable_filter',
      JSON.stringify(u)
    );
    check(
      'y se dice QUÉ filtro lo provoca',
  ...
  if (conEnlace) {
    ...
    check(
      'GA4 agrupado por campaña se marca (es día×cliente)',
    ...
    check('GA4 por fecha NO se marca', porFecha.meta?.unavailable['ga_sessions'] === undefined);
    ...
    check(
      'las suscripciones por fecha se marcan (son una foto)',
