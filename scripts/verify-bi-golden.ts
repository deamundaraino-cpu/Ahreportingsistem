...
const CASOS: Caso[] = [
  ...
  {
    nombre: 'ventas y retorno',
    metrics: ['sales_count', 'revenue', 'roas', 'cpa'],
    dimension: 'none',
  },
  {
    nombre: 'rendimiento de anuncios',
    metrics: ['impressions', 'clicks', 'ctr', 'cpc', 'cpm', 'reach', 'frequency'],
    dimension: 'none',
  },
  {
    nombre: 'hotmart',
    metrics: ['hotmart_revenue', 'hotmart_sales', 'hotmart_roas', 'hotmart_cpa', 'hotmart_roi'],
    dimension: 'none',
  },
  {
    nombre: 'ga4',
    metrics: ['ga_sessions', 'ga_bounce_rate', 'ga_avg_session_duration'],
    dimension: 'none',
  },
  {
    nombre: 'offline',
    metrics: ['offline_leads', 'offline_ventas', 'offline_revenue'],
    dimension: 'none',
  },
  ...
  {
    nombre: 'serie por día',
    metrics: ['leads_count', 'spend'],
    dimension: 'date',
    date_grouping: 'day',
  },
  {
    nombre: 'serie por semana',
    metrics: ['leads_count'],
    dimension: 'date',
    date_grouping: 'week',
  },
  ...
  {
    nombre: 'por campaña (cruzada)',
    metrics: ['spend', 'leads_count', 'cpl', 'sales_count'],
    dimension: 'utm_campaign',
    limit: 20,
    sort: 'desc',
  },
  {
    nombre: 'por campaña CRUDA',
    metrics: ['leads_count'],
  ...
  {
    nombre: 'por país (gasto no cruza)',
    metrics: ['leads_count', 'spend', 'cpl'],
  ...
  {
    nombre: 'por source',
    metrics: ['leads_count', 'sales_count', 'conversion_rate'],
...
async function main() {
  ...
  let desde = opt('desde');
  let hasta = opt('hasta');
  const previo: Snapshot | null = existsSync(BASELINE)
  ...
  if (!desde || !hasta) {
    if (previo && !CAPTURAR) {
      ...
      const iniMesPasado = new Date(
        Date.UTC(finMesPasado.getUTCFullYear(), finMesPasado.getUTCMonth(), 1)
      );
  ...
  console.log(
    `Clientes: ${clientes.map((c) => `${c.nombre}${c.publicLink ? '' : ' (sin enlace)'}`).join(', ')}\n`
  );
  ...
  for (const cliente of clientes) {
    ...
    for (const caso of CASOS) {
      try {
        ...
        const filas = rows.map((r) => {
          ...
          const out: FilaGuardada = {
            __dim: limpiar((r as Record<string, unknown>).dimension_value),
          };
    ...
    try {
      ...
      otros.funnel = { error: (e as Error).message };
      process.stdout.write('!');
    }
    ...
    try {
      ...
      otros.comparacion = { error: (e as Error).message };
      process.stdout.write('!');
    }

    try {
      otros.distinct_campanas = (
        await runDistinctValues({
          cliente_id: cliente.id,
          // eslint-disable-next-line @typescript-eslint/no-explicit-any
      ...
      otros.distinct_campanas = { error: (e as Error).message };
      process.stdout.write('!');
    }
  ...
  if (CAPTURAR) {
    ...
    const nFilas = Object.values(resultados)
      .flat()
      .reduce((a, r) => a + r.filas.length, 0);
    console.log(`\n✓ Línea base guardada en scripts/golden/bi-baseline.json`);
    console.log(
      `  ${clientes.length} clientes × ${CASOS.length} casos = ${nFilas} filas congeladas`
    );
    const conError = Object.values(resultados)
      .flat()
      .filter((r) => r.error);
    if (conError.length) {
  ...
  for (const cliente of clientes) {
    ...
    if (!base) {
      check(`${cliente.nombre}: presente en la base`, false, 'no estaba');
      continue;
    }
    ...
    for (const caso of CASOS) {
      ...
      if (!b || !a) {
        check(etiqueta, false, 'caso ausente');
        continue;
      }
