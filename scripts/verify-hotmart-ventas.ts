...
async function main() {
  ...
  for (const fn of [
    'guardar_hotmart_venta',
    'purgar_hotmart_raw',
    'purgar_hotmart_pii',
    'purgar_sales_events_raw',
    'fusionar_config_api',
  ]) {
    const { error } = await db.rpc(
      fn,
      fn === 'guardar_hotmart_venta'
        ? { p_fila: {} }
  ...
  const { data: filas, error } = await db
    .from('hotmart_ventas')
    .select(
      'id, cliente_id, transaction_id, fecha_venta, aprobada_at, estado, bruto, bruto_usd, tipo, clasificacion_origen, parent_transaction_id, reembolsada_at'
    )
    .limit(5000);
  if (error) {
    check('lectura', false, error.message);
    process.exit(1);
  }
  ...
  check(
    'no hay transacciones duplicadas por cliente',
    dup.length === 0,
    dup.slice(0, 3).join(', ')
  );
  ...
  const ceroSospechoso = todas.filter((f) => Number(f.bruto) > 0 && Number(f.bruto_usd) === 0);
  check(
    'ningún importe se convirtió a 0 teniendo bruto > 0',
    ceroSospechoso.length === 0,
    ceroSospechoso
      .slice(0, 3)
      .map((f) => String(f.transaction_id))
      .join(', ')
  );
  ...
  check(
    'toda venta devuelta tiene reembolsada_at',
    devueltas.every((f) => f.reembolsada_at !== null),
    String(devueltas.filter((f) => !f.reembolsada_at).length)
  );
  const vivasConMarca = todas.filter(
    (f) => (f.estado === 'aprobada' || f.estado === 'completa') && f.reembolsada_at !== null
  );
  check(
    'ninguna venta cobrada arrastra marca de reembolso',
  ...
  const incoherentes = todas.filter(
    (f) => (f.tipo === 'sin_clasificar') !== (f.clasificacion_origen === 'sin_clasificar')
  );
  check(
    'tipo y clasificacion_origen coinciden',
    incoherentes.length === 0,
    incoherentes
      .slice(0, 3)
      .map((f) => `${f.transaction_id}: ${f.tipo}/${f.clasificacion_origen}`)
      .join(' | ')
  );
  ...
  console.log(
    `  por nombre de producto: ${clasificadas - porOferta} (se rompe si renombran el producto)`
  );
  if (pct < 95) {
    console.log(
      `  ⚠ Por debajo del 95%: revisa el mapa de ofertas antes de fiarte del desglose por tipo.`
    );
  }
  ...
  if (conPadre.length > 0) {
    ...
    console.log(
      `  con compra padre: ${conPadre.length}, de los cuales ${huerfanos.length} apuntan fuera de la muestra`
    );
  }
