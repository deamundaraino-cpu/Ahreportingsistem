...
const ORIGENES = [
  ...
  {
    tab_name: 'Formulario filtro logico- Abril 2026 V2',
    columnas: ['cual_es_tu_rango_aproximado_de_ingresos'],
  },
...
async function main() {
  ...
  for (const [i, campo] of CAMPOS.entries()) {
    ...
    console.log(
      `   ${Object.keys(campo.valores_map).length} valores → ${buckets.length ? buckets.join(', ') : '(cada rango por separado)'}`
    );
    if (yaEstan.has(campo.clave)) {
      console.log('   ya existe, no se toca\n');
      continue;
    }
    if (!APLICAR) {
      console.log('   se crearía\n');
      continue;
    }
