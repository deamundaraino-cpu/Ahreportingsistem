...
async function lotesDe(tabla: string, clienteId: string, sheetId: string) {
  ...
  for (let desde = 0; ; desde += 1000) {
    ...
    for (const r of data as any[])
      lotes.set(r.sync_batch_id, (lotes.get(r.sync_batch_id) ?? 0) + 1);
...
async function main() {
  ...
  for (const sheetCfg of sheets) {
    ...
    check(
      peorTiempo < 45,
      'la pestaña más lenta cabe en el límite de la función',
    ...
    check(
      sumaDiarias === sumaConv,
      'los agregados diarios suman lo mismo que las conversiones',
  ...
  console.log(
    fallos === 0 ? '✅ Todas las comprobaciones pasan' : `❌ ${fallos} comprobación(es) fallida(s)`
  );
