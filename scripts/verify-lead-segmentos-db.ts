...
async function main() {
    ...
    const { loadLeadCampos, loadLeadSegmentos } = await import('../src/lib/report-utm/lead-campos-db')
    ...
    for (const c of (clientes ?? []) as any[]) {
        ...
        elegido = c; campos = cs; segmentos = ss
        break
    }
    ...
    const [total] = await runBiQuery({
        cliente_id: elegido.id, date_from: DESDE, date_to: HASTA,
        metrics: [token, 'leads_count', 'spend'] as any, dimension: 'none',
    })
    ...
    const porFecha = await runBiQuery({
        cliente_id: elegido.id, date_from: DESDE, date_to: HASTA,
        metrics: [token] as any, dimension: 'date', date_grouping: 'day',
    })
    const sumaDias = porFecha.reduce((n, r) => n + Number((r as any)[token] ?? 0), 0)
    check('agrupado por fecha suma lo mismo que el total',
        sumaDias === valorTotal, `${sumaDias} vs ${valorTotal}`)

    const porCampana = await runBiQuery({
        cliente_id: elegido.id, date_from: DESDE, date_to: HASTA,
        metrics: [token] as any, dimension: 'utm_campaign',
    })
    const sumaCampanas = porCampana.reduce((n, r) => n + Number((r as any)[token] ?? 0), 0)
    check('agrupado por campaña suma lo mismo que el total',
        sumaCampanas === valorTotal, `${sumaCampanas} vs ${valorTotal}`)
    ...
    const [conFormula] = await runBiQuery({
        cliente_id: elegido.id, date_from: DESDE, date_to: HASTA,
        metrics: ['spend'] as any, dimension: 'none',
    ...
    if (gasto === 0) {
        ...
        check('la fórmula `spend / lseg__…` da un CPL',
            valorTotal === 0 ? cpl === 0 : cpl > 0, `cpl=${cpl}, seg=${valorTotal}, spend=${gasto}`)
        ...
        const [conFiltro] = await runBiQuery({
            cliente_id: elegido.id, date_from: DESDE, date_to: HASTA,
            metrics: ['spend', 'leads_count'] as any, dimension: 'none',
            filters: { [makeLeadFieldDim(campo.clave)]: seg.valores[0] ?? '(otros)' },
        } as any)
        check('un FILTRO por campo de lead sigue anulando el gasto',
            Number(conFiltro?.spend ?? 0) === 0, String(conFiltro?.spend))
    }
    ...
    const ds = await cargarRespuestasLead(
        rtm, elegido.id, DESDE, HASTA, [campo], { [campo.clave]: 'catalogo' }, true,
        { [campo.clave]: segmentos.filter(s => s.campo_clave === campo.clave).map(s => ({
    ...
    if (ds.incompleto) {
        ...
        check('el cubo del dashboard da el mismo conteo que el motor del BI',
            desdeCubo === valorTotal, `dashboard=${desdeCubo} vs bi=${valorTotal}`)
    }
