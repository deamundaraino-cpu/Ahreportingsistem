...
check('01→10 ago devuelve exactamente 3 semanas, la última de un solo día',
    JSON.stringify(caso) === JSON.stringify(esperado),
    JSON.stringify(caso))
check('01→10 ago termina el 10, no el 09 (EL bug)',
...
for (const [desde, hasta, dias] of casosLunes) {
    ...
    check(`${desde}→${hasta} pinta ${dias} días`,
...
check('la primera semana se recorta al inicio del rango (mié→dom)',
    JSON.stringify(corta[0]) === JSON.stringify({ weekNumber: 1, start: '2026-08-05', end: '2026-08-09' }),
    JSON.stringify(corta[0]))
check('la última semana se recorta al final del rango',
    ultima(corta).end === '2026-08-20', ultima(corta)?.end)
...
check('weekNumber empieza en 1 y no es la semana ISO del año',
    mesAgosto[0].weekNumber === 1, String(mesAgosto[0].weekNumber))
check('weekNumber es secuencial 1..n',
    mesAgosto.every((s, i) => s.weekNumber === i + 1))

check('las semanas 2..n empiezan en lunes',
    mesAgosto.slice(1).every(s => esLunes(s.start)),
    mesAgosto.slice(1).map(s => s.start).join(' '))

const invertido = getWeeksInRange('2026-08-12', '2026-08-01')
check('un rango invertido se colapsa a un solo día',
    JSON.stringify(invertido) === JSON.stringify([{ weekNumber: 1, start: '2026-08-12', end: '2026-08-12' }]),
...
for (let i = 0; i < 7; i++) {
    ...
    check(`un solo día (${d}) da 1 semana de 1 día`,
...
for (const [desde, hasta, dias, etiqueta] of cruces) {
    const w = getWeeksInRange(desde, hasta)
    check(`${etiqueta} (${desde}→${hasta}) pinta ${dias} días`,
...
// duplique un día, sin depender de que alguien piense en probar los lunes.
{
    ...
    console.log(`  · ${pares.toLocaleString('es')} pares evaluados, ${paresLunes.toLocaleString('es')} terminan en lunes`)
    check('los días pintados siempre igualan el span del rango', falloSpan === '', falloSpan)
    check('la primera semana empieza en el inicio y la última acaba en el fin', falloBordes === '', falloBordes)
    ...
    check('el barrido cubre más de 5.000 rangos que terminan en lunes',
        paresLunes > 5_000, String(paresLunes))
}
...
for (const mala of invalidas) {
    ...
    check(`'${mala}' como inicio devuelve [] sin lanzar`,
    ...
    check(`'${mala}' como fin devuelve [] sin lanzar`,
...
for (const [y, m, dias, primero, ultimo] of mesesConNombre) {
    ...
    check(`getMonthWeeks(${y}, ${m}) → ${dias} días, ${primero}…${ultimo}`,
...
const diasDelMes = (y: number, m: number) =>
    Math.round((Date.UTC(m === 12 ? y + 1 : y, m === 12 ? 0 : m, 1) - Date.UTC(y, m - 1, 1)) / 86_400_000)
...
for (const y of [2026, 2028]) {
    ...
    for (let m = 1; m <= 12; m++) {
        ...
        if (!fallo && (diasPintados(w) !== dias || w[0]?.start !== primero || ultima(w)?.end !== ultimoDia)) {
