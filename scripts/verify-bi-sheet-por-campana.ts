...
async function main() {
  ...
  let elegido: { id: string; nombre: string; publicId: string; clave: string; agg: string } | null =
    null;
  ...
  for (const c of clientes) {
    ...
    const { data: muestra } = await db
      ...
      .limit(200);
    const atribuibles = ((muestra ?? []) as Array<{ valores: Record<string, string> }>).filter(
      (r) => filaEsAtribuible(r.valores)
    ).length;
    ...
    elegido = {
      ...
      publicId: c.public_cliente_id,
      clave: campo.clave,
      agg: campo.agregacion,
    };
    break;
  }
  ...
  check(
    'la suma por campaña es igual al total',
    Math.abs(total - suma) < 0.01,
  ...
  check(
    'hay más de una campaña en el desglose',
    conCampana.length > 1,
  ...
  const filasUtiles = conGasto.filter(
    (r) =>
  ...
  check(
    'al menos una campaña trae gasto Y campo de Sheet a la vez',
    filasUtiles.length > 0,
    `${filasUtiles.length} fila(s) con ambos`
  );
  ...
  if (filasUtiles.length > 0) {
    ...
    console.log(
      `    gasto ${Number(ej.spend).toFixed(2)} · ${elegido.clave} ${ej[metric]} · costo unitario ${cpl.toFixed(2)}`
    );
    check('el costo por unidad es un número finito y positivo', Number.isFinite(cpl) && cpl > 0);
  }
  ...
  check(
    'el total por fecha sigue coincidiendo con el total',
    Math.abs(total - sumaFecha) < 0.01,
    `total=${total} porFecha=${sumaFecha}`
  );
}
