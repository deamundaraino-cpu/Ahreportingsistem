...
import {
    normLabel, unifiedTarget, UNIFIED_DIMS, platformFromUtm,
    matchFilterCondition, matchFilterConditionNorm,
    platformScopeFromFilters, hasEntityFilter, entityNameFilterPredicate,
    splitEntityGroups, metricCrossesDimension, RECOMMENDED_METRICS,
    METRIC_META, METRIC_GROUP_META, metricsOfGroup, metricGlossary,
    hasNonAttributableFilter, DIMENSION_META,
...
check('un grupo puro de campaña sí restringe',
    hasEntityFilter(undefined, grupoPuro, 'utm_campaign'))
check('un grupo MIXTO no restringe el nombre (su otra rama admite cualquiera)',
...
check('splitEntityGroups separa puro de mixto',
...
check('campo de lead anula el gasto', hasNonAttributableFilter(f({ 'leadfield:ingresos': 'alto' }), undefined))
check('utm_campaign NO anula el gasto (se recorta por nombre)',
    !hasNonAttributableFilter(f({ utm_campaign: 'Promo' }), undefined))
check('utm_source NO anula el gasto (se recorta por plataforma)',
...
check('toda métrica declara grupo y breakdown',
    metricas.every(m => !!METRIC_META[m].group && !!METRIC_META[m].breakdown))
check('todo grupo declarado tiene al menos una métrica',
    METRIC_GROUP_META.every(g => metricsOfGroup(g.key).length > 0))
check('toda métrica pertenece a un grupo declarado',
    metricas.every(m => METRIC_GROUP_META.some(g => g.key === METRIC_META[m].group)))
check('las recomendadas existen en el catálogo',
    RECOMMENDED_METRICS.every(m => !!METRIC_META[m]))
check('las recomendadas cruzan por campaña',
    RECOMMENDED_METRICS.every(m => metricCrossesDimension(m, 'utm_campaign')))
check('las recomendadas cruzan por fecha',
    RECOMMENDED_METRICS.every(m => metricCrossesDimension(m, 'date')))
check('leads cruza por país (es columna de lead_events)',
    metricCrossesDimension('leads_count', 'ip_country'))
check('el gasto NO cruza por país',
    !metricCrossesDimension('spend', 'ip_country'))
check('el gasto SÍ cruza por campaña (esto es lo que se arregló)',
    metricCrossesDimension('spend', 'utm_campaign'))
check('el gasto SÍ cruza por anuncio y conjunto',
...
check('un token de campo de formulario no dispara aviso',
    metricCrossesDimension('fieldagg:sum:presupuesto', 'ip_country'))

seccion('Catálogo: nomenclatura')
check('los tres "leads" tienen etiquetas distinguibles', new Set([
    METRIC_META.leads_count.label, METRIC_META.leads_form.label, METRIC_META.offline_leads.label,
]).size === 3)
check('ya no hay dos "Pagos iniciados" idénticos',
    METRIC_META.initiates_checkout.label !== METRIC_META.hotmart_pagos_iniciados.label)
check('no hay etiquetas duplicadas en todo el catálogo',
...
check('TODA métrica del catálogo tiene glosario',
...
async function verificarDatos() {
    ...
    const { data: clientes } = await db.schema('report_utm').from('clientes')
        .select('id,nombre,public_cliente_id').not('public_cliente_id', 'is', null).limit(50)
    if (!clientes?.length) {
    ...
    const total = await runBiQuery({
        ...base, metrics: ['leads_count', 'spend'] as BiMetric[], dimension: 'none',
    })
    ...
    check(`no se pierde ningún lead (${leadsTotal} → ${leadsCampana})`,
        leadsCampana === leadsTotal,
        `diferencia de ${leadsTotal - leadsCampana}`)
    check(`el gasto cuadra (${spendTotal.toFixed(2)} → ${spendCampana.toFixed(2)})`,
    ...
    const conAmbos = porCampana.filter(r => Number(r.spend ?? 0) > 0 && Number(r.leads_count ?? 0) > 0)
    const sinCruce = porCampana.filter(r => r.__nocross === 1)
    console.log(`  · ${porCampana.length} filas · ${conGasto.length} con gasto · ${conAmbos.length} con gasto Y leads · ${sinCruce.length} sin cruce`)
    if (spendTotal > 0) {
        check('al menos una campaña tiene gasto desglosado', conGasto.length > 0)
        check('al menos una campaña tiene gasto Y leads (el cruce funciona)',
            conAmbos.length > 0,
            'ninguna fila combina ambos: el cruce no está resolviendo')
        check('esas filas tienen CPL calculado',
    ...
    check('las filas sin cruce están marcadas y tienen gasto 0',
    ...
    const { data: md } = await db.from('metricas_diarias')
    ...
    check('ningún par de campañas distintas colapsa al mismo nombre normalizado',
        colisiones.length === 0,
    ...
    for (const dim of ['ad', 'adset'] as const) {
        const filas = await runBiQuery({
            ...base, metrics: ['leads_count', 'spend'] as BiMetric[], dimension: dim, limit: 200,
        })
    ...
    const opciones = await runDistinctValues({
        cliente_id: mejor.id, dimension: 'utm_campaign', date_from: desde, date_to: hasta,
    })
    ...
    check('toda opción del desplegable existe como fila del informe',
        fantasma.length === 0,
    ...
    if (objetivo?.dimension_value) {
        ...
        check('el total filtrado coincide con la fila de la tabla (leads)',
            lf === Number(objetivo.leads_count ?? 0),
            `tabla=${objetivo.leads_count} vs filtro=${lf}`)
        check('el total filtrado coincide con la fila de la tabla (gasto)',
