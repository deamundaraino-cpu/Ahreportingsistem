...
async function sql<T>(query: string): Promise<T[]> {
    ...
    const proj = get('NEXT_PUBLIC_SUPABASE_URL').replace(/.*https:\/\//, '').replace(/\.supabase\.co.*/, '')
    const token = get('SUPABASE_ACCESS_TOKEN')
    if (!proj || !token) throw new Error('Faltan NEXT_PUBLIC_SUPABASE_URL o SUPABASE_ACCESS_TOKEN en .env.local')
...
async function main() {
    ...
    console.log(`   ${pad('Cliente', 26)} ${'Leads'.padStart(8)} ${'Cambian'.padStart(9)} ${'%'.padStart(7)}`)
    let totLeads = 0, totCambian = 0
    for (const r of porCliente) {
        const leads = n(r.leads), cambian = n(r.cambian)
        totLeads += leads; totCambian += cambian
        const pct = leads > 0 ? (cambian / leads) * 100 : 0
        console.log(`   ${pad(String(r.nombre).trim(), 26)} ${String(leads).padStart(8)} ${String(cambian).padStart(9)} ${pct.toFixed(1).padStart(6)}%`)
    }
    const pctTot = totLeads > 0 ? (totCambian / totLeads) * 100 : 0
    console.log(`   ${pad('TOTAL', 26)} ${String(totLeads).padStart(8)} ${String(totCambian).padStart(9)} ${pctTot.toFixed(1).padStart(6)}%`)
    ...
    console.log(`   ${pad('Día', 12)} ${'Hoy (UTC)'.padStart(11)} ${'Nuevo (COL)'.padStart(12)} ${'Dif.'.padStart(8)}`)
    for (const r of porDia) {
        const dif = n(r.diferencia)
        console.log(`   ${pad(String(r.dia).slice(0, 10), 12)} ${String(n(r.antes_utc)).padStart(11)} ${String(n(r.despues_col)).padStart(12)} ${(dif > 0 ? '+' : '') + dif}`.padEnd(4))
    }
    ...
    if (!entregas.length) {
        ...
        for (const e of entregas) {
            console.log(`     · ${pad(String(e.nombre), 28)} ${e.period_label}  (${String(e.date_from).slice(0, 10)} → ${String(e.date_to).slice(0, 10)})`)
        }
    }
